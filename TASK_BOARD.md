...
| Optical-density residual colour | AZ0 development pass / AZ1 ready | Fixed `.15/.35` neutral/opponent density residual beats AO6 on automatic style/non-basic metrics with zero new clipping and receives `6/7/5` blind preferences; 2/3 rounds pass with zero confirmed severe artifacts. Keep only as Look Approximation until unchanged fresh-population confirmation. |
...
| 1s | U5.R2AZ1 | Confirm the fixed AZ0 optical-density operator on an independent image population | ready | no retuning; exact operator/gates; severe veto before preference; no stock/calibration/product claim |
