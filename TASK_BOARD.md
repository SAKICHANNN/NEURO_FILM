...
| Physical image formation | P4 stationary reference execution closed; distinct physical algorithm leaf next | P4N closes hard routing. P4O exposes a legacy CPU timeout; P4P/Q retain an exact opt-in scalar-CDF stationary executor that completes two 24MP streams in 60-68s at about 284MB with unchanged legacy small hashes. No nonstationary, measured-film, calibration or product claim opens. |
...
| Controlled film wedge sources | P2K downloading; P2L1/P5E closed | Apollo 16 segment 05 has only two endpoint-like levels and its otherwise well-sampled boundary is 127px wide, so both sensitometry and MTF/kernel fitting close. Retain broad-transition stress only. Apollo 7 SO-368 P2K remains an incomplete stream and partial bytes are not evidence. |
| Compact explicit colour capacity | AS0 absolute pass; AS1 selects simpler baseline | Gaussian residual improves over the global base, but equal-parameter trilinear log-odds wins 3/4 analytic targets and lowers median RMSE by 2.28% relative to Gaussian. Keep trilinear only as a future new-controlled-pair baseline; AO9's 71-pair pool stays sealed. |
