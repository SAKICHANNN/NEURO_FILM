...
| Complete fixed-bank Oracle | BH0 complete / selector-router closed | Two independent 50-render runs are byte-identical and automatic/severe-clean. Complete three-round rankings select fixed B0 as the best global arm; leave-one-round-out hard choices improve only 2/3 folds by 2 ranks total (3.85%) and only three sources have a stable non-global choice, failing the frozen 6-rank/8%/four-source gates. No selector/router; next use a new population to confirm B0 versus AO6 globally. |
