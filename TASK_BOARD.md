...
| 1cf | U5.R2S3D | Fit explicit flow from independent neutral/styled distributions | ready: synthetic pilot frozen | SW-24 vs RFF-MMD-192 on 8 styles/2 replicates; distribution fit and hidden operator recovery both mandatory; local deterministic GPU allowed |
| 1cg | U5.R2S4D | Test diversified conditional distribution matching with one shared explicit flow | pending S3 repeated `distribution_fail` | pooled/correct/shuffled RFF-MMD controls; generated condition truth only; real-data auxiliary-condition DoR remains absent |
