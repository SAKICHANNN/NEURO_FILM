...
| AO6 procedural FilmFX value | BC0 legacy arms closed automatically | Fixed monochrome encoded-RGB grain is deterministic/material and adds no raw clipping, but per-channel composition clipping creates `.01302` high-frequency chroma against `.004`; visual review is forbidden. Next test shared linear optical-density grain, not a strength rescue. |
