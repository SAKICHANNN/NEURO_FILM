...
| 1cg | U5.R2S4D | Test diversified conditional distribution matching with one shared explicit flow | ready: S3 activation passes | pooled/correct/shuffled RFF-MMD controls; generated condition truth only; real-data auxiliary-condition DoR remains absent |
...
| 1cj | U5.R2U1D | Compare clean-room HCC pseudo-pairs using the bounded O0 flow | implemented; execution pending repeated S4 | isolated HCC/pair-fit/runner passes seven tests and rejects absent activation without output; new seeds and correct/random/pooled/shuffled controls remain untouched |
...
| 1cm | U5.R2W1D | Test fixed reference-look retrieval/regression across generated content | implemented; execution queued behind S4/U1 | fixed descriptor/bank, hard direction plus continuous-strength retrieval, single/multi ridge and activation/partition-gated runner pass twelve targeted / 946 full tests; exact tiny repeat covers single/four, seen/unseen, paired upper bound and `53/55/56`; formal seeds untouched |
| 1cn | U5.R2W2F | Test a surviving output-only mechanism on local FilmSet recipes | source/partition preflight repeat-pass; execution queued behind W1 | commit-bound `AB435E24...` repeat freezes 512 target refs, 256 source probes, 256/128/81 internal partitions with zero pool leakage/pixel access; global-explainability first; no final-628 reuse, film/stock claim or capacity rescue |
