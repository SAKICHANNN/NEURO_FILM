...
| Physical image formation | P3L scene-linear backing-return topology closed; attribution only | The unchanged P1-forward + P3D-additive return still fails on nine independent CC0 RAW scene-linear inputs: max local delta `.36101` and 5,563 isolated excursions, despite zero new hard boundaries. P3M may attribute the fixed failure spatially but cannot retune or rescue it. |
