...
| Physical image formation | P3P/P5H visible-value fail; P5F runtime closed | P3P's constant-preserving positive halo is structurally clean and red-weighted, but fixed physical energy reaches only `.004091 < .01` response; no amplification. P5F changes 2.09% of codes yet all 27 blind pairs tie. Retain numerical evidence only; physical effects still lack independent visible value. |
...
| Parallel projection-curve operator | BN2 untouched-content confirmation pass; BN3 ready | On disjoint AY2 content the unchanged source-adaptive explicit operator improves mean 5.35% over its global counterpart, wins 60.94%, improves P95/worst tails, retains 1.0226x AO6 style and has zero cube/boundary/nonpositive-Jacobian failures. Freeze it; compare adaptive vs global visually on an independent population before any integration. |
