...
from scripts.build_reference_match_integration_manifest_v19 import (
    CLAIM_CEILING,
    PAYLOAD_SCOPE,
...
MANIFEST = (
    ROOT / "configs/reference_match_main_integration_manifest_v19.json"
)
SCHEMA = (
    ROOT
    / "configs/schemas/"
    "reference_match_main_integration_manifest_v19.schema.json"
)
PRIOR = ROOT / "configs/reference_match_main_integration_manifest_v18.json"
...
def test_frozen_v19_rebuilds_and_preserves_v18() -> None:
    frozen = _manifest()
...
def test_v19_binds_bmkl_diagnostic_without_contract_drift() -> None:
    manifest = _manifest()
    ...
    assert "docs/evidence/P113_BMKL_FAILURE_SIGNATURES.json" in files
    assert (
        "scripts/analyze_dpct_invocation_failure_signatures.py"
        in files
    )
    assert (
...
@pytest.mark.parametrize(
    "mutate",
    [
        lambda value: value.update(payload_scope="P1-P114"),
...
def test_v19_tamper_fails_closed(mutate) -> None:
...
def test_v19_schema_rejects_before_git_access(tmp_path: Path) -> None:
    manifest = deepcopy(_manifest())
