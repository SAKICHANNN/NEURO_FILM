...
from scripts.audit_p315_r1gl_nikon_nef_no_copy_intake import execute

ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "configs/p315_r1gl_nikon_nef_no_copy_intake_v1.json"
PRODUCER = ROOT.parent / "追色"


def test_p315_contract_is_frozen_and_claim_is_bounded() -> None:
    config = json.loads(CONFIG.read_text(encoding="utf-8"))
    assert config["status"] == "FROZEN_BEFORE_FIRST_CONSUMER_DECODE"
    assert config["protocol"] == "zhuise.nikon-nef-compression34713-unpack.v1"
    assert len(config["sources"]) == 2
    assert "no generic Nikon NEF" in config["claim_ceiling"]
...
def test_p315_exact_no_copy_intake(order: str) -> None:
    report = execute(CONFIG, PRODUCER, order)
    assert report["status"] == "PASS_PRIVATE_R1GL_NIKON_NEF_NO_COPY_INTAKE"
