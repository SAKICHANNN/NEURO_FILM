...
from scripts.build_reference_match_integration_manifest_v14 import (
    CLAIM_CEILING,
    PAYLOAD_SCOPE,
...
MANIFEST = (
    ROOT / "configs/reference_match_main_integration_manifest_v14.json"
)
SCHEMA = (
    ROOT
    / "configs"
    / "schemas"
    / "reference_match_main_integration_manifest_v14.schema.json"
)
PRIOR = ROOT / "configs/reference_match_main_integration_manifest_v13.json"
...
def test_frozen_v14_schema_and_manifest_rebuild_byte_identically() -> None:
    frozen = _manifest()
...
def test_v14_binds_android_emulator_runtime_and_preserves_v13() -> None:
    manifest = _manifest()
    ...
    assert "configs/reference_match_main_integration_manifest_v13.json" in paths
    assert "configs/reference_match_main_integration_manifest_v14.json" not in paths
    runner = next(
        row
    ...
    assert runner["blob"] == "8dd004fc06f274fe73168935af6dddaeb89fdea4"
    instrumentation = next(
        row
        for row in manifest["payload_files"]
        if row["path"]
        == (
            "runtime/android_srgb_quantizer_testlab/test/src/com/"
            "neurofilm/srgbquantizer/QuantizerInstrumentation.java"
        )
    )
    assert (
        instrumentation["blob"]
        == "85044470b528dc240d57c906883850e3a8b02562"
    )
...
@pytest.mark.parametrize(
    "mutate",
    [
        lambda value: value.update(payload_scope="P1-P96"),
...
def test_v14_manifest_tamper_fails_closed(mutate) -> None:
...
def test_v14_schema_rejects_before_git_access(
    tmp_path: Path,
    mutate,
) -> None:
    manifest = deepcopy(_manifest())
...
def test_v14_schema_unavailable_and_wrong_base_fail_closed(
    tmp_path: Path,
) -> None:
    manifest = _manifest()
    with pytest.raises(
