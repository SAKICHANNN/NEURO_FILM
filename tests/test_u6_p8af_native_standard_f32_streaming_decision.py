...
def test_p8af_decision_matches_formal_report() -> None:
    decision = json.loads(
        (
            ROOT
            / "configs/"
            "u6_p8af_native_standard_f32_streaming_decision_v1.json"
        ).read_text()
    )
    ...
    assert report["pass"]
    assert report["output_identity_vs_full_frame_exact"]
    assert report["peak_process_tree_rss_bytes"] == decision["result"][
        "peak_process_tree_rss_bytes"
    ]
    assert report["median_rss_ratio_vs_full_frame_f32"] == decision[
        "result"
    ]["median_rss_ratio_vs_full_frame_f32"]
    ...
    assert decision["next_leaf"].startswith("U6.P8AG")
