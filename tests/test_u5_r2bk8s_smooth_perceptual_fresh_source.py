...
def test_bk8s_contract_is_frozen_before_pixel_access() -> None:
    ...
    assert sum(row["bytes_reported_mb"] for row in rows) == 187.98
    assert config["maximum_download_bytes"] == 200 * 1024 * 1024
...
def test_bk8s_sources_are_disjoint_from_prior_tracked_pools() -> None:
    config = json.loads(CONFIG.read_text(encoding="utf-8"))
...
def test_bk8s_selection_excludes_nonphotographic_metadata() -> None:
    config = json.loads(CONFIG.read_text(encoding="utf-8"))
