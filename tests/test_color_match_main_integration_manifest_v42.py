...
from scripts.build_reference_match_integration_manifest_v42 import (
    PAYLOAD_SCOPE,
    SCHEMA_ID,
...
MANIFEST = ROOT / "configs/reference_match_main_integration_manifest_v42.json"
SCHEMA = (
    ROOT
    / "configs/schemas/reference_match_main_integration_manifest_v42.schema.json"
)
PRIOR = ROOT / "configs/reference_match_main_integration_manifest_v41.json"
...
def test_v42_rebuilds_and_binds_p170_product_discovery() -> None:
    value = frozen()
    ...
    assert "reference_match_product_capabilities_payload" in blob(
        "src/color_match/product_capabilities.py"
    )
    assert "not-promoted" in blob(
...
def test_v42_tamper(mutation) -> None:
