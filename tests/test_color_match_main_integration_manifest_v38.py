...
from scripts.build_reference_match_integration_manifest_v38 import SCHEMA_ID,PAYLOAD_SCOPE,build_manifest,build_schema,encode_json,validate_manifest,validate_manifest_schema
from src.color_match.strict_json import strict_json_loads
ROOT=Path(__file__).resolve().parents[1];MANIFEST=ROOT/"configs/reference_match_main_integration_manifest_v38.json";SCHEMA=ROOT/"configs/schemas/reference_match_main_integration_manifest_v38.schema.json";PRIOR=ROOT/"configs/reference_match_main_integration_manifest_v37.json"
...
def test_v38_rebuilds_and_binds_rejected_candidate_lifetime_fix():
    ...
    assert "del candidate" in blob("src/color_match/safety.py")
    assert "test_rejected_candidate_pixels_are_released_before_identity_clone" in blob("tests/test_color_match_safety.py")
