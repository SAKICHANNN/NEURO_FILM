...
from scripts.build_reference_match_integration_manifest_v36 import SCHEMA_ID,PAYLOAD_SCOPE,build_manifest,build_schema,encode_json,validate_manifest,validate_manifest_schema
from src.color_match.strict_json import strict_json_loads
ROOT=Path(__file__).resolve().parents[1];MANIFEST=ROOT/"configs/reference_match_main_integration_manifest_v36.json";SCHEMA=ROOT/"configs/schemas/reference_match_main_integration_manifest_v36.schema.json";PRIOR=ROOT/"configs/reference_match_main_integration_manifest_v35.json"
...
def test_v36_rebuilds_and_binds_reference_lifetime_fix():
    ...
    assert "del reference_working" in blob("src/color_match/files.py")
    assert "test_match_releases_reference_pixels_before_source_render" in blob("tests/test_color_match_files.py")
