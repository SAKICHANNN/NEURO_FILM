...
def test_bh1s_contract_is_frozen_before_pixel_access() -> None:
    ...
    assert sum(row["bytes_reported_mb"] for row in rows) == 212.85
    assert config["maximum_download_bytes"] == 224 * 1024 * 1024
    assert config["next_leaf"]["fixed_arms"] == [
        "fixed_b0",
        "fixed_ao6_colour_only_t15_c35",
    ]
    assert config["next_leaf"]["complete_pairwise_rounds"] == 3
...
def test_bh1s_sources_are_disjoint_from_all_prior_rawpixls_pools() -> None:
    config = json.loads(CONFIG.read_text(encoding="utf-8"))
...
def test_bh1s_selection_excludes_obvious_nonphotographic_metadata() -> None:
    config = json.loads(CONFIG.read_text(encoding="utf-8"))
