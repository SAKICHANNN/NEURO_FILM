...
from src.eval.cross_layer_neutral_nps import evaluate, load_contract

ROOT = Path(__file__).resolve().parents[1]
CONTRACT = ROOT / "configs/u6_p4df_cross_layer_neutral_nps_v1.json"


def test_contract_parent() -> None:
    assert (
        load_contract(ROOT, CONTRACT)["experiment_id"]
        == "u6.p4df-cross-layer-neutral-nps-v1"
    )
...
def test_parent_drift(tmp_path: Path) -> None:
    ...
    path.write_text(
        CONTRACT.read_text().replace(
            "retain_typed_target_density_cross_layer_cloud_reference", "wrong"
        )
    )
