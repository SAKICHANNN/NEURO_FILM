...
DECISION = (
    ROOT / "configs/u6_p8as_native_ao6_display_v3_decision_v1.json"
)
...
def test_p8as_decision_binds_exact_pass_report() -> None:
    ...
    assert decision["result"]["base_v2_v3_byte_exact"] is True
    assert decision["result"]["display_v2_v3_byte_exact"] is True
    assert decision["production_default_changed"] is False
    assert decision["next_leaf"].startswith("U6.P8AT")
