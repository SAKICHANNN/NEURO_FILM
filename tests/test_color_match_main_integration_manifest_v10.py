...
MANIFEST = (
    ROOT / "configs/reference_match_main_integration_manifest_v10.json"
)
SCHEMA = (
    ROOT
    / "configs"
...
PRIOR = ROOT / "configs/reference_match_main_integration_manifest_v9.json"
...
def test_frozen_v10_schema_and_manifest_rebuild_byte_identically() -> None:
    frozen = _manifest()
...
def test_v10_binds_dynamic_icc_abi_and_preserves_v9() -> None:
    manifest = _manifest()
    ...
    assert "configs/reference_match_main_integration_manifest_v9.json" in paths
    assert "configs/reference_match_main_integration_manifest_v10.json" not in paths
    assert (
...
@pytest.mark.parametrize(
    "mutate",
    [
        lambda value: value.update(payload_scope="P1-P81"),
...
def test_v10_manifest_tamper_fails_closed(mutate) -> None:
...
def test_v10_schema_rejects_before_git_access(
    tmp_path: Path,
    mutate,
) -> None:
    manifest = deepcopy(_manifest())
...
def test_v10_schema_unavailable_and_wrong_base_fail_closed(
    tmp_path: Path,
) -> None:
    manifest = _manifest()
    with pytest.raises(
