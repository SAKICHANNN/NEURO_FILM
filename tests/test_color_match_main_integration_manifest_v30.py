...
from scripts.build_reference_match_integration_manifest_v30 import SCHEMA_ID,PAYLOAD_SCOPE,build_manifest,build_schema,encode_json,validate_manifest,validate_manifest_schema
from src.color_match.strict_json import strict_json_loads
ROOT=Path(__file__).resolve().parents[1];MANIFEST=ROOT/"configs/reference_match_main_integration_manifest_v30.json";SCHEMA=ROOT/"configs/schemas/reference_match_main_integration_manifest_v30.schema.json";PRIOR=ROOT/"configs/reference_match_main_integration_manifest_v29.json"
...
def test_v30_rebuilds_and_binds_rec2020_sdr_file_rail():
    ...
    files=blob("src/color_match/files.py")
    assert "save_rec2020_16_png" in files
    assert "linear_rec2020 file output requires 16-bit PNG" in files
    assert '{"linear_srgb", "linear_rec2020"}' in files
