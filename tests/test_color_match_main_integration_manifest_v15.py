...
from scripts.build_reference_match_integration_manifest_v15 import (
    CLAIM_CEILING,
    PAYLOAD_SCOPE,
...
MANIFEST = (
    ROOT / "configs/reference_match_main_integration_manifest_v15.json"
)
SCHEMA = (
    ROOT
    / "configs"
    / "schemas"
    / "reference_match_main_integration_manifest_v15.schema.json"
)
PRIOR = ROOT / "configs/reference_match_main_integration_manifest_v14.json"
...
def test_frozen_v15_schema_and_manifest_rebuild_byte_identically() -> None:
    frozen = _manifest()
...
def test_v15_binds_android_sdr_runtime_and_preserves_v14() -> None:
    manifest = _manifest()
    ...
    assert "configs/reference_match_main_integration_manifest_v14.json" in paths
    assert "configs/reference_match_main_integration_manifest_v15.json" not in paths
    runner = next(
        row
    ...
    assert runner["blob"] == "f51655ca8d8dafc33fd525f7d02f9fa54aeced02"
    ...
    assert (
        ...
        == "ba8aaac4625aa14d0d203e6b656d6ef7d4e1f890"
    )
...
@pytest.mark.parametrize(
    "mutate",
    [
        lambda value: value.update(payload_scope="P1-P100"),
...
def test_v15_manifest_tamper_fails_closed(mutate) -> None:
...
def test_v15_schema_rejects_before_git_access(
    tmp_path: Path,
    mutate,
) -> None:
    manifest = deepcopy(_manifest())
...
def test_v15_schema_unavailable_and_wrong_base_fail_closed(
    tmp_path: Path,
) -> None:
    manifest = _manifest()
    with pytest.raises(
