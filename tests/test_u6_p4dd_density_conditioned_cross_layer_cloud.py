...
from src.eval.density_conditioned_cross_layer_cloud import evaluate, load_contract

ROOT = Path(__file__).resolve().parents[1]
CONTRACT = ROOT / "configs/u6_p4dd_density_conditioned_cross_layer_cloud_v1.json"


def test_contract_parents() -> None:
    assert (
        load_contract(ROOT, CONTRACT)["experiment_id"]
        == "u6.p4dd-density-conditioned-cross-layer-cloud-v1"
    )
...
def test_parent_drift(tmp_path: Path) -> None:
    ...
    path.write_text(
        CONTRACT.read_text().replace(
            "retain_exact_bounded_cross_layer_cloud_row_stream", "wrong"
        )
    )
