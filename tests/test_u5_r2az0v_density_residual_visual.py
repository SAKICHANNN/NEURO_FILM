...
from src.eval.density_residual_visual import build_visual_evidence


ROOT = Path(__file__).resolve().parents[1]


def test_visual_builder_is_deterministic_and_hides_mapping(tmp_path: Path) -> None:
    config = json.loads(
        (
    ...
    assert [
        row["sha256"] for row in first["blind_sheets"]
    ] == [
