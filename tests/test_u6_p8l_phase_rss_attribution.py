...
from scripts.benchmark_u6_p8l_phase_rss import validate_contract


ROOT = Path(__file__).resolve().parents[1]
CONTRACT = ROOT / "configs/u6_p8l_phase_rss_attribution_v1.json"


def test_p8l_contract_requires_two_exact_12mp_attribution_runs() -> None:
    ...
    assert len(config["measurement"]["phase_order"]) == len(
        set(config["measurement"]["phase_order"])
    )
