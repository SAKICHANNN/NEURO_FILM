...
MANIFEST = (
    ROOT / "configs/reference_match_main_integration_manifest_v9.json"
)
SCHEMA = (
    ROOT
    / "configs"
...
PRIOR = ROOT / "configs/reference_match_main_integration_manifest_v8.json"
...
def test_frozen_v9_schema_and_manifest_rebuild_byte_identically() -> None:
    frozen = _manifest()
...
def test_v9_binds_native_icc_abi_and_preserves_v8() -> None:
    manifest = _manifest()
    ...
    assert "configs/reference_match_main_integration_manifest_v8.json" in paths
    assert "configs/reference_match_main_integration_manifest_v9.json" not in paths
    assert (
        "configs/schemas/reference_srgb_icc_profile_conformance_v1.schema.json"
        in {row["path"] for row in manifest["contract_schemas"]}
    )
    assert "srgb_icc_profile_v1" in manifest["required_public_exports"]
    assert "native/reference_srgb_icc_profile_v1.c" in paths
    assert "native/reference_srgb_icc_profile_v1.h" in paths


@pytest.mark.parametrize(
    "mutate",
    [
        lambda value: value.update(payload_scope="P1-P79"),
...
def test_v9_manifest_tamper_fails_closed(mutate) -> None:
...
def test_v9_schema_rejects_before_git_access(
    tmp_path: Path,
    mutate,
) -> None:
    manifest = deepcopy(_manifest())
...
def test_v9_schema_unavailable_and_wrong_base_fail_closed(
    tmp_path: Path,
) -> None:
    manifest = _manifest()
    with pytest.raises(
