...
MANIFEST = (
    ROOT / "configs/reference_match_main_integration_manifest_v13.json"
)
SCHEMA = (
    ROOT
    / "configs"
...
PRIOR = ROOT / "configs/reference_match_main_integration_manifest_v12.json"
...
def test_frozen_v13_schema_and_manifest_rebuild_byte_identically() -> None:
    frozen = _manifest()
...
def test_v13_binds_output_quantizer_and_preserves_v12() -> None:
    manifest = _manifest()
    ...
    assert "configs/reference_match_main_integration_manifest_v12.json" in paths
    assert "configs/reference_match_main_integration_manifest_v13.json" not in paths
    source = next(
        row
        for row in manifest["payload_files"]
        if row["path"] == "native/reference_srgb_oetf_quantize_v1.c"
    )
    assert (
        source["blob"]
        == "a1cb380a054351c97866e2507257f80c5cce4364"
    )
    streaming = next(
        row
        for row in manifest["payload_files"]
        if row["path"]
        == "scripts/audit_srgb_oetf_quantize_streaming_v1.py"
    )
    assert (
        streaming["blob"]
        == "f6ea8a31bd89e805ea2c6fe0a95c64194e241394"
    )
...
@pytest.mark.parametrize(
    "mutate",
    [
        lambda value: value.update(payload_scope="P1-P89"),
...
def test_v13_manifest_tamper_fails_closed(mutate) -> None:
...
def test_v13_schema_rejects_before_git_access(
    tmp_path: Path,
    mutate,
) -> None:
    manifest = deepcopy(_manifest())
...
def test_v13_schema_unavailable_and_wrong_base_fail_closed(
    tmp_path: Path,
) -> None:
    manifest = _manifest()
    with pytest.raises(
