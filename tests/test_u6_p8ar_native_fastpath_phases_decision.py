...
DECISION = (
    ROOT / "configs/u6_p8ar_native_fastpath_phases_decision_v1.json"
)
...
def test_p8ar_decision_selects_repeat_stable_display_phase() -> None:
    ...
    assert result["dominant_phase_repeat_exact"] is True
    assert result["dominant_phase"] == "ao6_native_display_v2_seconds"
    ...
    assert decision["next_leaf"].startswith("U6.P8AS")
