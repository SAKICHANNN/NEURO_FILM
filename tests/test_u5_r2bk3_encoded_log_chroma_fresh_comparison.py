...
from src.eval.encoded_log_chroma_fresh_comparison import (
    ARMS,
    render_fixed_arms,
    validate_contract,
)
...
def test_bk3_three_arm_contract_is_frozen_and_hash_bound() -> None:
    config = json.loads(CONFIG.read_text(encoding="utf-8"))
    assert config["status"] == "contract_frozen_implementation_ready"
    assert [row["arm_id"] for row in config["fixed_arms"]] == list(ARMS)
...
def test_bk3_contract_forbids_fit_retune_routing_and_clipping() -> None:
    config = json.loads(CONFIG.read_text(encoding="utf-8"))
...
def test_bk3_salience_and_safety_gates_are_fixed() -> None:
    gate = json.loads(CONFIG.read_text(encoding="utf-8"))["automatic_gate"]
    assert gate["expected_outputs"] == 33
    assert gate["minimum_bk2_population_median_style_delta_e76"] == 5.0
...
def test_bk3_contract_validates_and_fixed_arms_render_safely() -> None:
    config = json.loads(CONFIG.read_text(encoding="utf-8"))
...
def test_bk3_contract_mutation_fails_closed() -> None:
    config = json.loads(CONFIG.read_text(encoding="utf-8"))
