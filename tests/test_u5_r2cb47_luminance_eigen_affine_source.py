...
DECISION = ROOT / "configs/u5_r2cb47_luminance_eigen_affine_source_decision_v1.json"


def test_cb47_source_role_binds_exact_disjoint_population() -> None:
    ...
    assert len(rows) == payload["source_count_exact"] == 9
    assert len({row["make"] for row in rows}) == payload["camera_make_count_exact"]
    assert payload["exact_decoded_sha_overlap_with_cb46"] == 0
