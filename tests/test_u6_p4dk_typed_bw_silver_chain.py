...
from src.eval.typed_bw_silver_chain import evaluate, load_contract

ROOT = Path(__file__).resolve().parents[1]
CONTRACT = ROOT / "configs/u6_p4dk_typed_bw_silver_chain_v1.json"


def test_contract_parents() -> None:
    assert (
        load_contract(ROOT, CONTRACT)["experiment_id"]
        == "u6.p4dk-typed-bw-silver-chain-v1"
    )
...
def test_parent_drift(tmp_path: Path) -> None:
    ...
    path.write_text(
        CONTRACT.read_text().replace(
            "retain_typed_colour_sensitometry_cloud_chain", "wrong"
        )
    )
