...
def test_p8z_decision_matches_formal_report() -> None:
    decision = json.loads(
        (
            ROOT
            / "configs/u6_p8z_native_domains_conformance_decision_v1.json"
        ).read_text()
    )
    ...
    for replay in report["replays"]:
        ...
        assert replay["sensitometry_inplace_exact"]
        assert replay["interpretation_inplace_exact"]
        assert replay["invalid_input_output_unchanged"]
        assert (
            replay["sensitometry_maximum_absolute_error"]
            <= replay["tolerance"]
        )
        assert (
            replay["interpretation_maximum_absolute_error"]
            <= replay["tolerance"]
        )
    assert decision["production_default_changed"] is False
    assert decision["next_leaf"].startswith("U6.P8AA")
