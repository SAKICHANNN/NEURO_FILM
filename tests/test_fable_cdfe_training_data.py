...
def test_partial_cache_and_changed_identity_are_rejected_before_allocation(tmp_path):
    ...
    with pytest.raises(FileNotFoundError):
        load_fitting_cache(tmp_path, plan, locked_fitting_ids=identities, receipt_sha256={})
    with pytest.raises(ValueError, match='locked'):
        load_fitting_cache(tmp_path, plan, locked_fitting_ids=identities[:-1]+['held'], receipt_sha256={})
