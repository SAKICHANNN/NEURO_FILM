...
MANIFEST = ROOT / "configs/reference_match_main_integration_manifest_v43.json"
SCHEMA = (
    ROOT
    / "configs/schemas/reference_match_main_integration_manifest_v43.schema.json"
)
PRIOR = ROOT / "configs/reference_match_main_integration_manifest_v42.json"
...
def test_v43_rebuilds_and_binds_p172_scene_linear_raw_rejection() -> None:
    value = frozen()
    ...
    p172 = blob("tests/test_color_match_files.py")
    assert "unsupported-decoded-rail" in p172
    assert "test_file_adapter_rejects_late_decoded_scene_linear_raw_atomically" in p172
...
def test_v43_tamper(mutation) -> None:
