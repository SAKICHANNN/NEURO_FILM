...
from src.eval.sensitometry_cloud_capacity_v2 import evaluate, load_contract

ROOT = Path(__file__).resolve().parents[1]
CONTRACT = ROOT / "configs/u6_p4di_sensitometry_cloud_capacity_v2.json"


def test_contract_parents() -> None:
    assert (
        load_contract(ROOT, CONTRACT)["experiment_id"]
        == "u6.p4di-sensitometry-cloud-capacity-v2"
    )
...
def test_parent_drift(tmp_path: Path) -> None:
    ...
    path.write_text(CONTRACT.read_text().replace("correlation_identity", "wrong", 1))
