...
from scripts.benchmark_u6_p6zf_scanner_glare_channel_serial_fft import (
    SCHEMA,
    numerical_probe,
    worker,
)
...
CONTRACT = ROOT / "configs" / "u6_p6zf_scanner_glare_channel_serial_fft_v1.json"


def test_contract_freezes_unchanged_12mp_gates() -> None:
    ...
    assert (
        contract["automatic_gates"]["maximum_candidate_peak_process_tree_rss_bytes"]
        == 2**30
    )


@pytest.mark.parametrize("algorithm", ["fft-2d-reference", "channel-serial-block-fft"])
...
def test_probe_is_partition_stable_and_reference_exact() -> None:
