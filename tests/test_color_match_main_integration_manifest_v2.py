...
from scripts.build_reference_match_integration_manifest_v2 import (
    CLAIM_CEILING,
    PAYLOAD_SCOPE,
...
PRIOR_MANIFEST = (
    ROOT / "configs/reference_match_main_integration_manifest_v1.json"
)
...
def test_frozen_v2_manifest_rebuilds_byte_identically() -> None:
    frozen = _manifest()
...
def test_v2_manifest_expands_but_does_not_mutate_p58() -> None:
    manifest = _manifest()
    ...
    assert (
        ...
        not in {
            row["path"] for row in manifest["payload_files"]
        }
    )
...
@pytest.mark.parametrize(
    "mutate",
    [
        lambda value: value.update(payload_scope="P1-P64"),
        ...
        lambda value: value["contract_schemas"][0].update(
            sha256="0" * 64
        ),
...
def test_v2_manifest_tamper_fails_closed(mutate) -> None:
...
def test_v2_schema_rejects_before_git_access(
    tmp_path: Path,
    mutate,
) -> None:
    manifest = deepcopy(_manifest())
...
def test_v2_schema_unavailable_fails_before_git_access(
    tmp_path: Path,
) -> None:
    with pytest.raises(
        IntegrationManifestError,
...
def test_v2_wrong_base_or_pre_shared_payload_fails_closed() -> None:
    manifest = _manifest()
