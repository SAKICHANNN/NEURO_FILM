...
from scripts.build_reference_match_integration_manifest_v37 import SCHEMA_ID,PAYLOAD_SCOPE,build_manifest,build_schema,encode_json,validate_manifest,validate_manifest_schema
from src.color_match.strict_json import strict_json_loads
ROOT=Path(__file__).resolve().parents[1];MANIFEST=ROOT/"configs/reference_match_main_integration_manifest_v37.json";SCHEMA=ROOT/"configs/schemas/reference_match_main_integration_manifest_v37.schema.json";PRIOR=ROOT/"configs/reference_match_main_integration_manifest_v36.json"
...
def test_v37_rebuilds_and_binds_source_lifetime_fix():
    ...
    assert "del source" in blob("src/color_match/files.py")
    assert "test_render_releases_source_pixels_before_output_encode" in blob("tests/test_color_match_files.py")
