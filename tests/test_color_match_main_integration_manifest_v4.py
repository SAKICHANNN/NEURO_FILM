...
MANIFEST = (
    ROOT / "configs/reference_match_main_integration_manifest_v4.json"
)
SCHEMA = (
    ROOT
    / "configs"
...
PRIOR = ROOT / "configs/reference_match_main_integration_manifest_v3.json"
...
def test_frozen_v4_schema_and_manifest_rebuild_byte_identically() -> None:
    frozen = _manifest()
...
def test_v4_adds_p67_and_preserves_p66() -> None:
    manifest = _manifest()
    ...
    assert "configs/reference_match_main_integration_manifest_v3.json" in paths
    assert "configs/reference_match_main_integration_manifest_v4.json" not in paths


@pytest.mark.parametrize(
    "mutate",
    [
        lambda value: value.update(payload_scope="P1-P68"),
        ...
        lambda value: value["contract_schemas"][5].update(
            sha256="0" * 64
        ),
...
def test_v4_manifest_tamper_fails_closed(mutate) -> None:
...
def test_v4_schema_rejects_before_git_access(
    tmp_path: Path,
    mutate,
) -> None:
    manifest = deepcopy(_manifest())
...
def test_v4_schema_unavailable_and_wrong_base_fail_closed(
    tmp_path: Path,
) -> None:
    manifest = _manifest()
    with pytest.raises(
