...
from scripts.build_reference_match_integration_manifest_v28 import SCHEMA_ID,PAYLOAD_SCOPE,build_manifest,build_schema,encode_json,validate_manifest,validate_manifest_schema
from src.color_match.strict_json import strict_json_loads
ROOT=Path(__file__).resolve().parents[1]
MANIFEST=ROOT/"configs/reference_match_main_integration_manifest_v28.json"
SCHEMA=ROOT/"configs/schemas/reference_match_main_integration_manifest_v28.schema.json"
PRIOR=ROOT/"configs/reference_match_main_integration_manifest_v27.json"
...
def test_v28_rebuilds_and_binds_input_and_publication_identities():
    ...
    files=blob("src/color_match/files.py");reporting=blob("src/color_match/reporting.py")
    assert "reference_file_sha256" in files and "source_file_sha256" in files
    assert "expected_stage_sha256=expected_stage_sha256" in files
    assert "sha256_file(row.source_path)" not in reporting
