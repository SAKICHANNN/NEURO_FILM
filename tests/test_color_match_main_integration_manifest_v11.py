...
MANIFEST = (
    ROOT / "configs/reference_match_main_integration_manifest_v11.json"
)
SCHEMA = (
    ROOT
    / "configs"
...
PRIOR = ROOT / "configs/reference_match_main_integration_manifest_v10.json"
...
def test_frozen_v11_schema_and_manifest_rebuild_byte_identically() -> None:
    frozen = _manifest()
...
def test_v11_binds_exact_eotf_abi_and_preserves_v10() -> None:
    manifest = _manifest()
    ...
    assert "configs/reference_match_main_integration_manifest_v10.json" in paths
    assert "configs/reference_match_main_integration_manifest_v11.json" not in paths
    assert (
    ...
    assert "native/reference_srgb_eotf_f32_v1.c" in paths
    assert "native/reference_srgb_eotf_f32_v1.h" in paths


@pytest.mark.parametrize(
    "mutate",
    [
        lambda value: value.update(payload_scope="P1-P83"),
...
def test_v11_manifest_tamper_fails_closed(mutate) -> None:
...
def test_v11_schema_rejects_before_git_access(
    tmp_path: Path,
    mutate,
) -> None:
    manifest = deepcopy(_manifest())
...
def test_v11_schema_unavailable_and_wrong_base_fail_closed(
    tmp_path: Path,
) -> None:
    manifest = _manifest()
    with pytest.raises(
