...
from src.eval.target_density_cross_layer_cloud import evaluate, load_contract

ROOT = Path(__file__).resolve().parents[1]
CONTRACT = ROOT / "configs/u6_p4de_target_density_cross_layer_cloud_v1.json"


def test_contract_parent() -> None:
    assert (
        load_contract(ROOT, CONTRACT)["experiment_id"]
        == "u6.p4de-target-density-cross-layer-cloud-v1"
    )
...
def test_parent_drift(tmp_path: Path) -> None:
    ...
    path.write_text(
        CONTRACT.read_text().replace(
            "retain_density_conditioned_cross_layer_gaussian_cloud_reference", "wrong"
        )
    )
