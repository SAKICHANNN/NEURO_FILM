...
from src.eval.native_rec2020_lab_source_compress_v2 import (
    NativeRec2020LabCompressV2Error,
...
def native_library(tmp_path_factory: pytest.TempPathFactory):
    build = build_native_rec2020_lab_compress_v2(
        root=ROOT, output_dir=tmp_path_factory.mktemp("u1_4c26_native")
    )
    return load_native_rec2020_lab_compress_v2(Path(build["dll_path"]))
...
def test_native_matches_python_source_compression(native_library) -> None:
    ...
    actual_lab, actual_rgb, _ = apply_native_rec2020_lab_compress_v2(
        native_library, source_lab, target_lab, thread_count=2
    )
    assert np.array_equal(actual_lab, expected_lab)
    assert np.max(np.abs(actual_rgb - expected_rgb)) <= 2e-15
    assert np.isfinite(actual_rgb).all()
    assert np.all(actual_rgb >= -2e-6)
    assert np.all(actual_rgb <= 1.0 + 2e-6)


def test_native_matches_python_on_dense_random_boundary_fixture(native_library) -> None:
    rng = np.random.default_rng(20260820)
    source_rgb = rng.random((128, 192, 3), dtype=np.float32)
    source_lab = np.ascontiguousarray(
        linear_rgb_to_lab(source_rgb, working_space="linear_rec2020")
    )
    ...
    expected_rgb = lab_to_linear_rgb(expected_lab, working_space="linear_rec2020")
    actual_lab, actual_rgb, _ = apply_native_rec2020_lab_compress_v2(
        native_library, source_lab, target_lab, thread_count=8
    )
    assert np.array_equal(actual_lab, expected_lab)
    assert np.max(np.abs(actual_rgb - expected_rgb)) <= 2e-15
...
def test_thread_partition_is_exact(native_library) -> None:
    source_lab, target_lab = _fixture()
    serial = apply_native_rec2020_lab_compress_v2(
        native_library, source_lab, target_lab, thread_count=1
    )
    parallel = apply_native_rec2020_lab_compress_v2(
...
def test_nonfinite_failure_is_atomic(native_library) -> None:
    ...
    with pytest.raises(NativeRec2020LabCompressV2Error):
        apply_native_rec2020_lab_compress_v2(
            native_library,
            source_lab,
            target_lab,
