...
def test_stock_pilot_integrity_decodes_and_rejects_sealed_roll(tmp_path: Path) -> None:
    ...
    rolls = [{"roll_id": "rollA", "contains_official_test_frame": False}]
    ...
    assert report["eligibility_by_stock"]["kodak_ga_100_5095"]["density_identifiability_eligible"] is False

    sealed_rolls = [{"roll_id": "rollA", "contains_official_test_frame": True}]
    try:
        audit_stock_pilot_integrity(
            download_root=root,
            acquisition=acquisition,
...
def test_stock_pilot_integrity_detects_exact_duplicate(tmp_path: Path) -> None:
    ...
    report = audit_stock_pilot_integrity(
        ...
        acquisition=acquisition,
        download_report=download_report,
        metadata_report={"stock_summaries": {}},
        frame_rows=frames,
        roll_rows=[{"roll_id": "rollA", "contains_official_test_frame": False}],
        software_commit="test",
    )
