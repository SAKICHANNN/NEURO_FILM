...
MANIFEST = (
    ROOT / "configs/reference_match_main_integration_manifest_v8.json"
)
SCHEMA = (
    ROOT
    / "configs"
...
PRIOR = ROOT / "configs/reference_match_main_integration_manifest_v7.json"
...
def test_frozen_v8_schema_and_manifest_rebuild_byte_identically() -> None:
    frozen = _manifest()
...
def test_v8_binds_icc_conformance_and_preserves_v7() -> None:
    manifest = _manifest()
    ...
    assert "configs/reference_match_main_integration_manifest_v7.json" in paths
    assert "configs/reference_match_main_integration_manifest_v8.json" not in paths
    assert (
...
@pytest.mark.parametrize(
    "mutate",
    [
        lambda value: value.update(payload_scope="P1-P77"),
...
def test_v8_manifest_tamper_fails_closed(mutate) -> None:
...
def test_v8_schema_rejects_before_git_access(
    tmp_path: Path,
    mutate,
) -> None:
    manifest = deepcopy(_manifest())
...
def test_v8_schema_unavailable_and_wrong_base_fail_closed(
    tmp_path: Path,
) -> None:
    manifest = _manifest()
    with pytest.raises(
