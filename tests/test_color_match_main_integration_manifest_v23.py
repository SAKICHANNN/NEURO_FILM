...
from scripts.build_reference_match_integration_manifest_v23 import (
    CLAIM_CEILING,
    PAYLOAD_SCOPE,
    SCHEMA_ID,
    build_manifest,
...
MANIFEST = (
    ROOT / "configs/reference_match_main_integration_manifest_v23.json"
)
SCHEMA = (
    ROOT
    / "configs/schemas/"
    "reference_match_main_integration_manifest_v23.schema.json"
)
PRIOR = ROOT / "configs/reference_match_main_integration_manifest_v22.json"
...
def test_frozen_v23_rebuilds_and_supersedes_v22() -> None:
    frozen = _manifest()
...
def test_v23_binds_bounded_replay_and_runtime_no_follow_fix() -> None:
    validate_manifest_schema(_manifest())
    runtime_source = _payload_bytes(
        "src/color_match/shared_runtime_staging_transaction.py"
    ).decode("utf-8")
    staging_source = _payload_bytes(
        "src/color_match/staging_io.py"
    ).decode("utf-8")
    replay_source = _payload_bytes(
        "src/color_match/replay.py"
    ).decode("utf-8")
    assert "paths = _bounded_output_paths(values, count=count)" in (
        runtime_source
    )
    assert "paths = staging_output_paths(values, count=count)" not in (
        runtime_source
    )
    assert "def _bounded_output_paths(" in staging_source
    assert "batch = _working_image_batch(sources)" in replay_source
    payload_paths = {
        row["path"] for row in _manifest()["payload_files"]
    }
    assert "tests/test_color_match_staging_io_limits.py" in payload_paths
    assert "tests/test_color_match_replay.py" in payload_paths
    assert (
...
@pytest.mark.parametrize(
    "mutate",
    [
        lambda value: value.update(payload_scope="P1-P127"),
...
def test_v23_tamper_fails_closed(mutate) -> None:
...
def test_v23_schema_rejects_before_git_access(tmp_path: Path) -> None:
    manifest = deepcopy(_manifest())
