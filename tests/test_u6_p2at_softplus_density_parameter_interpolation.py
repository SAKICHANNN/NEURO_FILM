...
CONTRACT = ROOT / "configs/u6_p2at_softplus_density_parameter_interpolation_v1.json"


def test_p2at_intermediate_density_confirmation_replays() -> None:
    ...
    assert first["gate_results"]["maximum_confirmation_relative_mean_error"] is False
    assert first["measurements"]["parameter_refit_count_zero"] is True
    assert first["measurements"]["minimum_developed_density"] > 0.0


def test_p2at_parameter_profile_exact_nodes_and_no_extrapolation() -> None:
    contract = load_contract(CONTRACT)
    ...
    assert np.array_equal(actual_a, np.asarray(profile.a))
