...
def test_bk11s_contract_is_frozen_before_pixel_access() -> None:
    ...
    assert sum(row["bytes_reported_mb"] for row in rows) == 214.76
    assert config["maximum_download_bytes"] == 230 * 1024 * 1024
...
def test_bk11s_sources_are_disjoint_from_all_prior_tracked_pools() -> None:
    config = json.loads(CONFIG.read_text(encoding="utf-8"))
    ...
    current_makes = {row["make"].casefold() for row in config["candidates"]}
...
def test_bk11s_selection_excludes_nonphotographic_metadata() -> None:
    config = json.loads(CONFIG.read_text(encoding="utf-8"))
