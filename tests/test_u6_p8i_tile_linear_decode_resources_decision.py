...
DECISION = (
    ROOT / "configs/u6_p8i_tile_linear_decode_resources_decision_v1.json"
)
REPORT = (
    ROOT / "outputs/u6_p8i_tile_linear_decode_resources_v1/run_a/report.json"
)


def test_p8i_decision_binds_exact_negative_evidence() -> None:
    ...
    assert decision["same_output_identity_as_p8d_through_p8h"]


def test_p8i_closes_candidate_without_weakening_gates() -> None:
    decision = json.loads(DECISION.read_text(encoding="utf-8"))
    ...
    assert decision["next_leaf"].startswith("U6.P8J")
