...
def test_contract_and_exact_source_audit(tmp_path: Path) -> None:
    ...
    assert report["decision"] == "retain_non_renderable_intermediate_material_role_profile"
    assert len(report["material_stock_pairs"]) == 3
    assert all(
        domains == ["mtf", "granularity"]
        for domains in report["material_domains_by_stock_pair"].values()
    )
...
def test_source_hash_drift_fails_before_pdf_decode(tmp_path: Path) -> None:
    ...
    config["source"]["path"] = drift.relative_to(ROOT).as_posix() if drift.is_relative_to(ROOT) else "outputs/source_recon/u5_r2by0_kodak_2242/missing.pdf"
...
def test_conservative_material_gate_is_not_nominal_only(tmp_path: Path) -> None:
    ...
    for domain in ("mtf", "granularity"):
        for row in report["pairwise"][domain].values():
            assert row["material_channel_count"] >= 2
            for channel in row["channels"].values():
                assert channel["conservative_lower_bound"] <= channel["mean_centered_log_rmse"]
