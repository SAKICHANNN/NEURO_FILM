...
def native_library(tmp_path_factory: pytest.TempPathFactory):
    build = build_native_safe_lab_pointwise_v2(
        root=ROOT, output_dir=tmp_path_factory.mktemp("u1_4c27_native")
    )
    return load_native_safe_lab_pointwise_v2(Path(build["dll_path"]))
...
def test_native_matches_python_pointwise_chain(native_library) -> None:
    source = _fixture()
    context = safe_lab_context_from_lab(source)
    actual = apply_native_safe_lab_pointwise_v2(
        native_library,
        source,
    ...
    assert np.array_equal(actual, _python_pointwise(source))


def test_native_matches_dense_random_python_chain(native_library) -> None:
    rng = np.random.default_rng(20260820)
    source = np.ascontiguousarray(
        rng.normal((52.0, 3.0, -2.0), (18.0, 35.0, 35.0), (128, 192, 3)),
        dtype=np.float32,
    )
    context = safe_lab_context_from_lab(source)
    ...
    expected[..., 0] = source[..., 0] + 0.02 * 0.35 * (
        transferred[..., 0] - source[..., 0]
    )
    expected[..., 1:] = source[..., 1:] + 0.35 * (
        transferred[..., 1:] - source[..., 1:]
    )
    expected = apply_chroma_curve(source, expected, 0.45)
    expected = apply_color_guardrails(
        source, expected, 0.55, 0.35, 2.1, 16.0, None
    )
    actual = apply_native_safe_lab_pointwise_v2(
        native_library,
        source,
...
def test_thread_partition_is_exact(native_library) -> None:
    ...
    serial = apply_native_safe_lab_pointwise_v2(native_library, source, thread_count=1, **kwargs)
    parallel = apply_native_safe_lab_pointwise_v2(native_library, source, thread_count=4, **kwargs)
...
def test_nonfinite_failure_is_atomic(native_library) -> None:
    ...
    with pytest.raises(NativeSafeLabPointwiseV2Error):
        apply_native_safe_lab_pointwise_v2(
            native_library,
            source,
            source_context=context,
