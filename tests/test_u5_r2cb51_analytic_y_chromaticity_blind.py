...
OBSERVATIONS = (
    ROOT / "configs/u5_r2cb51_analytic_y_chromaticity_blind_observations_v1.json"
)


def test_cb51_blind_choices_are_complete_and_mapping_sealed() -> None:
    ...
    assert len(payload["source_order"]) == 17
    assert [row["round"] for row in payload["rounds"]] == [1, 2, 3]
    assert all(len(row["choices"]) == 17 for row in payload["rounds"])
    assert all(
