...
from scripts.build_reference_match_integration_manifest_v47 import (
    PAYLOAD_SCOPE,
    SCHEMA_ID,
    V47_REQUIRED_CONTRACT_SCHEMA_PATHS,
    V47_REQUIRED_PUBLIC_EXPORTS,
    build_manifest,
    build_schema,
    encode_json,
    validate_manifest,
...
MANIFEST = ROOT / "configs/reference_match_main_integration_manifest_v47.json"
SCHEMA = (
    ROOT
    / "configs/schemas/reference_match_main_integration_manifest_v47.schema.json"
)
PRIOR = ROOT / "configs/reference_match_main_integration_manifest_v46.json"
...
def test_v47_rebuilds_file_identity_hardened_payload() -> None:
    value = frozen()
    ...
    assert "configs/reference_match_main_integration_manifest_v46.json" in paths
    implementation = blob("src/color_match/files.py")
    assert "_OwnedDirectory" in implementation
    assert "_path_entry_has_identity" in implementation
    assert "_cleanup_owned_files" in implementation
    handoff = blob(
        "docs/drpt/REFERENCE_COLOR_MATCH_MAIN_INTEGRATION_HANDOFF.md"
    )
    assert "P176 extends the ownership boundary to regular files" in handoff
...
def test_v47_tamper(mutation) -> None:
