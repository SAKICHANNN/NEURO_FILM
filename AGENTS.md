...
| Latest algorithm evidence | 989 local tests pass. U5.R2Z1 now closes the FilmSet ClassNeg case-applicability route with two byte-identical reports. Once each held-out query's own operator is excluded, the off-diagonal Oracle improves shared O0 by only 7.32%, wins 62.5% and has non-positive bootstrap lower bounds, so the bank itself fails before selector capacity. Rank-2/rank-4 hard asymmetric policies are 28.54%/34.70% worse than shared and close `-389.94%/-474.01%` of the Oracle gap. All 24 operators remain structurally valid, fresh targets remain unread and no visual stage opens. This is paired software-recipe negative evidence only: no neural/router rescue, W2F1, film/stock claim, current-pixel fitting/training, LSM or production integration opens. |
