...
| Latest algorithm evidence | 1123 full-suite tests plus 12 focused AN1/AN0/J0 tests pass. AN0 exactly recovers three nonneutral two-matrix positive-film witnesses on complement-held patch/illuminant/exposure groups. AN1 then adds independent source/target patch-mean noise and 3% corrupted correspondences. Two exact reports show soft-L1 clean-confirmation RMSE `.000328`--`.000437`, maximum error below `.00265`, and `96.56%`--`97.33%` gains over robust one-matrix controls. The fixed soft-L1 promotion nevertheless closes because one preregistered outlier gain is `61.44% < 65%`; thresholds and scale stay frozen. A separately versioned two-stage sparse-outlier rejection challenger is allowed. This remains synthetic paired recovery, not a fit to Velvia or real film; production default, stock/calibration claims, current-pixel fitting/training and LSM remain unchanged. |
