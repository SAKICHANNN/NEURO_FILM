...
| Latest algorithm evidence | 1114 local tests pass. U5.R2AL1 evaluates a clean-room 96-raw/80-effective-parameter analytic chroma-sector curve bank on two frozen noncommuting staged colour reactions. Two formal processes are byte-identical. Candidate RMSE `.006254/.006222` improves 74.41%/74.89% over global curves and narrowly beats stationary K3; cube, Jacobian, neutral, continuity, replay, ramp and strength gates pass. Both rows exceed the frozen `.006` absolute ceiling and neither inverse converges, so the fixed sector bank closes without capacity/inverse/target/gate or photograph rescue. This is synthetic negative mechanism evidence, not film-operator evidence. The next leaf audits a globally bounded invertible coordinate-system plus analytic monotone-curve representation. The production default, stock/calibration claims, current-pixel fitting/training and LSM remain unchanged. |
...
| Ready/implementation | U5.R2AM1 | Execute corrected v3 `2ea192df...746c` (v1/v2 immutable): 21-raw/18-effective SO(3)-coordinate curves vs RGB curves, positive matrix+curves and K3 on two external-family synthetic targets; zero photographs/data |
