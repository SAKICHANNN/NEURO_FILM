...
| Latest algorithm evidence | 1167 local CPU tests pass. U5.R2AO7 evaluates the fixed AO6 `b0_plus_film_t15_c35` on a genuinely fresh 16-image/nine-make CC0 RAW population. Two 32-output runs are exact. All automatic gates pass: style/non-basic `11.5191/10.8875`, median residual from B0 `3.0668` Delta E76, 16/16 images and 9/9 makes above 1 Delta E76, and zero new clipping. Independent preference nevertheless closes: the candidate wins only 1/3 blind rounds, while two byte-identical layouts receive opposite rankings, showing that the residual is not robustly preference-salient. Retain AO6 as development evidence only; production, stock, calibration and independent-preference promotion remain closed. |
