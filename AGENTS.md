...
| Latest algorithm evidence | 967 local tests pass. U5.R2W2F0 closes FilmSet paired/global recipe explainability with byte-identical CUDA reports over 40 identities / 160 aligned payloads. Cinema is `basic_only`: shared O0 improves basic by only 2.44%. ClassNeg and Velvia are `adaptive_or_spatial_recipe`: per-pair-grid dispersion is `.16291/.20832` against `.08`; ClassNeg spatial residual is `.26069` against `.25`, while Velvia shared regret is `.16511` against `.15`. All 51 shared/per-pair O0 fits pass range, Jacobian, norm, inverse, coefficient and replay gates, so no domain is one coherent global recipe operator and the failure is not numerical collapse. U5.R2X0 is implemented/frozen to test whether the 2025 automatic palette-transfer path collapses exactly to affine and lacks orientation/range guarantees. W2F1, capacity/local rescue, film/stock claims, current-pixel fitting/training and LSM remain closed. |
