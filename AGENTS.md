...
| Latest algorithm evidence | 953 local tests pass. U5.R2S4D closes the frozen diversified conditional-distribution candidate with repeat-exact CUDA evidence. Correct generated condition correspondence reaches 71.36% held-out distribution improvement and passes bounded-flow structure/A-B stability, while shuffled reaches 24.79%; nevertheless hidden-operator median/p90 RMSE is `.10751/.11900`, and oracle improvement is only 8.45% over pooled and 24.98% over shuffled, missing the frozen `.07/.10` and both 25% gates. This is informative conditional matching without operator identification, not numerical collapse; no S4 capacity rescue, confirmation or visual candidate opens. U5.R2U1D is ready to test constructed hierarchical colour pseudo-pairs using only the bounded O0 flow. No film, stock or real unpaired-operator evidence exists; images, current-pixel fitting/training and LSM remain closed. |
