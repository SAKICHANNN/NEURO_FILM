...
| Latest algorithm evidence | U5.R2BK23 applies the frozen external content-cell gate to disjoint research-only FiveK Expert C rasters. Two reports are exact (`709372fa...b561`). Pooled unpaired appearance improves 92.89% and canonicalizer disagreement is only `.01669`, but only 64.06% of observed photograph cells pass and the worst regresses by 669.18%; observed and permuted views both return identity while all structure gates pass. This is a formal K=1/identity outcome showing that pooled appearance and canonicalizer agreement do not establish a shared cross-content operator. FiveK remains digital-retouch control evidence, not film/stock truth. AO6 t15/c35 remains the incumbent research Look Approximation; no confirmation rescue, router, product, calibration or LSM opens. |
