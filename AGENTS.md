...
| Latest algorithm evidence | 896 local tests pass. U5.R2O0 is the first frozen K/O-series representation to pass nonlinear fidelity, strict cube range, positive Jacobian, norm, inverse and repeat gates together: a 192-parameter stationary flow reaches `.01091/.00544` confirmation RMSE, improves `84.45%/80.27%` over affine, has min determinants `.117/.275`, max norms `3.993/2.781`, exact endpoints and byte-identical reports. U5.R2R1 shows why that representation matters: the fixed official D-LUT reference-palette trajectory has range/orientation survivors `0/40` despite norm `40/40`; step 40 is strong and non-affine (`.2080` identity RMSE, `.05240` affine residual) but has 48.02% sampled trilinear and 48.85% tetrahedral nonpositive determinants. It closes before images. Retain only a new hypothesis combining palette-score forcing with the O0 cube-preserving flow. U5.R2P0 remains future same-scene prior only; U5.R2Q0 closes the official SA-LUT checkpoint because training uses a direct Style2Log generator, GAN and incomplete lineage. U6.2B remains closed on severe salt speckles. The retained B0 global colour challenger remains U5.R2E1 density-cyan s0.50 (style 13.49, non-basic 7.39, zero new clipping, severe-clean). Film fitting, training and LSM remain closed. |
