...
| Latest algorithm evidence | 1108 local tests pass. U5.R2AK1 tests a clean-room 243-parameter quadratic-time cube flow against stationary 192/375-parameter controls on two frozen noncommuting staged colour reactions. Two formal processes are byte-identical. The candidate passes absolute fidelity and every cube, Jacobian, inverse, range, replay and partition gate, but is 73.94%/87.72% worse than stationary K4 and 2.56x/2.86x stationary K5. The fixed temporal family therefore closes without grid/stage/control/step/target or photograph rescue. This is synthetic negative capacity evidence, not film-operator evidence. The next algorithm leaf audits a clean-room analytic colour-selective monotone-curve representation while excluding NamedCurves canonicalization, attention, code, weights and colour-naming assets. The production default, stock/calibration claims, current-pixel fitting/training and LSM remain unchanged. |
