...
| Latest algorithm evidence | 891 local tests pass. U5.R2O0 is the first frozen K/O-series representation to pass nonlinear fidelity, strict cube range, positive Jacobian, norm, inverse and repeat gates together: a 192-parameter stationary flow reaches `.01091/.00544` confirmation RMSE, improves `84.45%/80.27%` over affine, has min determinants `.117/.275`, max norms `3.993/2.781`, exact endpoints and byte-identical reports. It validates a bounded explicit parameter family, not a new look; no redundant real-image frontier, fitting, stock or production claim opens. U5.R2P0 finds ChameleonTuner useful only for future same-scene misaligned pairs: it requires LSC/LoFTR correspondences between paired captures, cannot supervise unrelated similar photos, and its official repo currently has no code or licence. U5.R2N1 remains closed because its encoder follows geometry/content and its raw map escapes range. U6.2B remains closed on severe salt-speckle morphology. The retained B0 global colour challenger remains U5.R2E1 density-cyan s0.50 (style 13.49, non-basic 7.39, zero new clipping, severe-clean). Film fitting, training and LSM remain closed. |
