...
| Latest algorithm evidence | 1036 local tests pass. U5.R2AH1D closes the learned group-invariant reference-only predictor after two byte-identical 5,000-step CUDA runs. O0 range/Jacobian/inverse/replay, identity and same-look replication pass, but p90 operator RMSE is `.09723`; gains over identity/global are only `7.25%/4.89%`; content BA is `27.10%`; fixed-content look accuracy is `12.5%`; and 53/55/56 strength Spearman is `-1`. This is repeatable averaging/content-shortcut/strength failure even with exact crossed synthetic groups and known operator truth. No width/loss/adversary/seed/threshold rescue or AH1C confirmation opens; O0 remains independently retained. |
| Latest source evidence | 1036 local tests pass. U5.R2AH0's exact Deep Sets, Set Transformer, DANN, VICReg and Meta-OT audit remains mechanism precedent only; AH1D now demonstrates that the frozen hierarchical set predictor does not turn final reference photographs into identifiable operators. U5.R2AI0 is the next deterministic product-style leaf: exactly two orders x three inherited strengths compose the independently severe-clean anchor56 margin-4 and density-cyan champions, requiring material style/non-basic gain plus full ID11/severe veto. This does not alter stock/data gates. No photographs/current film pixels, direct neural RGB, stock/mode claim or integration opens. |
