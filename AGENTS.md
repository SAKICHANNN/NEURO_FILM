...
| Latest algorithm evidence | 1114 local tests pass before the pending post-AM1 full-suite refresh. U5.R2AM1 tests one 21-raw/18-effective bounded SO(3)-coordinate monotone-curve operator on the frozen density-cyan and positive-warm targets. Two 33,345-byte reports are exact. Analytic inverse, positive Jacobian, SO(3), curves, replay, hue, ramp and strength pass, but both fits miss `.015` RMSE, leave the RGB cube and exceed the `.03` neutral-spread gate. Density reaches only 4.49% over global curves, is 41.93% worse than positive-matrix curves and 2.014x K3; positive warm improves 46.27%/14.18% and is .653x K3 but still has `.02124` RMSE. The fixed family closes without clamp/capacity/target/gate/photo rescue. O0 remains the safe high-capacity representation control; operator/data identification, not another compact representation, is the priority. Production default, stock/calibration claims, current-pixel fitting/training and LSM remain unchanged. |
| Latest source evidence | 1114 local tests pass. U5.R2AM0 audits IAC as a representation source, not a film-learning result. The official paper predicts an image-adaptive `3x3` coordinate matrix and three curves, but reports random rank repair and no monotonicity, condition-number, whole-operator inverse, cube/Jacobian or severe-artifact gates. Its FiveK/exposure/WB paired tasks are digital enhancement evidence only, and no author-linked reproducible package was located in the audited official records/searches. Direct execution closes. Only deterministic bounded `SO(3)`, full-cube rather than image-extrema normalization, positive-increment curves and analytic inverse curves survive for a synthetic pilot. No new data, real-film identification, stock/mode claim or integration opens. |
