...
| Latest algorithm evidence | 1025 local tests pass. U5.R2AF1 closes the exact AceTone LUT tokenizer: all controls and byte-exact repeat pass, but every one of eight analytic safe LUTs reconstructs with `7.54–10.60%` negative-Jacobian cells, RGB RMSE `.0512–.0661` and median Delta E76 `8.29–11.14`; even identity folds at `10.60%`. Range and clipping gates pass, proving bounded output is not topology safety. No decoder/loss/checkpoint/projection rescue, Qwen, photos, visual stage, training, film claim or integration opens. |
| Latest source evidence | 1025 local tests pass. U5.R2AG0 derives a clean-room 65-scalar bounded convex-gradient colour map from exact ICNN/optimal-transport/Monge-gap/colour-transfer sources. Its output is in-cube and its Jacobian SPD by construction, but OT supplies only a canonical minimal-displacement palette map and cannot identify a physical film operator from unrelated images. One fixed 16-anchor paired-synthetic challenger to O0 opens; no external code, current pixels, unpaired fitting, generative model, stock claim or integration. AF1 remains closed. |
