...
| Latest algorithm evidence | 1086 local tests pass. U5.R2AI1 still closes the independent promotion of the R2AI0 composition: style/non-basic remains `13.1756/12.6753`, but style gain is `-.5261`, wins are `7/17` and maximum style `32.2442` exceeds the frozen envelope. No rescue opens. U5.R2AJ0B v1 remains closed on its false all-RGB assumption and AJ0B2 passes exact lane-aware integrity. AJ0C0 v1 remains preserved as a pre-execution contract failure. Corrected AJ0C0B now passes its two-process, independently reconstructed controls-only conformance at commit `d8fdcb1`: legal Hald parser/interpolation, analytic Jacobian, RGB16 TIFF, all exact positive/negative/basic/strength/novelty controls and the zero-access ledger pass. Archive bytes, primary CLUT metrics and photograph renders remain exactly zero. Only separate C1 contract design opens; no external CLUT has been measured, applied to a photograph or inspected aesthetically. The production default, stock/calibration claims, current-pixel fitting/training and LSM remain unchanged. |
...
| Ready/contract only | U5.R2AJ0C1 | Freeze the 194-member primary-bank structural frontier; do not read a primary candidate body or metric before the C1 contract commit |
