...
| Latest algorithm evidence | 1051 local tests pass. U5.R2AI0 retains one stronger deterministic B0 composition challenger after two exact 41-image runs. `density_then_anchor__density_s50` reaches gold style/non-basic Delta E76 `15.0576/11.3299`, zero new gold/stress hard clipping, beats both parents in all three autonomous blind rounds and has zero confirmed severe failures on all nine full-resolution gold images; ID11 red speckle/posterization does not reproduce. U5.R2AI1 now has a hash-bound independent-confirmation executor for the exact two parents and retained candidate, paired row/make gates, exact replay, source-visible blind sheets and fail-closed full-resolution review, but no confirmation output/result exists yet. The production default, stock/calibration claims, current-pixel fitting/training and LSM remain unchanged. |
| Latest source evidence | 1051 local tests pass. U5.R2AI1S acquired all 18 exact raw.pixls.us CC0 RAW rows / 271,651,338 bytes / nine camera makes and reproduced its manifest/report byte-identically. Automatic hash/decode/duplicate/development-overlap gates all pass. Autonomous review excludes `fujifilm_s2pro` without replacement because it visibly violates the preregistered target/chart exclusion; the exact confirmation population is therefore 17 rows across all nine makes, 11 recorded content/safety buckets and zero confirmed severe source failures. Unknown author/uploader/roll/lab/scanner fields remain explicit `unknown`; no appearance inference is allowed. This opens only the frozen R2AI1 confirmation of the unchanged B0 composition. No real-film identification, stock/mode claim or integration opens. |
