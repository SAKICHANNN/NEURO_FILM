...
| Latest source evidence | 1008 local tests pass. U5.R2AD0 confirms that RF2.C0 retained non-spatial DIR chemistry but forced spektrafilm inhibitor diffusion to zero, so one exact spatial-on/off external ablation is non-duplicate and ready. The pinned implementation is GPL-3.0-or-later and its profiles/LUTs are CC-BY-SA-4.0; no code/profile copy, fitting, teacher use, stock claim or integration opens. U5.R2AC0 separately retains Filmulator only as a generic shared-developer mechanism prior. |
