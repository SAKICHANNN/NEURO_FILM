...
| Latest algorithm evidence | U5.R2BK24 tests colour-blind DINOv2-S/14 semantic pseudo-pairs across 48 different FiveK photographs and 16 target-hidden development photographs. Two reports are exact (`b819057d...a1bb`). Retrieval uses all 48 targets with zero same-identity matches and beats a target-colour-permuted control by 12.90 median points, but the explicit cube-safe flow regresses 15/16 held-out photographs: median `-89.57%`, worst `-823.23%`. Structure and boundary gates pass, so this is a formal K=1/identity result: semantic similarity contains information but does not identify transferable colour response. FiveK remains digital-retouch control evidence, not film/stock truth. AO6 t15/c35 remains the incumbent research Look Approximation; no larger-encoder rescue, confirmation, router, product, calibration or LSM opens. |
