...
| Latest algorithm evidence | 995 local tests pass. U5.R2AA1 closes the public-datasheet Kodak 250D-to-2383 chain with exact repeat. Its nominal synthetic look is strong/non-basic: identity, joint-basic and per-channel-affine median Delta E76 are 54.43/31.58/20.60. It nevertheless fails frozen spectrum, placement, dye-map and full-ensemble nuisance/effect ratios (`.784/2.462`, `.408/1.918`, `.200/.872`, `1.432/4.467` median/p95), and nominal/full neutral chroma reaches 65.70/129.36. Source traces, reconstruction, range and Jacobian pass, but missing physics dominate. AA2, visuals, fitting, clamp/LUT/neural rescue, stock/calibration and production claims remain closed. U5.R2Z1 separately closes FilmSet ClassNeg case applicability before selector capacity. |
| Latest source evidence | 989 local tests pass. U5.R2AA0 verifies that the current official Kodak VISION3 250D and VISION 2383 sheets each publish channel characteristic, layer sensitivity and separated CMY dye curves; 2383 LAD adds neutral print aims. This opens only a synthetic nuisance-identifiability pilot: digital scene spectra, absolute dye amounts, printer-primary SPDs, scene timing and xenon/viewing response remain unidentified, so no real 250D-to-2383 operator or calibration exists. SF2.9R retains five exact ColorReference IT8/ISO 12641 archives only as physical spectral/target-manufacturing nuisance evidence. SF2.10R repeats exactly and preserves Color Precision only as a future source lead: eight retained HTML responses expose 927 comparison-image URLs, 20 weak stock-looking labels and 456 filename-implied Frontier/Noritsu pairs, with zero image requests. Its pixel DoR fails; current-pixel operator fitting, training, LSM and production integration remain closed. |
