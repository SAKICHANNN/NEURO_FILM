...
| Latest algorithm evidence | 1071 local tests pass. U5.R2AI1 still closes the independent promotion of the R2AI0 composition: style/non-basic remains `13.1756/12.6753`, but style gain is `-.5261`, wins are `7/17` and maximum style `32.2442` exceeds the frozen envelope. No rescue opens. U5.R2AJ0B v1 remains closed on its false all-RGB assumption. Separately versioned U5.R2AJ0B2 now passes two independent exact lane-aware HaldCLUT integrity runs at commit `9bd4e68`; all repeat checks pass, while each child alone remains non-promoting. This opens only AJ0C contract design for a synthetic per-candidate structural frontier. No CLUT has been applied to a photograph or inspected aesthetically. The production default, stock/calibration claims, current-pixel fitting/training and LSM remain unchanged. |
...
| Ready/contract only | U5.R2AJ0C | Freeze a synthetic-only per-candidate Hald structural frontier, control calibration and survivor policy before reading any primary candidate metric |
