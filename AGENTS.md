...
| Latest algorithm evidence | 901 local tests pass plus five focused S1D tests. U5.R2S1D repeatedly selects a nonlearned canonical-histogram KDE `.12` score at median/p90 oracle-output RMSE `.0327/.0508`, versus hard Hellinger Top-1 `.0873/.1283` and global mean `.1128/.1359`. KDE retains `.984` style strength, `.909` non-affine residual and `.935` reference separation with minimum determinant `.0126`, max norm `4.819`, inverse `1.19e-6` and exact spatial-permutation invariance. This is synthetic development; untouched seed `27012` still requires a frozen confirmation. It closes only raw-histogram hard retrieval for this score-recovery question, not future within-stock retrieval over identified operator signatures. U5.R2S0 remains the analytic mechanism pass. No learned score, image, film or stock evidence exists yet. Film fitting, current-pixel training and LSM remain closed. |
