...
| Latest algorithm evidence | 1030 local tests pass. U5.R2AG1 closes the 65-scalar bounded convex-gradient representation after two byte-identical formal runs. Range, exact identity, parameter bounds, analytic SPD/determinant/norm, inverse and serialization gates pass. The positive-film control passes at `.00945` RGB RMSE with `65.71%` affine gain, but the density control is `.04047` against the frozen `.015` ceiling; non-identity whole/partition execution also differs by 1–2 float64 ulps against an exact gate. No anchor, affine-wrapper, temperature, optimizer or tolerance rescue opens. O0 remains the retained synthetic-capable safe representation. |
| Latest source evidence | 1030 local tests pass. U5.R2AH0 opens one development-only ML mechanism audit from exact Deep Sets, Set Transformer, DANN, VICReg and Meta-OT sources. New generated groups will hold a known O0 operator fixed while independently varying content/nuisance; a hierarchical set encoder may predict only bounded O0 parameters under explicit invariance, anti-collapse and content probes. This is distinct from enlarging W1's failed fixed descriptor: new population/seeds are required and W1 confirmation stays sealed. Meta-OT consumes both measures and still does not identify a reference-only film operator. No photographs/current film pixels, direct neural RGB, stock/mode claim or integration opens. |
