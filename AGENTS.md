...
| Latest algorithm evidence | 748 local tests pass. U5.R2B retains `anchor56_chroma_margin4_challenger` as one B0 fixed global Look Approximation challenger; U5.R2C actual B1 remains human/data gated. U5.R2D-D2 close direct unpaired statistics-to-LUT recovery and raw hard retrieval as unidentified. U5.R2E0 now passes all frozen numerical/diversity gates for an original clean-room density-domain negative-to-print primitive: five descriptive witnesses are bounded, non-affine, replay/partition exact and pairwise distinct, with positive sampled Jacobians. This is not yet visual safety or product value; U5.R2E1 must run the frozen gold/stress frontier. Real-film fitting, training and LSM remain closed. |
