...
| Latest source evidence | 1041 local tests pass. U5.R2AH0's Deep Sets/VICReg/DANN/Meta-OT audit remains mechanism precedent only and AH1D closes that learned final-reference route. U5.R2AI0 instead shows that fixed composition of two independently severe-clean project operators can advance the style frontier without ML. U5.R2AI1 must now freeze a zero-overlap, rights/lineage-audited independent digital-photo/OOD population before any new render; order, strength, margin and gates cannot be retuned. No real-film identification, stock/mode claim or integration opens. |
