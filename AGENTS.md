...
| Latest algorithm evidence | 1051 local tests pass. U5.R2AI0 retains one stronger deterministic B0 composition challenger on the 41-image development population, but U5.R2AI1 closes its independent promotion. Two exact 17-image/nine-make runs are byte-identical and clipping-free; the candidate retains style/non-basic Delta E76 `13.1756/12.6753`, yet loses median style to the better parent by `.5261`, wins style on only `7/17` rows and reaches maximum style `32.2442` above the frozen `31.4746` envelope. Automatic gating fails, so blind/full-resolution review is correctly forbidden. No threshold, row, strength, order or margin rescue opens; the production default, stock/calibration claims, current-pixel fitting/training and LSM remain unchanged. |
| Latest source evidence | 1051 local tests pass. U5.R2AI1S acquired all 18 exact raw.pixls.us CC0 RAW rows / 271,651,338 bytes / nine camera makes and reproduced its manifest/report byte-identically. Automatic hash/decode/duplicate/development-overlap gates all pass. Autonomous review excludes `fujifilm_s2pro` without replacement because it visibly violates the preregistered target/chart exclusion; the final population is 17 rows across all nine makes, 11 recorded content/safety buckets and zero confirmed severe source failures. U5.R2AI1 has now consumed those rows and closed independent promotion; they may be development evidence only, never an untouched successor confirmation. Unknown author/uploader/roll/lab/scanner fields remain explicit `unknown`. No real-film identification, stock/mode claim or integration opens. |
