...
| Latest algorithm evidence | 953 local tests pass. U5.R2W1D closes output-only fixed reference-look recovery with repeat-exact evidence. Exact paired neutral/styled references recover the known bounded operator at median/p90 RMSE `.02498/.07282`, proving optimizer/operator headroom. Output-only single and four-reference methods all fail: unseen four-reference ridge reaches `.06631/.08191` but is 42.40% worse than global mean and 33.18% worse than identity, its content-group balanced accuracy is 98.44% versus 25% chance, and identity references produce `.06783` false-transform RMSE against a `.01` gate. `53/55/56` correctly remains one direction with continuous strength. No descriptor/capacity rescue, confirmation or visual candidate opens. Only U5.R2W2F0 FilmSet paired/global recipe explainability is ready; W2F1 output-only recovery, film/stock claims, current-pixel fitting/training and LSM remain closed. |
