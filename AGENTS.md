...
| Latest algorithm evidence | 1123 full-suite tests plus 14 focused AO0/AN1/AN0/J0 tests pass. AN0 exactly recovers three nonneutral two-matrix positive-film witnesses; AN1's fixed soft-L1 promotion closes on one `61.44% < 65%` relative gate despite `.000328`--`.000437` RMSE. AO0 then passes two exact bounded downloads and extraction of 24 paired author-rendered sRGB ColorChecker values from a CC-BY-4.0 real Velvia 50 figure. This opens only leave-one-chart-row-out display-proxy explainability: one stock/illuminant/exposure figure is not RAW, a transferable stock response or calibrated product evidence. |
