...
| Latest source evidence | 1036 local tests pass. U5.R2AH0's exact Deep Sets, Set Transformer, DANN, VICReg and Meta-OT audit remains mechanism precedent only; AH1D now demonstrates that the frozen hierarchical set predictor does not turn final reference photographs into identifiable operators. New algorithm work must change the information structure rather than enlarge the reference encoder. Meta-OT consumes both measures and still does not identify a reference-only film operator. No photographs/current film pixels, direct neural RGB, stock/mode claim or integration opens. |
