...
| Current evidence | 328 local tests pass. U1.6A-D prove finite-support execution, byte-identical two-pass safe-Lab, active simple-halation parity and exact sparse dust/scratch tiling, but physical/grain effects, integration, complete renderer and 100MP memory remain open; inherited square-speck/straight-scratch visuals remain heuristic and are not promoted. U2.1A provides verified replay identity. SF2.2R also rejects a 6.64GB Smithsonian metadata expansion because sampled stock-family evidence is disconnected by unit and lacks exact connected variants. Current profiles remain heuristic look approximations and current pixels remain closed for learning; training, operator fitting and LSM remain forbidden |
...
| Ready/frozen | U1.6F | Stage exact legacy grain through private temporary memmaps and radius-4 tiles; byte parity and cleanup/budget gates required, bounded RAM only, no physical/default/100MP claim |
