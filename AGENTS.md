...
| Latest algorithm evidence | 908 local tests pass. U5.R2S1C confirms fixed canonical-histogram KDE `.12` on 128 untouched palettes: median/p90 oracle-output RMSE `.03046/.05029`, direction `.959`, style retention `.985`, non-affine retention `.909` and reference separation `.934`; median error is 63.46% below hard Hellinger Top-1 and 71.88% below global mean. Minimum determinant `.0150`, max norm `3.993`, inverse `7.35e-7`, permutation and repeat all pass. This retains a synthetic score-recovery mechanism only. Real film histograms entangle scene content with style, so images remain closed pending a content-palette nuisance identifiability leaf. Raw-histogram hard retrieval loses this subproblem but future within-stock retrieval over evidence-backed operator signatures remains distinct. No film, stock or unpaired-operator evidence exists; current-pixel fitting/training and LSM remain closed. |
