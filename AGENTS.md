...
| Latest algorithm evidence | 726 local tests pass. U5.R2B retains `anchor56_chroma_margin4_challenger` as one B0 fixed global Look Approximation challenger. U5.R2C validates the exact K=7 empirical-ceiling protocol but actual B1 remains human/data gated. U5.R2D's 2,304-D Lab descriptor passes its numerical properties, yet red/blue palette L2 is 111.69 and an exact-reference two-LUT witness differs 1.0 on an absent colour. Thus spatial agnosticism does not remove content shortcuts or identify an unpaired operator. U5.R2D1 may benchmark CPU synthetic known operators only; real-film fitting, training and LSM remain closed. |
