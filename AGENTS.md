...
| Latest algorithm evidence | 891 local tests pass. U5.R2O0 is the first frozen K/O-series representation to pass nonlinear fidelity, strict cube range, positive Jacobian, norm, inverse and repeat gates together: a 192-parameter stationary flow reaches `.01091/.00544` confirmation RMSE, improves `84.45%/80.27%` over affine, has min determinants `.117/.275`, max norms `3.993/2.781`, exact endpoints and byte-identical reports. It validates a bounded explicit parameter family, not a new look; no redundant real-image frontier, fitting, stock or production claim opens. U5.R2P0 finds ChameleonTuner useful only for future same-scene misaligned pairs. U5.R2Q0 finds SA-LUT's inference structure conceptually useful because ML predicts simplex 4D-LUT weights and a context map before explicit interpolation, but closes the official checkpoint: training uses a direct Style2Log image generator, GAN supervision and incomplete image/LUT lineage. PST50 is generic grading, not film-stock truth; no checkpoint/dataset run or implementation opens. U5.R2N1 remains closed because its encoder follows geometry/content and its raw map escapes range. U6.2B remains closed on severe salt-speckle morphology. The retained B0 global colour challenger remains U5.R2E1 density-cyan s0.50 (style 13.49, non-basic 7.39, zero new clipping, severe-clean). Film fitting, training and LSM remain closed. |
| Latest source evidence | SF2.9R acquires five exact ColorReference IT8/ISO 12641 transmission-reference archives / 1,468,016 bytes. CRC, deterministic parse and 288/288 common-patch gates pass twice byte-identically; every archive supplies 41 direct spectral samples from 380--780nm. Common-patch target Lab differences are measurable (pairwise medians 1.52--4.22 Delta E76), but these are batch-average scanner-calibration targets manufactured toward IT8 aims, with no common uncalibrated recorder input, camera scene, independent roll/process replication or reusable training licence. Retain only physical spectral/target-manufacturing nuisance evidence; no stock operator fitting, training, LSM, redistribution or production integration opens. |
