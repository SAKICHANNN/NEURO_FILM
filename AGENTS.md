...
| Latest source evidence | 977 local tests pass. SF2.9R retains five exact ColorReference IT8/ISO 12641 archives only as physical spectral/target-manufacturing nuisance evidence. SF2.10R is implemented/frozen as an offline Color Precision metadata preflight: eight retained HTML responses expose 927 comparison-image URLs, 20 stock-looking filename labels and 456 filename-implied Frontier/Noritsu pairs, but no image URL has been requested. Labels are weak metadata; roll/process/profile and verified pairing remain unknown, scanner auto-adjustment is disclosed, and no explicit comparison-pixel research reuse grant was found. Pixels, operator fitting, training, LSM and production integration remain closed. |
