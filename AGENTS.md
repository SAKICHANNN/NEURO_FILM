...
| Latest source evidence | SF2.6R refreshes the strongest controlled-pair publication leads. SillyStill still exposes one illustrative pair, placeholder dataset links and no root licence; Emulating Emulsion exposes its two-matrix/three-sigmoid method and figures but no measurements, fitted parameters, code or reusable data licence. Neither opens fitting/training. SF2.5R remains a physical-register data stop. |
