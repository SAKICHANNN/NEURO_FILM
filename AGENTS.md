...
| Latest algorithm evidence | AO3's fixed tone/chroma factorization yields one exact automatic survivor at gold style/non-basic `10.53/5.53` with zero new gold/stress clipping. It is severe-clean on all nine full-resolution gold images, including ID11, but loses 0/3 frozen autonomous blind rounds to the retained vivid B0 comparator; only two sheet layouts are distinct. Retain the explicit boundary guard as real-chart-driven severe-clean engineering, not the aesthetic champion. One small author-rendered Velvia display proxy remains underidentified and is not RAW, independent confirmation, transferable stock response or calibrated/product evidence. |
