...
| Latest algorithm evidence | U5.R2BH1 completes the independent fixed B0-versus-AO6 global-policy confirmation on 12 fresh CC0 RAW sources from 12 camera makes. Two 24-output runs plus nine blind sheets are byte-identical (`1376d6ef...84fe`; stable render `9f52dd3e...2640`) and automatic/full-resolution severe gates pass with zero confirmed severe failures. B0 wins all three rounds 7-5 but reaches only 21/36 aggregate choices, one below the frozen 22-choice gate. Do not add rounds or retune: retain fixed AO6 t15/c35 as the incumbent research global Look Approximation policy and continue a genuinely distinct algorithm leaf. Production default, stock/calibration claims, current-pixel fitting/training, selector/router and LSM remain unchanged. |
