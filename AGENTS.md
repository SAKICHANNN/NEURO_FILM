...
| Latest algorithm evidence | 1024 local tests pass. U5.R2AF1 now has an isolated exact-checkpoint runner and project-owned manifest evaluator for eight analytic orientation-preserving LUTs; source/negative/duplicate/repeat, range, fidelity and Jacobian gates are implemented but formal checkpoint outputs remain unread. U5.R2AE1 remains closed: all eight strongly stylized/non-basic external LUTs had `1.61–9.50%` negative-Jacobian cells versus the frozen `.5%` ceiling. No photos, visual stage, repair, teacher, stock claim or integration opens. |
| Latest source evidence | 1019 local tests pass. U5.R2AF0 excludes AceTone's published Qwen/GRPO LUT-token selector as generative AI and finds no released per-LUT rights manifest/full AceTone-800K corpus; its public transfer benchmark has no declared dataset licence and was not downloaded. The included Apache-2.0 VQ-VAE checkpoint is bounded by a sigmoid but has no monotonicity, positive-Jacobian or invertibility objective. Only one exact-checkpoint synthetic safe-LUT topology stress opens; no Qwen, photos, benchmark, training, film claim or integration. |
