...
| Latest algorithm evidence | 967 local tests pass. U5.R2W2F0 closes FilmSet paired/global recipe explainability: Cinema is basic-only; ClassNeg/Velvia are adaptive-or-spatial, while all O0 structure gates pass. U5.R2X0 then closes the 2025 automatic affine-barycentric palette-transfer path analytically with exact repeat. Direct transfer equals one affine RGB operator to `9.99e-16` over 10,000 queries. Valid published transports still admit determinant `-1` and a `5p-2` witness mapping the RGB cube to `[-2,3]`. Palette extraction/CPLEX only choose affine coefficients; no nonlinear, orientation-safe or range-safe challenger exists in that global path. No image/CPLEX rescue, interactive-local claim, W2F1, film/stock claim, current-pixel fitting/training or LSM opens. |
