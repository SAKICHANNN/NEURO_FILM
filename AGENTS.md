...
| Latest algorithm evidence | 1099 local tests pass. U5.R2AI1 still closes independent promotion of the R2AI0 composition. U5.R2AJ0C0B passes exact controls-only Hald conformance, and the frozen U5.R2AJ0C1 primary execution now closes the external bank: two child byte streams and independent parent reconstruction are exact; all 194 candidates pass style/non-basic evidence, yet all 194 fail minimum Jacobian determinant and 175 exceed negative-Jacobian fraction, leaving zero structurally safe representatives. Style/non-basic median Delta E76 is `16.3649/14.1791`, so the route fails for topology rather than blandness. Zero photographs were rendered; no threshold, smoothing, projection, teacher or training rescue opens. The production default, stock/calibration claims, current-pixel fitting/training and LSM remain unchanged. |
