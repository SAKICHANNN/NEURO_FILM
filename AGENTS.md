...
| Latest algorithm evidence | U5.R2BL5 puts a `0.5/255` strict output margin inside FilmMatch's fitted two-matrix sigmoid while preserving paired capacity (`.04669/.09633` reflective/emissive RMSE, zero cube endpoint samples, minimum Jacobian `.01064`). U5.R2BL6 applies that exact fixed operator to 12 reused CC0 RAWs from 12 makes: two runs are byte-identical (`3410d2f1...e2c1c4`), median style displacement is `.12201`, output/new-boundary fractions are zero, and mapping-sealed autonomous review prefers BL5 over fixed AO6 in all three rounds and `32/36` choices with zero confirmed severe artifact. Retain a high-priority internal Look Approximation challenger, not population preference, independent confirmation, film truth or product evidence; AO6 remains incumbent until a genuinely fresh rights-cleared confirmation. |
