...
| Latest algorithm evidence | 983 local tests pass. U5.R2Z0 now tests the fixed-case intuition directly inside two known FilmSet Capture One recipe controls. ClassNeg has repeat-exact held-out case-bank Oracle value: mean RMSE improves 16.72% over shared O0, wins 75% and has a positive bootstrap lower bound. The frozen input-only spatial photometric nearest neighbour is nevertheless worse than shared (`.02072` vs `.01536`) and closes `-208.62%` of the Oracle gap, so no selector or router opens. Velvia misses the Oracle magnitude and win gates at 9.78%/68.75% and closes. Correct-recipe Oracles improve on the wrong-recipe control by 73.75-79.19% and win every query, while every bounded flow passes structure gates. This is paired software-recipe evaluator evidence only: no W2F1, visual candidate, film/stock claim, current-pixel fitting/training, LSM or production integration opens. |
| Latest source evidence | 977 local tests pass. SF2.9R retains five exact ColorReference IT8/ISO 12641 archives only as physical spectral/target-manufacturing nuisance evidence. SF2.10R now repeats exactly and preserves Color Precision only as a promising future source lead: eight retained HTML responses expose 927 comparison-image URLs, 20 weak stock-looking labels and 456 filename-implied Frontier/Noritsu pairs, with zero image requests. Roll/process/profile and verified pairing remain unknown, scanner auto-adjustment is disclosed, and no explicit comparison-pixel research reuse grant or digital counterpart manifest was found. Pixel DoR fails; operator fitting, training, LSM and production integration remain closed. |
