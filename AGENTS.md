...
| Latest algorithm evidence | 1011 local tests pass. U5.R2AD1 finds source-default spektrafilm spatial DIR diffusion has measurable local/non-basic value (median paired Delta E76 `.5458`, residual after simple local-contrast match `.5696`) and exactly repeats final PNGs, but every float output differs across runs by up to `7.61e-15` and worst new isolated red speckles are `.1481%` versus the frozen `.1%` limit. Automatic failure forbids vision, parameter/FFT/smoothing rescue and AD2. U5.R2AC1 and the DoRF response bank remain separately closed. |
| Latest source evidence | 1011 local tests pass. U5.R2AE0 retains pinned MIT `spectral_film_lut` only for one isolated synthetic structural-bank audit. Its headless path runs and exports 86 unique objects, but current profiles lack source/page/uncertainty fields; 31 historical datasheet PDFs were removed without a replacement mapping and their redistribution status is unidentified. No PDF/profile/source copy, fitting, teacher use, stock claim or integration opens. U5.R2AD1 and U5.R2AC1 remain closed. |
