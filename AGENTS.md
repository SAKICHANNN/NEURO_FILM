...
| Latest algorithm evidence | 1161 local CPU tests pass. U5.R2AO6 applies the fixed AO5 71-pair Velvia display-proxy operator only as a small factorized residual after the vivid B0 look. Two exact 164-output runs retain three zero-clipping candidates. `b0_plus_film_t15_c35` reaches gold style/non-basic/relative-film Delta E76 `13.2353/9.8742/2.0468`, beats fixed B0 in 2/3 blind layouts and the stronger residual in 3/3, with zero confirmed severe failures across both shortlisted candidates on all nine full-resolution gold images. Retain it as the new B0 development champion pending independent OOD confirmation. This does not establish Velvia stock response, calibration, independent human preference or product promotion. |
