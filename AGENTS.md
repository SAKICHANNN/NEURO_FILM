...
| Latest algorithm evidence | 932 local tests pass. U5.R2S3D closes both frozen unconditional distribution objectives with repeat-exact CUDA evidence. RFF-MMD-192 reduces fit loss but reaches only 11.80% held-out distribution improvement, hidden-operator median/p90 RMSE `.11591/.15423` and A/B operator RMSE `.07902`; SW-24 reaches 3.36%, `.11444/.16447` and `.13138`. Both miss the frozen 30%, `.07/.10` and `.04` gates while all cube/range/Jacobian/norm/inverse/replay/style gates pass. This is objective-level non-identification rather than numerical collapse; no S3 capacity rescue, confirmation or visual candidate opens. U5.R2S4D is now ready to test generated correct versus pooled/shuffled condition correspondences. No film, stock or real unpaired-operator evidence exists; images, current-pixel fitting/training and LSM remain closed. |
