...
| Latest algorithm evidence | 886 local tests pass. U5.R2N1 closes the external ModFlows B0 route before real images: checkpoint/clean-room 4-64-3 flow compatibility, exact repeat, identity error `5.48e-5`, min determinant `.6368` and max norm `1.607` pass, but histogram-preserving pixel permutation drops embedding cosine to `.7255`, palette/geometry distance ratio is only `.803`, and raw RGB spans `[-.073,1.119]`. No clamp/fine-tune/B6/reference-image rescue opens. U6.2B also closes both Boolean-grain crop policies on 4/5 and 5/5 severe salt-speckle failures; legacy `.018` is 0/5. The retained B0 global colour challenger remains U5.R2E1 density-cyan s0.50 (style 13.49, non-basic 7.39, zero new clipping, severe-clean). Film fitting, training and LSM remain closed. |
