...
| Latest algorithm evidence | 1019 local tests pass. U5.R2AE1 finds that all eight source-bound `spectral_film_lut` chains are strongly stylized (median Delta E76 `19.07–23.96`), non-basic (`9.81–23.72`) and distinct in all three common-output families; exact replay, duplicate and strength-path controls pass. But every LUT has `1.61–9.50%` negative-Jacobian cells versus the frozen `.5%` ceiling, so photographs/vision and AE2 are forbidden. No smoothing/projection/clamp/profile rescue, teacher, stock claim or integration opens. |
