...
| Latest algorithm evidence | 1051 local tests pass. U5.R2AI0 retains one stronger deterministic B0 composition challenger on the 41-image development population, but U5.R2AI1 closes its independent promotion. Two exact 17-image/nine-make runs are byte-identical and clipping-free; the candidate retains style/non-basic Delta E76 `13.1756/12.6753`, yet loses median style to the better parent by `.5261`, wins style on only `7/17` rows and reaches maximum style `32.2442` above the frozen `31.4746` envelope. Automatic gating fails, so blind/full-resolution review is correctly forbidden. No threshold, row, strength, order or margin rescue opens. U5.R2AJ0A now opens a genuinely independent licensed HaldCLUT source lane, but only bounded acquisition and synthetic structural screening are ready; no CLUT has been downloaded, decoded or rendered yet. The production default, stock/calibration claims, current-pixel fitting/training and LSM remain unchanged. |
| Latest source evidence | 1051 local tests pass. U5.R2AI1S acquired all 18 exact raw.pixls.us CC0 RAW rows / 271,651,338 bytes / nine camera makes and reproduced its manifest/report byte-identically. Automatic hash/decode/duplicate/development-overlap gates all pass. Autonomous review excludes `fujifilm_s2pro` without replacement because it visibly violates the preregistered target/chart exclusion; the final population is 17 rows across all nine makes, 11 recorded content/safety buckets and zero confirmed severe source failures. U5.R2AI1 has consumed those rows, so they are development evidence only. Separately, U5.R2AJ0A range-reads the RawTherapee archive metadata/README and finds an exact 421,602,289-byte, version-20150920 CC BY-SA 4.0 source with 194 frozen non-Creative colour CLUT entries; no LUT image body was accessed. Preset names remain informational Look Approximation labels, not real-film or stock truth. |
