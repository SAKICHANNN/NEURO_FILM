...
| Latest source evidence | 1030 local tests pass. U5.R2AG0's clean-room ICNN/optimal-transport/Monge-gap sources remain useful structural priors, but AG1 demonstrates that the resulting 16-anchor SPD-Jacobian family is too restrictive for both frozen nonlinear controls. OT remains only a canonical minimal-displacement palette map and cannot identify a physical film operator from unrelated images. No current pixels, unpaired fitting, generative model, stock claim or integration opens; AF1 and AG1 are closed. |
