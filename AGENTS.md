...
| Latest algorithm evidence | 1123 full-suite tests plus nine focused AN0/J0 tests pass. U5.R2AN0 adds a deterministic paired fitter for the existing two-matrix positive-film response operator. On a frozen 3,168-row synthetic known-truth design with complement-held patch, illuminant and exposure groups, two exact process reports recover all three nonneutral witnesses at `8.59e-17`--`1.06e-16` confirmation RMSE; one-matrix controls remain `.00982`--`.01252`. All cube, Jacobian, determinant, maximum-error and repeat gates pass. This proves noise-free observable operator recovery and the need for the second matrix in these witnesses, not a fit to Velvia or any real film. Noise/outlier robustness is next; production default, stock/calibration claims, current-pixel fitting/training and LSM remain unchanged. |
