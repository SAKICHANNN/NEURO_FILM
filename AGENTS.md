...
| Latest algorithm evidence | 901 local tests pass. U5.R2S0 is the first clean mechanism result combining reference-palette forcing with the safe O0 representation: three exact analytic palette scores produce identity RMSE `.132-.153`, best-affine residual `.053-.066`, minimum pairwise operator RMSE `.1078` and log-density gain `2.40-3.38`, while remaining inside the RGB cube with minimum determinant `.0142`, maximum norm `4.162`, inverse error `1.18e-6` and exact replay. This opens only a separately frozen synthetic canonical-histogram score/velocity recovery test; no learned score, image, film or stock evidence exists yet. U5.R2R1 remains closed because its official D-LUT trajectory has range/orientation survivors `0/40`. U5.R2P0 remains future same-scene prior only; U5.R2Q0 closes the official SA-LUT checkpoint because training uses a direct Style2Log generator, GAN and incomplete lineage. U6.2B remains closed on severe salt speckles. The retained B0 global colour challenger remains U5.R2E1 density-cyan s0.50 (style 13.49, non-basic 7.39, zero new clipping, severe-clean). Film fitting, current-pixel training and LSM remain closed. |
