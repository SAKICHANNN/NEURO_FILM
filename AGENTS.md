...
| Current evidence | 356 local tests pass. U1.6A-D/F prove finite-support execution, byte-identical two-pass safe-Lab, simple-halation, sparse dust and exact temporary-disk-staged legacy grain; U1.6E closes because halation/dust order is not output-identifiable. Physical halation, orchestration, complete renderer and 100MP total memory remain open. Dust shapes remain heuristic; grain 0.35 is visually rejected despite parity. U2.1A provides verified replay identity. SF2.2R rejects a disconnected 6.64GB Smithsonian expansion; SF2.3 finds only Ektar100/UltraMax400 individually eligible and one shared author, so the Commons union has zero redundant edges. RF2.C0 retains one Ektar100/fixed-e0 spektrafilm Look Approximation as an external comparison control (style 8.03, non-basic residual 7.34, 0% new clipping, no severe visual failure); auto exposure spans roughly 0.2x--4.2x scene luma and is negative adaptation evidence, not stock distinction. Current pixels remain closed for learning; training, operator fitting and LSM remain forbidden |
