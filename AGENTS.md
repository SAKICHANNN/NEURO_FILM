...
| Latest algorithm evidence | 913 local tests pass. U5.R2S2D closes simple content-nuisance recovery: raw styled KDE has lowest median oracle error `.06393` but strongly transforms identity-style controls at `.10494` and reaches max norm `8.228`, so it is content-confounded. Best bounded ridge is worse at `.07255`, with identity-negative `.09131`, same-style replicate `.07789` and norm `10.702`. Best density-ratio has exact identity semantics but error `.10534`, only 4.25% below global `.11002`. Both full reports are identical; no confirmation or capacity rescue opens. U5.R2S1C remains valid only when samples directly represent the target palette density. The next distinct synthetic leaf may directly fit the safe flow by distribution matching while separating distribution fit from oracle recovery. No film, stock or unpaired-operator evidence exists; images, current-pixel fitting/training and LSM remain closed. |
