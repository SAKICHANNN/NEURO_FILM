...
| Latest algorithm evidence | 1004 local tests pass. U5.R2AB1 finds that exact historical DoRF channel responses are not bland: all 46 strict triplets pass style and joint-basic residual gates, and 27 pass every individual shared/power/basic/neutral/derivative/range gate. The bank nevertheless fails frozen diversity because the closest survivor pair is only `.001016` output RMSE versus `.02`; AB2, images, bank/router and post-result rescue close. Independent 1D monotone curves also contain no cross-channel interaction by construction. U5.R2AA1 separately closes the Kodak 250D-to-2383 datasheet chain as strong/non-basic but nuisance-unidentified. |
