...
| Latest algorithm evidence | 737 local tests pass. U5.R2B retains `anchor56_chroma_margin4_challenger` as one B0 fixed global Look Approximation challenger; U5.R2C actual B1 remains human/data gated. U5.R2D proves global Lab statistics are palette-sensitive and do not identify absent-colour behavior. U5.R2D1 then finds exact canonical-reference delta improves 54.3% over target-only on generated operators, but ordinary unpaired source/reference interaction is 125% worse than the global mean and captures negative style. This is missing-information evidence, not permission for a larger network. U5.R2D2 may test multiple imperfect canonicalizers and hard retrieval on generated truth only; real-film fitting, training and LSM remain closed. |
