...
| Latest algorithm evidence | 775 local tests pass. U5.R2E1 retains one cyan-shadow/warm-highlight density challenger at strength 0.50 with style 13.49, non-basic residual 7.39, zero new clipping and no confirmed severe failure in 27 full-resolution renders. U5.R2F1-G1 show external ML conditioning can be strongly reference-sensitive and constrained distillation preserves style, but every frozen safety route yields zero automatic survivor. U5.R2H1 finds A0 source-content routing signal, but H2 closes the exact hard 1-NN: 0/41 routed severe and 0/10 newly severe, yet all three blind rounds are 6 routed wins / 4 density-global wins below the frozen 7/10 gate. H0 now finds a clean-room Velvia 50 datasheet witness feasible because its public sheet has characteristic, sensitivity and separated CMY dye curves; Portra/Ektar full-spectral and negative-to-print models remain data-incomplete. H0A is synthetic-only and must first survive graph and metamer-identifiability gates. No retuning, stock, preference or production claim opens. Real-film fitting, training and LSM remain closed. |
