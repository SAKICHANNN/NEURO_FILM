...
| Latest algorithm evidence | 1071 local tests pass. U5.R2AI1 still closes the independent promotion of the R2AI0 composition: style/non-basic remains `13.1756/12.6753`, but style gain is `-.5261`, wins are `7/17` and maximum style `32.2442` exceeds the frozen envelope. No rescue opens. U5.R2AJ0B v1 remains closed on its false all-RGB assumption and AJ0B2 passes exact lane-aware integrity. AJ0C0 v1 is now also preserved as a pre-execution contract failure: it confused probe-cube and legal Hald geometry and under-specified evidence/negative/novelty/selection rules, with zero archive or primary access. Separately versioned AJ0C0B corrects only those controls and is ready for implementation; C1 stays blocked behind an exact two-process pass and its own contract. No external CLUT has been measured, applied to a photograph or inspected aesthetically. The production default, stock/calibration claims, current-pixel fitting/training and LSM remain unchanged. |
...
| Ready/implementation | U5.R2AJ0C0B | Run the corrected strict synthetic-only Hald parser/Jacobian/gradient/basic/strength/novelty controls twice with exactly zero archive or primary-candidate access |
| Blocked on C0B pass | U5.R2AJ0C1 | Freeze, then screen all 194 licensed Color CLUTs individually; do not read a primary candidate metric before the C1 contract commit |
