...
| Latest algorithm evidence | 989 local tests pass. U5.R2Z0 finds repeat-exact ClassNeg case-bank Evaluator Oracle value (16.72% mean gain, 75% wins, positive bootstrap lower), but its frozen input-only spatial selector is worse than shared and closes `-208.62%` of the gap; Velvia misses Oracle gates. U5.R2Z1 is now implemented and frozen before formal fitting: separate query-content/operator-signature PCA, leave-one-query-and-operator-out rank-2 then rank-4 scalar-loss ridge, fold-local signature shuffles and OOD fallback. Fresh ClassNeg identities 17--32 remain code-gated behind a complete development pass. No Z1 result, selector promotion, neural/router/visual candidate, W2F1, film/stock claim, current-pixel fitting/training, LSM or production integration exists yet. |
