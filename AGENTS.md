...
| Latest algorithm evidence | 1071 local tests pass. U5.R2AI1 still closes the independent promotion of the R2AI0 composition: style/non-basic remains `13.1756/12.6753`, but style gain is `-.5261`, wins are `7/17` and maximum style `32.2442` exceeds the frozen envelope. No rescue opens. U5.R2AJ0B v1 remains closed on its false all-RGB assumption. Separately versioned U5.R2AJ0B2 passes two exact lane-aware HaldCLUT integrity runs. U5.R2AJ0C is now split: C0 freezes a controls-only encoded-sRGB/trilinear conformance and threshold calibration with zero archive/primary reads; only an exact C0 pass may open a separately committed C1 screen of 194 primary Color CLUTs. No external CLUT has been measured, applied to a photograph or inspected aesthetically. The production default, stock/calibration claims, current-pixel fitting/training and LSM remain unchanged. |
...
| Ready/implementation | U5.R2AJ0C0 | Run the frozen synthetic-only Hald parser/Jacobian/gradient/basic/strength/novelty controls twice with exactly zero archive or primary-candidate access |
| Blocked on C0 pass | U5.R2AJ0C1 | Freeze, then screen all 194 licensed Color CLUTs individually; do not read a primary candidate metric before the C1 contract commit |
