...
| Latest algorithm evidence | 1068 local tests pass. U5.R2AI0 retains one stronger deterministic B0 composition challenger on the 41-image development population, but U5.R2AI1 closes its independent promotion. Two exact 17-image/nine-make runs are byte-identical and clipping-free; the candidate retains style/non-basic Delta E76 `13.1756/12.6753`, yet loses median style to the better parent by `.5261`, wins style on only `7/17` rows and reaches maximum style `32.2442` above the frozen `31.4746` envelope. No rescue opens. U5.R2AJ0B safely acquires the exact HaldCLUT archive but its unchanged v1 contract closes when a B&W auxiliary image is mode `L`, not RGB/RGBA. A diagnostic-only profile inventory cannot convert that failure into a pass. Separately versioned AJ0B2 is now frozen to confirm exact lane-aware decode profiles twice; no CLUT has been applied to a photograph or inspected aesthetically. The production default, stock/calibration claims, current-pixel fitting/training and LSM remain unchanged. |
| Latest source evidence | 1068 local tests pass. U5.R2AI1S acquired all 18 exact raw.pixls.us CC0 RAW rows / 271,651,338 bytes / nine camera makes and reproduced its manifest/report byte-identically; its final 17 rows are now consumed development evidence. The licensed RawTherapee archive is locally retained at exactly 421,602,289 bytes, published MD5 `4742e362...85e1` and SHA-256 `0ffca81f...9dc2`. AJ0B v1 fails before evidence promotion on the first grayscale B&W auxiliary entry. Read-only post-failure enumeration finds 227 Color entries all RGB, 66 B&W entries all 8-bit `L`, two 16-bit RGB root controls and no ICC in 295 images. The primary 194 non-Creative Color paths remain unchanged. Preset names remain informational Look Approximation labels, not real-film or stock truth. |
...
| Ready/versioned correction | U5.R2AJ0B2 | Reconfirm all 295 exact lane-aware format/mode/bit-depth/Hald profiles in two new processes; no photograph render or aesthetic inspection |
