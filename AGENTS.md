...
| Latest algorithm evidence | U5.R2BH0 completes the previously censored fixed-bank Oracle on ten fresh CC0 RAW sources. Two 50-output runs and reports are byte-identical (`976f9e73...aacc`; stable `1c398ba9...dcb`) and pass automatic/severe gates. Fixed B0 is the best all-round global arm, but leave-one-round-out hard selection gains only 2 ranks/3.85% with three stable non-global sources, below the frozen 6-rank/8%/four-source gates. Selector/router training and dense blending close; B0 remains fresh global Look Approximation evidence pending an independent B0-versus-AO6 confirmation. Production default, stock/calibration claims, current-pixel fitting/training and LSM remain unchanged. |
