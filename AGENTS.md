...
| Latest algorithm evidence | 972 local tests pass. U5.R2W2F0 closes FilmSet paired/global recipe explainability: Cinema is basic-only; ClassNeg/Velvia are adaptive-or-spatial, while all O0 structure gates pass. U5.R2X0 closes the 2025 automatic affine-barycentric palette-transfer path analytically with exact repeat. U5.R2Y0 then closes NegClone 0.2.0 as a stock learner with exact-repeat film-free counterfactuals: scene colour alone moves its alleged stock bias by `.8485` L2, exposure distribution alone moves the tone curve by `254.4/255`, and ordinary checker texture becomes `.4` grain while flat grey becomes zero. Median aggregation does not identify these nuisance variables. No more-photo/neural/preset rescue, W2F1, film/stock claim, current-pixel fitting/training or LSM opens. |
