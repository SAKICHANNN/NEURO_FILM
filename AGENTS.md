...
| Latest source evidence | 1114 local tests pass. U5.R2AL0 audits NamedCurves/NamedCurves+ as a representation source, not a film-learning solution. The papers use endpoint-fixed monotone RGB curves per colour group, but depend on a learned canonicalizer, learned colour-name probabilities and attention/transformer fusion trained on paired aesthetic retouches. The official repository at `35af838...` has no detected/root code licence and contains a learned colour-naming asset; code, checkpoints, assets and outputs are forbidden. Only a clean-room continuous analytic chroma-sector curve-bank hypothesis survives for a synthetic capacity pilot. No new data, real-film identification, stock/mode claim or integration opens. |
...
| Ready/source audit | U5.R2AM0 | Audit globally invertible coordinate-system plus monotone-curve representations; retain only a bounded clean-room synthetic hypothesis with analytic inverse |
