...
| Latest algorithm evidence | 1041 local tests pass. U5.R2AI0 retains one stronger deterministic B0 composition challenger after two exact 41-image runs. `density_then_anchor__density_s50` reaches gold style/non-basic Delta E76 `15.0576/11.3299`, zero new gold/stress hard clipping, beats both parents in all three autonomous blind rounds and has zero confirmed severe failures on all nine full-resolution gold images; ID11 red speckle/posterization does not reproduce. This is development Look Approximation evidence only: the production default, stock/calibration claims, current-pixel fitting/training and LSM remain unchanged. |
| Latest source evidence | 1046 local tests pass. U5.R2AH0's Deep Sets/VICReg/DANN/Meta-OT audit remains mechanism precedent only and AH1D closes that learned final-reference route. U5.R2AI0 instead shows that fixed composition of two independently severe-clean project operators can advance the style frontier without ML. U5.R2AI1S freezes 18 exact raw.pixls.us CC0 RAW rows / about 259 MB / nine camera makes and now has a resumable exact-hash downloader, neutral RAW decoder, lineage-complete manifest, within/cross-pool duplicate audit and contact-sheet preflight. At least 16 rows/eight makes, zero exact/dHash overlap and autonomous content/severe review must pass before any operator render. Unknown author/uploader/roll/lab/scanner fields remain explicit `unknown`; no appearance inference is allowed. No real-film identification, stock/mode claim or integration opens. |
