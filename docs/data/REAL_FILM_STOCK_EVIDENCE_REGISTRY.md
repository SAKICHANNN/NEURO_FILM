...
| ColorReference Velvia 100F Set 3 | exact stock and target-set statement | one physical target set; five slides observed by four scanner/software pipelines | strong same-slide scanner/software connectivity, no independent rolls | internal scanner-nuisance control only; SF2.7R pass | free testing/development statement but no standard redistribution licence; no stock fitting/training |
