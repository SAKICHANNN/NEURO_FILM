...
| PROV VPRS 17684/17690 | institutional master negatives plus a linked register described as tracking film stock | current evidence is series-level catalogue description only; per-negative stock rows are unobserved | agency/date/negative-number ranges exist; photographer/process/scanner detail remains unknown | SF2.5R metadata-only source feasibility | official API metadata is non-commercial CC-BY-NC; image rights/use require separate audit; no pixels requested |
