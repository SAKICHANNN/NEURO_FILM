...
| ColorReference Velvia 100F recorder-source six-set lane | exact material header on all 60 measurement members | six target-set/charge IDs x five slides; all share `PROD_DATE 2005:05`, physical sheet/roll/process unknown | five common film-recorder RGB grids and 30 measured slide identities, 288 patch IDs each | AQ1 repeated-set proxy identifiability pass: radius median/p95 `.553/1.280` Delta E76; AQ2 held-set explicit baselines only | source is unknown-profile recorder RGB, not camera/sRGB; target is measured slide not scanner RGB; no formal open licence, independent-roll, calibration or photo-transfer claim |
