...
| SF3.A3X controlled-data authorization queue | no new observation; ranks FilmMatch PortraMatch, PARVEC, FILM2PAINT/ZB and NTNU Colourlab by the exact stocks and controlled measurements already documented | asks holders to supply explicit roll/process/scanner/source/scene groups and a sealed confirmation role before pixels | no external message, payload, media or pixel request has occurred | acquisition-enablement record only; not `S0`, source admission or scientific evidence | user must explicitly authorize outreach; a reply must independently grant fitting/derived-output rights and pass a new exact-manifest source freeze |
