...
- Consumer branch now continues through P177; main `efb9ba82` has merged
  ...
  binding directory ownership. V46 correctly pins P173-P175 but is now a
  historical snapshot because P176/P177 close the same ownership class for
  published destinations, rollback backups and staging entries. A replacement
  manifest is pending. The branch also retains local 24 MP,
...
- D-PCT latest stable observed snapshot is `e667fe1e`; R0JA directly parses
  ...
  R0EL measures fresh Windows/Python worker processes on the exact native
  3040x2024 input: median peak RSS falls from 381,087,744 to 222,691,328
  ...
  R0EN measures fresh Windows/Python workers on the same native input:
  full/atomic/stream median peaks are 381,220,864 / 222,846,976 /
  77,516,800 bytes, and stream is 65.2152% below atomic with every stream
  worker below every atomic worker. All 12 output hashes remain exact. This is
  checksum-sink memory evidence only, not an incremental image encoder or
  media/product integration.
  R0EO directly writes 32 callback strips into deterministic 73,835,936-byte
  uncompressed float32 RGB TIFF files without retaining full RGB. Independent
  decoding validates exact tags, offsets/counts and the R0EN pixel hash;
  second-strip rejection, short writes and finalization failure publish no
  final file. The format deliberately carries no ICC, orientation or colour
  transform, so this remains private scene-linear TIFF feasibility rather
  than a product media contract.
  R0EP isolates that writer as a freestanding three-export C byte-sink ABI
  with caller-owned 64-byte state and no libc, heap, filesystem, VLA or
  full-frame storage. Four MSVC/LLVM 3040x2024 runs reproduce the R0EO file
  hash exactly; Android arm64/x86_64 links and macOS/iOS arm64 objects are
  ...
  R0ER wraps the unchanged writer in an Android native executable and performs
  eight temp/finish/flush/fsync/close/rename transactions. Every pulled
  82,836-byte TIFF independently decodes to the frozen pixel/strip/tag facts;
  third-strip rejection retains 64-row progress and leaves no final/temp
  ...
  R0EZ executes the same explicit-kind C ABI on an Android 14 x86_64 virtual
  device using one nontrivial Ricoh GR II DNG-final case and one Panasonic
  vendor-cam_xyz case. Two cold/wiped boots with two replays each match the
  pre-Android MSVC/LLVM oracle exactly over 257 pixels per kind, including
  in-place and nonfinite/overlap/invalid/singular failure-atomic paths.
  Runtime identity is `3f1ee308...33fcf`; report SHA is
  `0ffb8d3c...88f226`. Owned processes, AVD and temporary mapping return to
  ...
  R0FF hash-locks LibRaw 0.22.0 and independently reconstructs its actual DNG
  policy: prefer D65, otherwise the first daylight-type ColorMatrix; apply
  matching CameraCalibration/AnalogBalance; row-normalize and use LibRaw's
  pseudoinverse. It does not interpolate with AsShotNeutral or use
  ForwardMatrix. All four `rawpy.color_matrix` values match within one
  float32 ULP and all unchanged R0EV pixel gates pass twice. The separate
  `rgb_xyz_matrix` diagnostic matches LibRaw's hardcoded colordata for three
  cameras and remains zero for Samsung, proving that diagnostic table is not
  the final DNG matrix source. Stable identity is
  `3046a853...4a840f9`; second report SHA is
  ...
  R0FI adds a constructed policy-identifiability oracle from six locked CC0
  photographs, four Bayer layouts and two exact replays. The synthetic camera
  values are explicitly generated by inverting the frozen Adobe
  dual-illuminant policy; all 24 DNGs are accepted by rawpy and the official
  SDK, and Stage-3 block interiors recover the camera values exactly. On this
  construction, Adobe-policy p99 linear error is `1.7724e-5` versus
  `4.0504e-2` for the LibRaw D65-only policy, a ratio of `0.0004376`; stable
  identity is `9ebeced2...89560c5`. Because the oracle is generated from the
  Adobe policy, this proves policy identifiability and implementation
...
| Rollback-safe image/recipe/report transaction | P16-P18 transactional local file path; corrected P173/P175 binds every newly created directory to its file identity plus an exclusive random transaction marker; P174 rejects nested run-file topology before decode; P176 binds published destination and rollback-backup entry identities, preserves foreign replacements and retains the original backup when rollback cannot safely restore it; P177 applies exact entry-identity cleanup to all local/reference/shared/runtime staging and delivery stage files | COMPLETE for current local renderer, within explicitly bounded filesystem semantics | Bind P30 authorization and real external output receipts to a separately versioned transaction leaf / Neuro-Film |
...
| Main-integration evidence | Main `efb9ba82` merged consumer parent `f53faeb5` (P1-P172). V45 is rejected. V46 remains valid historical P173-P175 evidence but omits confirmed P176/P177 file-entry ownership fixes and is paused as the latest integration candidate | P1-P172 MERGED; V45 MUST NOT MERGE; V46 SUPERSEDED FOR LATEST REVIEW | Freeze a replacement manifest containing P174-P177 against refreshed main / consumer |
...
| Main-project availability | P176/P177 transaction-writer coverage passes 155 with three platform skips and the isolated color-match suite passes 1,429 with five skips. A refreshed overlap/merge-tree snapshot is pending; V46's prior clean merge is historical only | CORRECTED IMPLEMENTATION VERIFIED; REPLACEMENT MANIFEST PENDING | Rebuild strict payload/overlap/merge-tree evidence against refreshed main / consumer |
