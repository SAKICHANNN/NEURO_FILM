...
- **Pilot decision:** select Kodak Gold 100-5, Kodak Gold 400-5, Fuji NPH400 and Konica Super XG 100 for bounded acquisition/identifiability audits. Selection is not promotion: all process/lab/scanner/market fields remain `unknown`; ambiguous Fuji 100/400 remain `S0`; no expert is `S2`.
...
- **Verification/handoff:** JSON parses; four focused registry tests and all 120 repository tests pass; the exact BlueNeg 13-string/53-roll/491-frame cross-check passes with no mismatches; `git diff --check` passes. After a clean scoped commit, rerun the audit at the committed software identity and freeze the exact four-stock acquisition manifest before downloading pixels.
