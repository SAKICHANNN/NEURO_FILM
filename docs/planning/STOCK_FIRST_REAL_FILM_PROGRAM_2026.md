...
| ColorReference Velvia 100F recorder-source six-set lane | exact material header on every measured member | six target-set/charge IDs x five slides; shared production date 2005:05, roll/process unknown | five common recorder-device grids plus 8,640 prospective source/measurement rows | source is unknown-profile recorder RGB; no camera scene or scanner RGB; set/charge and measurement nuisance remain | free testing/development statement, no standard redistribution licence | AQ0 source/semantics pass; AQ1 may test repeated-set proxy identifiability only, no fitting yet |
