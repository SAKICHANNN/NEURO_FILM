...
| Xi Film marketplace | current official marketplace exposes eight verified analogue works from one contributor; exact target coverage is two Ektar100, one Portra400 and zero Velvia50 | one contributor/source; roll identity and independent scanner/process groups are not advertised | high-resolution positive scans delivered only after payment | public licensing intake records usage, media, territory, duration, budget and project notes, but does not explicitly grant ML fitting/training, derived-parameter or dataset rights | paid per-image licensing with manual email delivery | current capacity cannot meet three-stock, source-diversity or group-held-out gates; remain watchlist-only and make no purchase or file request until inventory and explicit ML/group terms materially change |
...
| Gelatin Labs C2PA signed scans | official lab-signed provenance can bind physical-film origin, roll number, stock, scanner and process/push-pull fields | public material exposes no downloadable target-stock scan; only a Portra 400 folder illustration is observed among the three targets | provenance fields are structurally strong, but no public independent roll/process/scanner/source groups or asset manifest exist | customers retain scan ownership; official declarations mark training, inference and data mining as not allowed | SF3.A3O zero-pixel source close; only a separately licensed exact export could reopen the route |
| FILM2PAINT / Chromatic Divide reversal targets | exact paper records Velvia 50 plus other reversal stocks on ColorChecker Digital SG and IT8 targets | controlled illumination, known target references and hyperspectral film scans are documented | paper-level target/format/exposure rows exist, but no public independent roll/process/scanner/source or confirmation identities | article copyright and thesis CC BY-NC do not establish a dataset licence; complete Basel DSpace inventory has no dataset payload | SF3.A3P zero-pixel source close; retain strong method evidence and reopen only for an independently licensed exact dataset export |
| YFCC100M three-stock/current-Portra connectivity | exact existing SF1.1 CC BY 2.0 metadata rows; no new page or pixel access | seven uploader UIDs connect Ektar 100, Velvia 50 and some Portra 400 label | strict current-formulation text retains only four qualifying UIDs; uploader is not person, roll or process identity and SF1.3B already closes identifiability | exact source report remains metadata-only; live-page and derivative rights were not revisited | SF3.A3Q fails the inherited five-author gate and closes this exact expansion before any external request |
| FilmMatch PortraMatch controlled source | official pages explicitly describe current Portra 400, Sony FX3 side-by-side charts, five rolls, about 20k colour samples, three illuminants, -5..+5 EV and a later validation roll | linear Reflecta RPS 10M scans and a separate later roll are methodologically strong, but exact roll/process/scanner/source identities and a sealed confirmation manifest are not published | no Portra-specific observation payload is public; the linked practice charts are the already-known Ektachrome/generic source | no dataset fitting/derived-parameter licence, exact manifest or checksums; the public product is a paid LUT/PowerGrade bundle | SF3.A3R zero-pixel source close; reopen only for an independently licensed exact Portra observation export with group roles |
| Luminant public Portra appearance source | 191 Portra-labelled human-captured photochemical/optically reimaged entries under site-wide CC BY 4.0; 24 hash-ranked original-JPEG HEADs total 124,845,733 bytes | one author/site; no explicit roll, process, scanner, neutral-pair or sealed-confirmation groups | original files are addressable, but SF3.A3S reads zero image bodies and zero pixels | rights and asset availability pass; source-independent stock identification fails before acquisition | retain as a future group-complete Portra input only; no fitting, stock response, candidate 3 or product claim |
| SF3.A3U Portra three-source heldout | fixed Commons Portra/Ektar development; unseen same-author current-Portra/Ultramax primary confirmation; Luminant positive-only confirmation conditionally sealed | 24 nicknick JPEGs / 9,188,831 bytes; old `Portra Pro 400` excluded; cross-split exact/dHash<=4 duplicates zero | RGB BA `.5000`, Portra recall `.9167`, wrong-stock specificity `.0833`, permutation `p=.599`; HOG BA `.6667`, primary-minus-best nuisance `-.1667` | formal primary failure closes before any Luminant page/image read | exact appearance-centroid family closed; no descriptor/threshold/sample rescue, fitting, stock response, candidate 3 or product claim |
| Noritsu Portra derived-profile source | exact MIT repository states real LS-600 validation, 27 frames/3 stocks and raw/TIFF-pair fitting; Portra profile Git blob/size are public | owner scans and native-capture evidence are explicitly private and not redistributed; profile body remains unread | no public pair payload, pair rights, manifest/checksums or independent roll/process/scanner/confirmation groups | SF3.A3V zero-pixel/body-free source close | retain only source-method identity; do not consume the NPZ as Portra truth, calibration, candidate 3 or product profile |
| Parker Day `DADDY PLEASE` Ektar project | exact parent declares Ektar 100, Canon EOS-1V, 100 models, 1,000 photographs and CC0; child index and all compact metadata are complete | 100 explicit model/content groups x10, but one author/project/camera and no roll/process/scanner identities | 24 hash-ranked child pages bind parent/AVIF identities; all 1,000 media bodies and pixels remain unread | source structure passes; source-independent stock identification, same-source cross-stock and neutral pairing fail before acquisition | SF3.A3T zero-pixel source close; model groups are not film-workflow groups and no image acquisition/fitting opens |
| Inland Aperture Portra/Ektar pages | seven exact Flickr pages: Portra 400 x4 and Ektar 100 x3, all Canon EOS1 / EOS 1 and per-work CC BY 4.0 | one author/source/camera and nearby road-trip geography; no explicit roll, process, scanner, same-scene or confirmation roles | exact page/rights manifest passes at `e2325455...bcdf6`; media HEAD/range/body and pixels remain zero | same-author/same-camera cross-stock availability passes, but independent-source and film-workflow admission fails | SF3.A3W zero-pixel source close; geographic adjacency is not pairing and no acquisition, natural-label rescue, fitting, candidate 3 or product claim opens |
| SF3.A3X autonomous authorization queue | six controlled physical sources: FilmMatch PortraMatch; Demystify physical+digital chart packs; PARVEC; FILM2PAINT/ZB; NTNU Colourlab; Royal Danish Academy / National Museum of Denmark Velvia ageing study | requests exact stock/roll/process/scanner/source/scene identities and pre-pixel development/validation/sealed-confirmation roles; the ageing study separately requests stable sample/replicate identities across 1992, freezer reference and 2024 | no message, checkout or file request sent; Demystify could supply immediate Portra/Ektar chart pairs for EUR119.80 total, but current organization-sharing and fitting rights are insufficient | asks explicitly for internal commercial R&D, bounded deterministic fitting, non-reconstructive derived-parameter use and aggregate publication rights; raw redistribution is optional | ready only for user-authorized outreach; Demystify also needs separate purchase approval, and every response must pass a new source intake before download, pixel access, fitting or stock/product claim |
| Denmark Velvia 50 natural-ageing study | exact CC BY 4.0 article and 2,916,544-byte PDF document six reversal films, colour-checker exposure, nine 96-sample sets with quadruplicates, freezer reference and 1992/2024 densitometry | Velvia 50 is exact and controlled; seven natural-ageing locations are recovered, but no public row-level development/confirmation assignment exists | official OJS/Pure expose only the article PDF; the cited Excel measurements and slide documentation are not public and figures are not data | article licence does not publish or license the underlying measurement payload; no manifest/checksums or sample identifiers | SF3.A3Y zero-pixel source close; retain as a high-value authorization candidate, never digitize figures or infer stock response |
| Hershenson-Allen fixed poster archive | official pages document 41,987 poster images / 25,809 films with corresponding 4x5 colour transparencies, fixed view cameras, vacuum table, colour card and Velvia 50 as an archive-level example | bounded graphic objects and fixed capture are materially different from natural-photo pools, but no public item is bound to Velvia 50 | queryable SQL is not a public manifest or neutral reference; no payload checksums, scanner/process groups, development roles or sealed confirmation exist | all rights reserved plus `ai-train=no,use=reference`; no fitting-compatible image/data licence | SF3.A3Z zero-media source close; do not query the database, fetch media, infer item stocks, fit, open candidate 3 or map product capability |
| 2026 multi-lab colour-film restoration comparison | same 1972 original camera negative and positive-print reference sent to six restoration laboratories; three scenes x24 frames; independent director-of-photography restoration held outside participant input | materially useful auxiliary A5 restoration/operator-uncertainty topology, but film stock and exact lab/scanner identities are unknown | official Elsevier/Crossref metadata bind the article; DataCite exact-title returns zero dataset records; no public observation payload, manifest or checksums | article is closed access; Crossref exposes TDM/STM policy entries, not observation-data fitting rights | SF3.A4A zero-media source close; never count as Velvia/Portra/Ektar evidence or candidate 3, and reopen only if exact licensed media, groups and sealed roles are published |
| Live Flickr explicit-current all-three supplement | one new CC BY 2.0 author outside SF3.A3Q exposes 47 Ektar 100, 29 explicit `New Portra 400` / `Portra 400 (new)` and one Velvia 50 metadata row | strict current-Portra all-three author connectivity rises from 4 to the inherited minimum 5 | complete 3,285-row public metadata inventory and target manifest SHA `44c1aea2...c5ef2`; image URLs, bodies, EXIF and pixels remain unread | sole Velvia row explicitly says `desaturated`; zero unaltered Velvia support and no explicit cross-stock camera/roll/process/scanner/sealed-confirmation groups | SF3.A4B closes pixel admission while retaining the connectivity correction; no natural-label rescue, acquisition, fitting, candidate 3 or product claim |
...
| Color Precision comparison source | 20 URL-derived weak labels plus 12 chart stock/process labels; not verified physical IDs | official Charts download has 24 fixed-setup pages and paired Frontier/Noritsu views; roll/process truth and image-to-EV assignment unknown | U5.R2BD1 internally audits 191 unique full-photo PDF objects as 24 unordered page bags | scanner interpretation, exposure mixture and missing embedded-profile semantics remain nuisance | all-rights-reserved; no training/redistribution grant | raw/basic-normalized cross-scanner top-1 20.83%/33.33%, both below frozen gates; close stock learning and retain nuisance stress only |
