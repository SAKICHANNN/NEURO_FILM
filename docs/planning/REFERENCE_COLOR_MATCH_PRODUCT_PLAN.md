...
| P43A | DONE | audit fixed producer invocation package without executing mutable checkout | pin producer/package/schema/fixture/wheel/runtime identities; reject stale same-name wheels | `2fb1aa9` | no adapter |
...
| P44A | DONE | freeze exact-invocation A1/A4/A5 protocol | existing six Velvia known-operator sources/targets, unchanged promotion thresholds, no result-conditioned tuning | `6c825a9` | retain P43 |
...
| P46A | DONE | freeze P44 failure-signature analysis | consume existing progress only; no rerender, target access, tuning or algorithm implementation | `f3d6e90` | retain P44 report |
...
| P48A | DONE | freeze shared-apply numeric facts and atomic guard | reuse P29 thresholds; bind producer diagnostics plus independently measured output range/boundary | `05aaa84` | retain P47 candidate-only |
...
| P49A | DONE | freeze shared-path staging authorization | require independent P45 admission, P47/P48 exact binding and a consumer promotion binding for the same capability/model/options/evidence | `e456eee` | no applied/delivery claim |
...
| P55A | DONE | freeze shared local-delivery authorization | live rerun exact P54 and bind P52/P51/P50/P49/P48/P47 chain; emit no-write local scope only | `fb65d07` | no destination |
| P55B | DONE | implement canonical shared delivery authorization | require P49 product-ready state and all ordered sources authorized; bind shared reference+FilmFX output label | `8fcc59e`; code/schema/tests | authorization only |
| P55C | DONE | prove foreign-chain/tamper/claim closure | live output tamper, valid foreign chain member, state/scope/output-label/claim/identity mutation reject | 11 dedicated tests | fail closed |
| P55D | DONE | propagate evidence and peer handoff | 571 color-match; 1466 pass/1 skip/36 known; latest-main merge 36 pass | evidence commit | real use still P45-gated |
| P56A | DONE | freeze shared atomic local export | reconstruct exact P55 immediately before mutation, protect P50/P53/P54 artifacts, copy exact P53 bytes | `47ae622` | local only |
...
| P57A | DONE | freeze shared local-delivery restart verification | caller binds exact P56 report SHA/delivery ID; bounded reread and rehash every P53 staging source plus delivered file | `7be00cd` | read-only |
...
| P58A | DONE | freeze main-integration evidence bundle | pin consumer payload/base/main commits, changed-path Git blobs, zero-overlap set and exact verification commands | `f8a2a63` | no main write |
...
| P63A | DONE | freeze handle-based runtime staging restart observation | caller pins exact P62 report SHA/run/qualification; report and outputs are each opened once and the same handles are identity-bound, double-read and finally rehashed | `46b6ea8`; contract/docs | observation only; no path consumption |
| P63B | DONE | implement canonical P62 handle verifier | reject missing/foreign report, report/output substitution, relocation, post-open mutation, duplicate identities and non-regular/reparse/device/ADS paths | `46b6ea8`; code/schema/tests | no cleanup or delivery |
| P63C | DONE | prove handle/race/resource closure within declared OS scopes | adversarial rename/replace, same-tick overwrite, hardlink/duplicate identity, nonregular/reparse inputs, count/size budgets and cleanup faults either reject or bind the opened object | 87 adjacent pass/3 privilege skips | Windows denies write/delete sharing; POSIX remains sequential observation |
| P63D | DONE | propagate evidence and reserve new integration manifest | 707 all-color; 1602 pass/4 skips/36 known; latest-main merge 180 pass/3 skips | evidence commit | P63 record alone never authorizes later path reopen |
| P64A | DONE | freeze additive immutable integration manifest v2 | bind exact P1-P63 reviewed payload while retaining P58 v1 as immutable prior evidence | `d6e562a` | review only |
...
| P71A | DONE | freeze path-free encoded colour-metadata attestation | exact P62 PNG/JPEG/TIFF embedded sRGB ICC bytes, orientation absence and no-conflict policy are bound to each P65/P67 output | `01496ea`; code/schema/tests | process-local evidence only |
| P71B | DONE | implement exact ICC/tag inspector | bounded PNG iCCP, full-scan JPEG APP2 reconstruction and unique TIFF 34675 inspection bind exact consumer sRGB profile bytes | `01496ea` | no profile conversion |
| P71C | DONE | prove metadata parser/resource/conflict closure | duplicate/late/oversized/trailing profiles, alternate APP/XMP/text/TIFF colour tags, decompression budget, JSON ambiguity and mutable receipt graphs reject | 48 focused; 150 chain/1 skip; adversarial review clear | fail closed |
| P72A | DONE | publish attestation-consuming MatchView bridge v2 | accepts only exact P71-attested batches, reruns P71 and frozen EOTF, and binds metadata IDs/profile hash into descriptor provenance | `3126657`; 72 focused | P69 v1 remains immutable and assumption-bound |
| P73A | DONE | publish integration manifest v6 | bind P1-P72 `3126657`, main `2f9a7c0`, 305 payload paths, 45 exports, 19 schemas, zero overlap and exact v5 identity | `7eb4749` | superseded by checkout-safe v7 |
