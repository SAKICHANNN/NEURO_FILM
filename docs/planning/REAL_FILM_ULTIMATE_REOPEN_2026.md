...
| Color Precision comparison source | 20 URL-derived weak labels plus a separately frozen official Charts download; physical truth unverified | Charts has 24 fixed-setup pages/12 stock-process variants across Frontier and Noritsu; exposure identities are unassigned | SF2.10R metadata and U5.R2BD0/BD1 source audits remain rights-limited. BD1 reaches only 20.83% raw/33.33% normalized symmetric cross-scanner top-1 and closes stock learning; retain scanner-nuisance stress only |
...
C1 config `7a6d4924...44ef6` freezes the exact universe, native-grid
...
The formal two-process execution is byte-exact and independently
reconstructed, but closes with zero survivors: all 194 candidates pass
style/non-basic evidence while all 194 fail minimum Jacobian determinant; 175
also exceed the negative-Jacobian-fraction gate. No photograph was rendered
and the frozen branch forbids threshold, smoothing, projection or teacher
rescue. This establishes a stylish-but-structurally-unsafe external-bank
negative result, not stock truth or a general impossibility result. Aesthetic
