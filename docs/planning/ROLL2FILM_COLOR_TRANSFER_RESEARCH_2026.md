...
**Status:** primary publication-oriented research authority as of 2026-07-12

**Paper type:** colour-transfer method, not a benchmark paper

**Current execution state:** data-independent formulation and falsification work only

**Download/training state:** no dataset download, GPU training, participant study or external contact is authorized by this document
...
### 2.1 Local audit, 2026-07-12

| Lane | Physical state | Scientific state | Valid use now |
|---|---|---|---|
| `data/film_domain` | 3,896 JPEGs, about 8.355 GiB; 3,786 images across eight named directories plus 110 `_vintage_generic` images | **quarantined**. The legacy audit covered 4,210 manifest rows and found 0 eligible rows, no durable source/uploader/roll/scanner grouping, unknown Flickr-use scope and 140 near-duplicate pairs. The current files also mix hundreds of embedded/absent ICC profiles. | internal failure exploration only; not paper training, released weights or stock truth |
...
| FiveK source and `freeze_v1` | absent on this Mac | historical Windows records are not current files; FiveK would be neutral retouching, not film truth, even if restored | none until restored; then generic operator/canonicalisation auxiliary only |
...
|---|---|---|---|
| FilmSet | **remote-verified** by official file listing and actual member download | 5,285 digital originals + three paired film-recipe targets; hidden-pair transfer evaluation | primary Level-A corpus |
...
Velvia styles, with 4,657 training samples and 638 test samples. The targets
...
This is **remote-verified**, not merely a literature link: the official Kaggle
CLI listed concrete archive members without credentials, and a 412KB member
(`FilmSet/test/Cinema/DSCF7071.png`) was actually fetched to temporary storage
on 2026-07-12 with SHA-256
`7c4b1b8f716448941135eef5dc7d4f9d0a0575d0a5d859ca33a7b3e1745fc9d2`.
The full 11.26GB archive was deliberately not downloaded in this documentation
leaf.
...
- keep all 638 aligned test pairs hidden for transfer evaluation;
...
| film-recipe algorithm paper | FilmSet training targets used unpaired; 638 aligned pairs hidden for final evaluation |
...
4. Keep the 638 paired test targets inaccessible until the method and
...
inferred operator to all 638 hidden digital originals, together with matched
...
|---|---|---|---|
| U5.CT0 | complete by this document | corrected problem, current data audit, nearest-work boundary and claim ladder | authority docs agree that the primary paper is colour transfer |
| U5.CT1 | ready, no data | invertible operator contract and known-operator simulator | property tests plus group-size recovery curves |
| U5.CT2 | ready, no images | FilmSet/BlueNeg manifest adapters, license snapshots, group/split schema | metadata hashes and fail-closed eligibility report |
| U5.CT3 | pending on CT1 | synthetic identifiability and nuisance audit | G1 pass/fail decision |
| U5.CT4 | approval-gated download | FilmSet paired-blind corpus and frozen split | archive/license snapshot, hashes, pair-blinding tests |
| U5.CT5 | pending on CT3/CT4 | unpaired pseudo-roll baselines and Roll2Film solver | hidden-pair protocol frozen before evaluation |
| U5.CT6 | approval-gated download | BlueNeg preview roll pilot | correct-roll matched-control result and G2 decision |
| U5.CT7 | pending on CT5/CT6 | amortised set inference and complete ablation | simplest survivor; no kitchen-sink additions |
| U5.CT8 | participant-gated | hidden transfer and preference study | G3-G6 decision with confidence intervals |
