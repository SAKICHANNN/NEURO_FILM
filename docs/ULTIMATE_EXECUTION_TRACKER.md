...
| U5.R2S3D | ready: unpaired explicit-flow pilot frozen | Fit safe flow directly by independent distribution matching and separate fit from operator recovery | U5.R2S2D + U5.R2O0 | eight styles × two independent observation sets; fixed SW-24 and RFF-MMD-192, 200 steps, local deterministic GPU allowed. Distribution improvement and oracle/replicate/structure/repeat gates must jointly pass; seed 28203 untouched; `docs/planning/U5_R2S3_UNPAIRED_DISTRIBUTION_OPERATOR_PILOT_CONTRACT.md` |
| U5.R2S4D | pending on U5.R2S3 repeat closure | Test one shared explicit flow across correctly corresponding synthetic content-condition distributions | U5.R2S3 repeated `distribution_fail` only | compare pooled, correct-condition and fixed shuffled-condition RFF-MMD on identical generated observations; generated condition truth only, never inferred RGB/CLIP/source labels. This is ICLR-2024-inspired, not theorem reproduction; real pixels require a separate auxiliary-condition DoR; `docs/planning/U5_R2S4_DIVERSIFIED_DISTRIBUTION_OPERATOR_DEVELOPMENT_CONTRACT.md` |
