...
| SF1.1 | in progress: full-YFCC metadata-only shared-author gate | Acquire/filter public 65,644,027,904-byte YFCC100M SQLite for exact stocks and shared authors | SF1.0B | no pixels; SHA-256 plus 7,826-part S3 ETag verification; exclude single-photo multi-stock mentions; >=100 exclusive rows/30 UIDs each and >=12 shared Ektar/Velvia UIDs before live-rights preflight; otherwise close |
...
| ID | Status | Deliverable | Dependencies | Exit evidence |
|---|---|---|---|---|
| U1.1 | in progress | `render_film` consumes `WorkingImage` | U0.4 | JPEG/PNG/TIFF script E2Es pass; standard-sRGB and unprofiled PNG/TIFF16 preserve uint16 ingress, while unsupported 16-bit ICC fails closed. RAW E2E and removal of the explicit legacy sRGB8 adapter remain pending. |
