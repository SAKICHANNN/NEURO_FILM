...
| U1.6 | in progress: staged-grain child ready | Halo-aware tile/cache renderer | U1.3 | U1.6A-D prove execution, safe-Lab, simple-halation and sparse dust parity; U1.6E closes on order identifiability; U1.6F freezes exact disk-staged grain; physical halation, orchestration, streaming/cache and bounded total memory remain pending |
...
| U1.6F | frozen: ready | Exact legacy grain with bounded-RAM temporary-disk staging | U1.6A/U1.6E | chunked PCG64, radius-4 high-pass, legacy-order global reductions, byte parity, scratch budget/cleanup and full CPU suite; no new grain/default/performance claim; `docs/planning/U1_6F_GRAIN_STAGED_CONTEXT_CONTRACT.md` |
