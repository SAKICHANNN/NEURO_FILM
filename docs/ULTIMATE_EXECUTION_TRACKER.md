...
| U5.R2Z1 | implemented/frozen: formal run ready | Test whether asymmetric query/operator applicability can recover the ClassNeg evaluator gap without raw-content similarity | Z0 `case_bank_oracle_only` + failed global/spatial photometric selectors | isolated scalar-loss reranker, leakage-safe rank ladder, 32 fold-local shuffles, OOD fallback and fresh-target lock implemented; 989 CPU tests pass. Fresh identities 17--32 open only after automatic development pass; no neural/router/visual/final-628/film claim; `docs/planning/U5_R2Z1_ASYMMETRIC_APPLICABILITY_CONTRACT.md` |
