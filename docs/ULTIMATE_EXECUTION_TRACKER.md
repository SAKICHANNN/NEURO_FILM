...
| U5.R2H0C1 | active: contract frozen / acquisition | CAVE integrity and conditional-support feasibility | U5.R2H0C | Official ZIP central directory is hash-pinned; transfer bytes must match official path/size/CRC. Fixed scene-grid sampling and radius-1 cross-scene pair gates are frozen before witness output; `docs/U5_R2H0C1_CAVE_CONDITIONAL_VARIABILITY_CONTRACT.md` |
