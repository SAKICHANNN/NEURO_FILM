...
| U5.R2W2F0 | ready: paired/global explainability only | Determine whether three local FilmSet recipe domains are coherent bounded global operators | repeated paired-only U5.R2W1 + frozen FilmSet source/target/internal-dev manifests | commit-bound preflight repeats at `AB435E24...`, freezing 256/128/81 internal dev/confirm/stress IDs with zero leakage/pixel access. Freeze aligned-pair budget, then compare identity/basic/shared/per-pair O0 and spatial residual. W2F1 output-only remains closed; final 628 forbidden; `docs/planning/U5_R2W2F_FILMSET_REFERENCE_LOOK_BRIDGE.md` |
