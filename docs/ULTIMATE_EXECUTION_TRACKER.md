...
| SF2.5R | in progress: metadata-only contract frozen | PROV VPRS 17684 digitised negatives ↔ VPRS 17690 film-stock register feasibility | official public API + catalogue descriptions | two normalized API passes must establish whether register contents are digitally accessible; no image/TIFF/VEO request, fitting, training or LSM; `docs/planning/SF2_5R_PROV_NEGATIVE_REGISTER_RECONNAISSANCE_CONTRACT.md` |
...
| U5.R2B | in progress: contract frozen | Strong global operator frontier | U5.R2A, U5.R1 | Seven fixed same-renderer global policies on 9 gold + 32 stress A0/B0-development inputs; automatic style/non-basic/clipping then complete survivor visual veto; no fitting/routing/integration; `docs/planning/U5_R2B_GLOBAL_OPERATOR_FRONTIER_CONTRACT.md` |
