...
| U5.R2AM0 | ready: source/method audit | Audit image-adaptive invertible coordinate systems as a distinct explicit colour representation | AL1 close + need for analytic rather than iterative inverse | determine official source/code/licence/reproducibility facts and isolate only a clean-room bounded globally invertible coordinate-transform plus monotone-curve hypothesis; paired retouching evidence is not film evidence, content adaptation is not operator identification, and no photo/film fitting opens |
