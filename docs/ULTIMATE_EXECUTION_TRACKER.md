...
| U5.R2Y0 | implemented/frozen: formal source/probe repeat ready | Audit whether NegClone 0.2.0 stock fingerprints isolate stock rather than scene colour, exposure and texture | X0 close + exact MIT PyPI sdist | exact source hashes plus no-film red/blue, dark/bright and flat/checker counterfactuals; no network, community pixels or preset generation. Retain only as a negative control if shortcuts reproduce; `docs/planning/U5_R2Y0_NEGCLONE_CONTENT_SHORTCUT_CONTRACT.md` |
