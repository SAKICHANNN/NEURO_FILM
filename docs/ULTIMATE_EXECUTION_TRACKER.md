...
| SF2.3 | contract frozen; implementation ready | Union the three immutable Commons snapshots into a conservative exact-stock/shared-author connectivity graph | SF0.4/SF0.6A/SF0.6B snapshots + SF1.3B negative evidence | metadata only; two-author edges, cycle/redundancy and cross-stock identity gates frozen; no network/pixels/fitting/training/LSM; `docs/planning/SF2_3_COMMONS_UNION_SHARED_AUTHOR_CONNECTIVITY_CONTRACT.md` |
...
| U1.6 | in progress: global-field primitive or orchestration next | Halo-aware tile/cache renderer | U1.3 | U1.6A-D/F pass and U1.6E closes; U1.6G0 proves physical/density halation needs percentile, shape-stable global resample and scratch-DAG prerequisites; orchestration, streaming/cache and bounded total memory remain pending |
