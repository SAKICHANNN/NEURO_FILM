...
| SF2.10R | implemented/frozen: formal metadata repeat ready | Audit Color Precision comparison topology, scanner nuisance support and pixel-rights DoR | SF2.9R limitation + eight retained public HTML responses | offline parser only; 927 embedded image URLs are inventory metadata and must not be requested. Filename stock/process/scanner labels remain weak evidence; pixel access requires explicit rights, replication and pairing DoR; `docs/planning/SF2_10R_COLOR_PRECISION_METADATA_PREFLIGHT.md` |
