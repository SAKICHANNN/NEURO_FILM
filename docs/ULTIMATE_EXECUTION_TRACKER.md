...
| U5.R2L1 | ready: contract frozen | Deterministic full-frame clipping preflight for hard s0.65/s0.50 selection | U5.R2L0 | regenerate both immutable strengths byte-exactly; full-frame endpoint rule must reproduce all 41 Oracle assignments and selected bytes with no retuning; no ML/blend/production claim; `docs/planning/U5_R2L1_FULL_FRAME_STRENGTH_PREFLIGHT_CONTRACT.md` |
