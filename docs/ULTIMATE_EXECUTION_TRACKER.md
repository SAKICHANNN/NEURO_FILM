...
| SF1.1 | in progress: full-YFCC metadata-only shared-author gate | Acquire/filter public 65,644,027,904-byte YFCC100M SQLite for exact stocks and shared authors | SF1.0B | no pixels; SHA-bound source manifest; exclude single-photo multi-stock mentions; >=100 exclusive rows/30 UIDs each and >=12 shared Ektar/Velvia UIDs before live-rights preflight; otherwise close |
