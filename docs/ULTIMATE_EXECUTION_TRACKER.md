...
| U5.R2Z1 | ready: contract not frozen | Test whether asymmetric query/operator applicability can recover the ClassNeg evaluator gap without raw-content similarity | Z0 `case_bank_oracle_only` + failed global/spatial photometric selectors | freeze development-only leave-one-case/group controls, simplest hard reranker, shared/random/shuffled/photometric baselines, confirmatory lock and no-router/no-visual branches before fitting. This remains paired software-recipe control, not film |
