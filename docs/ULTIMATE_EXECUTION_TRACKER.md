...
| U5.R2L0 | ready: contract frozen | Post-E1 per-image hard strength Oracle for the retained density-cyan operator | U5.R2E1 | hash-pinned `s0.50` baseline versus `s0.65` challenger; choose `s0.65` only under the inherited per-image clipping ceiling, then require a material style-gain Oracle and all-41 full-resolution severe veto; evaluator-only, no deployable router claim; `docs/planning/U5_R2L0_DENSITY_STRENGTH_ORACLE_CONTRACT.md` |
