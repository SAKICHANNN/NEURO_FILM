...
| U5.R2AN1 | ready: robustness contract frozen | Test paired recovery under dual patch-mean noise and sparse corrupted correspondences | AN0 exact recovery pass | new confirmatory perturbation seeds; robust two-matrix versus linear two-matrix and robust one-matrix. Must remain below `.001` clean-confirmation RMSE and win the outlier/control gates; synthetic only |
