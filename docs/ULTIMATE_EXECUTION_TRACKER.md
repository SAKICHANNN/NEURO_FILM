...
| U5.R2H0C2 | next ready: freeze comparison | Group-held-out simple measured-spectrum canonicalizer comparison | U5.R2H0C1 | Compare smooth reconstruction against cross-scene hard measured-spectrum retrieval on known CAVE witness targets; explicit deterministic output, no film pixels/neural model/stock claim, external source replication still required |
