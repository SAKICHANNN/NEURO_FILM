...
| U5.R2AN3 | ready | Test one fixed redescending soft-weight paired fitter without hard row deletion | AN2 mechanism close + AN1 fixed soft-L1 result | use the same synthetic witnesses, perturbations, scale and unchanged AN1 absolute/relative gates; one preregistered Cauchy influence candidate, no loss/scale sweep and no proxy/photo access before pass |
