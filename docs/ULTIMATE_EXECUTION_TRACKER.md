...
| U1.6 | in progress: dust child ready | Halo-aware tile/cache renderer | U1.3 | U1.6A execution, U1.6B safe-Lab and U1.6C simple-halation are numerically proven; U1.6D sparse dust contract is frozen; physical/grain effects, integration, streaming/cache and bounded total memory remain pending |
...
| U1.6D | frozen: ready | Exact sparse-event dust/scratch tiled composite | U1.6A/U1.6B | compact event context, legacy layer and tiled float/sRGB8 byte parity, active real-raster smoke, fail-closed validation and full CPU suite; `docs/planning/U1_6D_SPARSE_DUST_TILING_CONTRACT.md` |
