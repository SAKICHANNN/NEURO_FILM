...
| U5.R2X0 | implemented/frozen: formal analytic repeat ready | Determine whether the 2025 automatic affine-barycentric palette-transfer path is more than one affine RGB operator | W2F0 close + official JCST equations | clean-room theorem implementation freezes direct-vs-affine equivalence plus valid negative-determinant and cube-range witnesses. No image, CPLEX, film or local-path claim; `docs/planning/U5_R2X0_PALETTE_TRANSFER_AFFINE_COLLAPSE_CONTRACT.md` |
