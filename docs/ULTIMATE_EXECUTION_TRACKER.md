...
| SF2.1A | frozen/ready | Openverse exact-stock strict-licence shared-creator metadata graph | SF2.0C0 close + bounded official API reconnaissance | four quoted colour-stock queries, 48 metadata requests max, no thumbnails/landing pages/pixels; pass opens only separately frozen live-rights/label preflight |
