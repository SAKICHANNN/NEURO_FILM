...
| U5.R2W2F | source/partition preflight repeat-pass; execution queued behind W1 | Transfer a surviving output-only mechanism to three local FilmSet recipe domains | U5.R2W1 + frozen FilmSet source/target/internal-dev manifests | commit-bound metadata-only preflight repeats at `AB435E24...`, freezing 512 target refs, 256 source probes and 256/128/81 internal dev/confirm/stress IDs with zero pool leakage and zero pixel access. Global-explainability precedes CFSM; final 628 is forbidden. Capture One mechanism evidence only; `docs/planning/U5_R2W2F_FILMSET_REFERENCE_LOOK_BRIDGE.md` |
