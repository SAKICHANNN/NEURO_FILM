...
| SF1.2 | ready: page-only rights feasibility | Verify at most 128 bounded Flickr pages for live CC BY support across the 16 Ektar/Velvia shared UIDs | SF1.1 pass | at least five authors with a live page for each stock; no image request, pixel scope, training or operator fitting |
...
| LSM1 | data-gated: no eligible stock | Per-stock support/connectivity/rights/identifiability feasibility matrix | a stock's RF/SF evidence | SF1.1 metadata connectivity passes one edge, but live rights, pixels and stock identifiability still fail DoR; no training/clustering |
