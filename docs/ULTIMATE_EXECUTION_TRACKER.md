...
| U5.R2AK1 | ready: frozen synthetic capacity contract / implementation | Compare a 243-parameter quadratic-time cube flow against stationary 192/375-parameter controls on two noncommuting staged reactions | AK0 prior-only + O0 representation | config freezes analytic A→B/B→A truths, equal split/optimizer, absolute/relative fidelity, cube/Jacobian/inverse/replay gates and no-photo boundary before fitting; `docs/planning/U5_R2AK1_TIME_DEPENDENT_CUBE_FLOW_CAPACITY_CONTRACT.md` |
