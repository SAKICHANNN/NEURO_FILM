...
| U1.6G3 | complete: static-plan pass / integration closed | Physical/density field DAG, blur classification and lifetime/resource plan | U1.6G0/G1/G2 | AST-backed 41/31-node graphs preserve 8/2 and 6/1 inventories, transitive lazy-stream lifetimes and separate 24MP/100MP resource categories; gradient windows pass, but G4B closes exact G1-v1 row batching and integration remains false pending a separately versioned deterministic reducer; `docs/U1_6G3_HALATION_FIELD_DAG_LIFETIME_RESULTS.md` |
...
| U1.6G4C | contract frozen / implementation ready | `shape-stable-global-resample-v2-explicit-f32` chunk-invariant area reducer | U1.6G4B | explicit increasing-index float32 reduction plus exact terminal bounds shared by full/row builders; new-seed cross-chunk bytes, two fixed real-luma compatibility cases and severe-field review are frozen; G1-v1 remains unchanged; `docs/planning/U1_6G4C_CHUNK_INVARIANT_GLOBAL_RESAMPLE_CONTRACT.md` |
