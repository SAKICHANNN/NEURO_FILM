...
| U5.R2AD1 | in progress: contract frozen | Test spatial DIR inhibitor diffusion against the exact RF2.C0 non-spatial control | AD0 external-ablation pass | exact source/runtime/gold inputs, external defaults, RF2.C0 replay hashes, matched Gaussian local-contrast nuisance and style/artifact gates frozen before rendering; no search or post-result rescue; `docs/U5_R2AD1_SPEKTRAFILM_SPATIAL_DIR_ABLATION_CONTRACT.md` |
