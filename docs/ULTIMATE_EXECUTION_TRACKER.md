...
| SF0.3 | complete | Decode integrity, duplicate/border/content and stock×roll×proxy support audits | SF0.2 | 189/189 RGB PNG decode; zero exact dups; zero cross-frame dHash≤4; Gold display-candidate; others density-only; report SHA-256 `0e09c901...`; vision pending; `docs/REAL_FILM_STOCK_PILOT_INTEGRITY_RESULTS.md` |
...
| RF1.4 | ready | Per-stock label/content/nuisance identifiability and leakage-safe holdouts | SF0.3 | correct stock must beat pooled, wrong-stock, generic historical, retrieval and nuisance controls; density and display domains reported separately |
