...
| U5.R2AJ0C0 | ready: frozen controls-only implementation | Calibrate and conform the Hald structural screen without reading an external CLUT body | AJ0B2 pass | exact parent/config, red-fastest encoded-sRGB trilinear semantics, analytic Jacobian, 25 gradients, RGB16 reader conformance, basic/non-basic/duplicate/strength/negative controls and two-process canonical repeat are frozen; archive bytes and primary metrics must remain exactly zero; `docs/planning/U5_R2AJ0C0_HALD_STRUCTURAL_CONTROL_CALIBRATION_CONTRACT.md` |
| U5.R2AJ0C1 | blocked on C0 pass and separate contract commit | Screen the 194 licensed external Color Halds individually on synthetic structural, matched-basic and novelty evidence | exact C0 pass | use native N=144/256 streaming, per-candidate vetoes, strength/basic duplicate collapse and deterministic cap-12 policy; no photograph/aesthetic review |
