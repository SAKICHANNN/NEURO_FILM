...
| U5.R1B | ready for bounded design | Stress generator plus real cross-algorithm failure suite | U5.R1A | Parent-scene grouping, no final-test leakage, current known failures represented; no participant/model action without later gates |
| GH0 | complete | Cursor Goal harness: rule, skill, state, stop hook, protocol, tests | none | `docs/drpt/CURSOR_GOAL_PROTOCOL.md`; 379 CPU tests; Goal state ACTIVE |
