...
| SF1.0A | ready: YFCC same-source support | Search the frozen YFCC15M metadata for Ektar100 and UltraMax400 under exact CC-BY text/UID gates | SF0.8B | pre-registered patterns; repeat-identical local scan; determines whether three-stock YFCC source matching is possible; no pixels/operator fitting |
| SF1.0B | data-gated: three-stock identifiability | Falsify Ektar100/UltraMax400/Velvia50 stock signal against source/content/colour shortcuts | SF1.0A | frozen support matrix; author-group holdout; shuffled, grayscale, source, content, low-frequency colour and matched saturation/contrast controls; no operator fitting |
