...
| U1.5 | in progress: structured rejection hardening | HEIF/HDR/gain-map detect/preserve or explicit rejection | U1.2 | U1.5A rejects unsupported containers/signals; U1.5B is frozen to close the first/last-4-MiB known-marker blind spot without claiming complete ISO 21496-1 detection or format support |
...
| U1.5B | frozen: ready | Structurally traverse JPEG APP and PNG text metadata for recognized gain-map signals outside the U1.5A edge sample | U1.5A | bounded parser/decompression, middle-of-file witnesses, ordinary-large negative controls and full regressions required; detection only, no HDR/ISO support claim |
