...
| U5.R2S4D | ready: S3 repeated activation passes | Test one shared explicit flow across correctly corresponding synthetic content-condition distributions | U5.R2S3 repeated `distribution_fail` | compare pooled, correct-condition and fixed shuffled-condition RFF-MMD on identical generated observations; generated condition truth only, never inferred RGB/CLIP/source labels. This is ICLR-2024-inspired, not theorem reproduction; real pixels require a separate auxiliary-condition DoR; `docs/planning/U5_R2S4_DIVERSIFIED_DISTRIBUTION_OPERATOR_DEVELOPMENT_CONTRACT.md` |
...
| U5.R2U1D | implemented: execution pending repeated S4 adjudication | Test clean-room HCC pseudo-pairs while fitting only the bounded O0 flow | U5.R2U0 + repeated eligible U5.R2S4 branch | isolated HCC/pair-fit module, runner activation gate and seven tests pass; new seeds; random-correct, pooled-HCC, correct-condition-HCC and shuffled-condition-HCC controls. Pair fit is diagnostic; hidden operator, A/B stability, structural safety and exact repeat jointly gate. No real pixels or identified-pair claim; `docs/planning/U5_R2U1_HIERARCHICAL_COLOUR_COUPLING_DEVELOPMENT_CONTRACT.md` |
...
| U5.R2W1D | implemented: execution queued behind active S4/U1 | Test output-only single/multi-reference look recovery across unrelated generated content | U5.R2W0 + U5.R2O0 | fixed descriptor/bank, hard direction plus continuous-strength retriever, separate single/multi ridge, activation/partition-gated runner and twelve tests pass including exact tiny repeat; 32 development directions x four strengths, held-out contents/nuisances, `53/55/56` same-input strength-path and paired-upper-bound controls remain formally untouched. No neural encoder, images, film or stock claim; `docs/planning/U5_R2W1_REFERENCE_LOOK_IDENTIFIABILITY_DEVELOPMENT_CONTRACT.md` |
| U5.R2W2F | implementation-stage preflight repeat-pass; commit-bound rerun pending | Transfer a surviving output-only mechanism to three local FilmSet recipe domains | U5.R2W1 + frozen FilmSet source/target/internal-dev manifests | metadata-only preflight freezes 512 target refs, 256 source probes and 256/128/81 internal dev/confirm/stress IDs with zero pool leakage and zero pixel access; rerun on its scoped commit before closure. Global-explainability precedes CFSM; final 628 is forbidden. Capture One mechanism evidence only; `docs/planning/U5_R2W2F_FILMSET_REFERENCE_LOOK_BRIDGE.md` |
