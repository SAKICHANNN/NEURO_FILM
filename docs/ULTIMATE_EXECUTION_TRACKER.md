...
| U1.6G3 | complete: static-plan pass / integration closed | Physical/density field DAG, blur classification and lifetime/resource plan | U1.6G0/G1/G2 | AST-backed 41/31-node graphs preserve 8/2 and 6/1 inventories, transitive lazy-stream lifetimes and separate 24MP/100MP resource categories; 27 focused/458 full tests pass; integration remains false pending gradient-window and row-chunked-stage capabilities; `docs/U1_6G3_HALATION_FIELD_DAG_LIFETIME_RESULTS.md` |
| U1.6G4A | contract frozen / implementation ready | Coordinate-exact scalar gradient windows | U1.6G3 | one-pixel original-coordinate expansion must reproduce current full-frame float32 `np.gradient` bytes at interiors/edges/corners without a full gradient field; row-chunked global staging remains independently missing; `docs/planning/U1_6G4A_COORDINATE_EXACT_GRADIENT_WINDOW_CONTRACT.md` |
