...
| U5.CT1 | in progress; affine v1 foundation complete | Versioned invertible explicit-operator contract plus known-operator pseudo-roll simulator | U1/U2 interfaces; no external data | `roll2film.affine.v1`, exact inverse/identity/serialization/determinism tests and pseudo-roll simulator pass; monotone curve/LUT capacity remains before full CT1 closure. Evidence: `docs/ROLL2FILM_CT1_E0_FOUNDATION.md` |
| U5.CT2 | in progress; contract frozen | FilmSet/BlueNeg eligibility, grouping, licence snapshot and pair-blinding contract | U0.3; FilmSet local, BlueNeg metadata-only | Contract: `docs/data/ROLL2FILM_FILMSET_MANIFEST_CONTRACT.md`; next require fail-closed generated manifests, hashes, zero exact/perceptual/embedding leakage and tests proving training cannot access hidden pairs |
| U5.CT3 | in progress; fixed-budget affine controls pass | Fixed-total-sample group, coverage, partition, nuisance, prior-swap and scanner identifiability study | CT1 | Exact partition equivalence; true exposure boundaries, independent support and correct single-operator groups beat paired controls at fixed 4,096 pixels. Prior shift and scanner composition expose expected confounding. `roll_information=not_established`; L2 truth, stronger factorials and matched real-roll controls remain. Evidence: `docs/ROLL2FILM_E0_FIXED_BUDGET_V2.md`. |
| U5.CT4 | in progress; no download required | Freeze local FilmSet archive and 4,657-internal/628-final paired-blind corpus | CT2 | Manifest/lockbox contract frozen; next require archive hash/count/rights caveat, content-disjoint internal pools and an official 628 evaluator inaccessible to training until full policy freeze |
| U5.CT5 | pending on CT2/CT3/CT4 | Matched-strength deterministic/classical baselines plus Roll2Film solver on FilmSet internal dev | CT2/CT3/CT4 | Best-basic/style floor frozen; method beats strongest run unpaired baseline on internal hidden pairs; official 628 remains sealed |
| U5.CT6 | scientifically gated; download pre-authorized 2026-07-15 | BlueNeg 8-bit preview/pseudo-GT correct-roll matched-control pilot | CT2/CT3 pass | Owner pre-authorized necessary data acquisition; still require metadata/licence/whole-roll split and storage/retention record before download; correct roll must beat random, same-film wrong-roll, date/location/scene-matched and shuffled-label controls |
| U5.CT7 | blocked on CT5/CT6 and MAP gate | Optional permutation-invariant amortised set inference and complete ablation | MAP known-truth + CT5/CT6 pass | Simplest survivor; no gain from shortcut, kitchen-sink or unsupported spatial branch |
| U5.CT8 | blocked until complete policy freeze | Final 628 transfer plus style/artifact evaluation | CT5–CT7 decisions frozen + participant approval for human claims | Official 628 opened once; actual transformed outputs pass target/style/artifact endpoints with uncertainty, coverage and failure gallery; no post-hoc retuning |
