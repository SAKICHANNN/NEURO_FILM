...
| U5.R2U1D | ready: repeated eligible S4 branch | Test clean-room HCC pseudo-pairs while fitting only the bounded O0 flow | U5.R2U0 + repeated eligible U5.R2S4 branch | isolated HCC/pair-fit module and activation gate pass; new seeds; random-correct, pooled-HCC, correct-condition-HCC and shuffled-condition-HCC controls. Pair fit is diagnostic; hidden operator, A/B stability, structural safety and exact repeat jointly gate. No real pixels or identified-pair claim; `docs/planning/U5_R2U1_HIERARCHICAL_COLOUR_COUPLING_DEVELOPMENT_CONTRACT.md` |
