...
| U5.R2AQ0 | ready | Audit the public ColorReference Velvia 100F recorder-source TIFF and measured slide reference files as a controlled paired-measurement candidate | AN robust-family close + FILM2PAINT data block | exact URLs/bytes, rights, recorder-device input semantics, film batch/process, measurement/scanner representation, pair keys and independent groups first; no fit or stock/calibration claim until the data gate passes |
