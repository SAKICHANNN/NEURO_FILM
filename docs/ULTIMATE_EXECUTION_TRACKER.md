...
| U5.R2Z1 | contract frozen: implementation ready | Test whether asymmetric query/operator applicability can recover the ClassNeg evaluator gap without raw-content similarity | Z0 `case_bank_oracle_only` + failed global/spatial photometric selectors | separate query/operator spaces; leave-one-query-and-operator-out rank-2 then rank-4 bilinear ridge; OOD fallback; fresh confirmatory identities 17--32 open only after development pass. No neural/router/visual/final-628/film claim; `docs/planning/U5_R2Z1_ASYMMETRIC_APPLICABILITY_CONTRACT.md` |
