...
| U1.6 | in progress: ordered-effects child ready | Halo-aware tile/cache renderer | U1.3 | U1.6A-D prove execution, safe-Lab, simple-halation and sparse dust numerical parity; U1.6E freezes the supported effect order; physical/grain effects, full orchestration, streaming/cache and bounded total memory remain pending |
...
| U1.6E | frozen: ready | One-pass ordered simple-halation then sparse-dust composite | U1.6A/U1.6C/U1.6D | same-base layer generation, observable order, final-only margin, float/seam `<=1e-6`, sRGB8 parity, compact context and full CPU suite; no grain/physical/CLI/default claim; `docs/planning/U1_6E_ORDERED_SUPPORTED_EFFECTS_CONTRACT.md` |
