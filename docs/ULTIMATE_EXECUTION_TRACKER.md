...
| U5.R2AE1 | in progress: implementation verified / formal run pending | Test whether a source-bound subset forms a replay-stable, non-basic external operator bank on a fixed synthetic cube | AE0 limited pass | project-owned external runner and independent manifest evaluator implement source/runtime/hash binding, eight exact chains, duplicate/strength controls and replay/folding/range/matched-basic family-diversity gates; 7 focused and 1018 full tests pass; formal metrics remain unread; `docs/U5_R2AE1_SPECTRAL_FILM_LUT_STRUCTURAL_BANK_CONTRACT.md` |
