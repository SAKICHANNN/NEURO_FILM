...
| U5.R2AL0 | ready: source/method audit | Audit NamedCurves/NamedCurves+ and isolate a clean-room fixed analytic color-selective monotone-curve hypothesis | AK1 capacity close + G0/G1 bounded-style evidence | distinguish explicit monotone curves from the incompatible canonicalizer/attention path, pin source/licence/data facts and define a no-code/no-weight/no-photo synthetic DoR |
