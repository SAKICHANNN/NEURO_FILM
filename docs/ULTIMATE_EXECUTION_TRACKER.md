...
| U1.5 | in progress: explicit rejection strengthened | HEIF/HDR/gain-map detect/preserve or explicit rejection | U1.2 | U1.5A rejects unsupported containers/signals; U1.5B closes the first/last-4-MiB known-marker blind spot for bounded JPEG/PNG metadata structures; complete ISO 21496-1 detection and real format support remain pending |
...
| U1.5C | ready: contract frozen | Bind exact rights-cleared libultrahdr real gain-map references to the existing fail-closed ingress boundary | U1.2C/U1.5B + pinned upstream fixtures | require exact CC-BY-4.0 bytes, MPO/two-frame inspection, pre-decode load rejection and renderer no-output; pass adds regression evidence only, fail opens a separately frozen repair; no general MPF/ISO/HDR claim |
