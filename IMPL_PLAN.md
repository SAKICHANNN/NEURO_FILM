...
```bash
pip install diffusers>=0.28.0 transformers accelerate safetensors
...
| 胶片 | Civitai 链接 |
|------|-------------|
| Kodak Portra 400 | civitai.com/models/.../kodak-portra-400 |
| Kodak Vision3 500T | civitai.com/models/.../vision3-500t |
| Kodak Ektar 100 | civitai.com/models/.../ektar-100 |
| Kodak Tri-X 400 | civitai.com/models/.../tri-x-400 |

```python
pipe.load_lora_weights("path/to/lora/portra400.safetensors")
```
...
**数据源**：
1. **FilmSet**: 3 风格 × 5285 张，已有
2. **Flickr / r/analog**: 爬取指定胶片标签的高质量扫描
3. **Civitai 数据集**: 部分 LoRA 作者公开训练数据
...
### 2.4 Velvia 50 + HP5（2 天）

这两种胶片社区无现成 LoRA，需从零训练：
