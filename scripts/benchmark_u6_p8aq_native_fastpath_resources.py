#!/usr/bin/env python3
"""Rerun frozen 12MP native resource gates with exact AO6 v2 paths."""
...
SCHEMA = "neuro_film.u6_p8aq_native_fastpath_resources_contract.v1"
...
def validate_contract(config: dict[str, Any]) -> dict[str, Any]:
    if (
        ...
        raise ValueError("unsupported P8AQ contract")
    parent = _load_exact_json(
        ROOT / config["parent_decision"],
        config["parent_decision_sha256"],
    )
    if not str(parent.get("next_leaf", "")).startswith("U6.P8AQ"):
        raise ValueError("P8AQ parent decision drift")
    ...
    if (
        ...
        raise ValueError("P8AQ frozen P8AN resource gates drift")
    _load_exact_json(
...
def _apply_display_v2_rows(
    *,
    ...
    status = library.nf_ao6_display_f32_apply_v2(
    ...
    if status != 0:
        raise RuntimeError(f"P8AQ AO6 display v2 failed: {status}")
...
def _worker(
    *,
    ...
    display_library = _load_display_v2(display_dll)
    ...
    if context_library.nf_ao6_context_f32_init_v1(
        ...
        raise RuntimeError("P8AQ context init failed")
    for y0 in range(0, height, tile_rows):
        ...
        if status != 0:
            raise RuntimeError(f"P8AQ context update failed: {status}")
    ...
    if context_library.nf_ao6_context_f32_finalize_v1(
        ...
        raise RuntimeError("P8AQ context finalize failed")
    ...
    def output_sink(y0: int, y1: int, rows: np.ndarray) -> None:
        ...
        if y0 != consumed_rows or rows.shape != (y1 - y0, width, 3):
            raise RuntimeError("P8AQ physical output row order drift")
        gauged = _apply_gauge(
    ...
    if consumed_rows != height:
        raise RuntimeError("P8AQ did not consume every output row")
    result = {
        "height": height,
        "width": width,
        "pixels": height * width,
...
def _build_components(
    ...
    builds = {
        ...
        "context": build_msvc_native_ao6_context_v2_dll(
            root=ROOT, output_dir=output_dir / "context"
        ),
        "display": build_msvc_native_ao6_display_v2_dll(
    ...
    for name, expected in config["component_dll_sha256"].items():
        if builds[name]["dll_sha256"] != expected:
            raise ValueError(f"P8AQ {name} DLL identity drift")
    return builds
...
def benchmark(config: dict[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    stable_core = {
        "schema": (
            "neuro_film.u6_p8aq_native_fastpath_resources_result.v1"
        ),
        ...
        "output_repeat_and_v1_exact": exact,
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        default="configs/u6_p8aq_native_fastpath_resources_v1.json",
    )
    parser.add_argument(
        "--output-dir",
        default="outputs/u6_p8aq_native_fastpath_resources_v1",
    )
