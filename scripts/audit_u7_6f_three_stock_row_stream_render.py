#!/usr/bin/env python3
"""Measure full-width row-stream safe-Lab execution in the 24MP batch."""
...
from src.preprocess import normalized_icc_profile_sha256, save_srgb16_png

CONFIG = ROOT / "configs/u7_6f_three_stock_row_stream_render_v1.json"
DEFAULT_OUTPUT = ROOT / "outputs/eval/u7_6f_three_stock_row_stream_render_result.json"
SCRATCH = ROOT / "outputs/eval/u7_6f_three_stock_row_stream_render_audit"
...
def run_worker(mode: str, output_directory: Path) -> dict[str, Any]:
        ...
        def full_frame_renderer(
            ...
            *,
            ...
            return save_srgb16_png(
                output, path, compression_level=compression_level
            )

        batch_module._stream_three_stock_png = full_frame_renderer
        batch_module.THREE_STOCK_RENDER_EXECUTION_ID = controls[
    ...
    return {
        ...
        "render_execution_id": manifest["render_execution_id"],
    }
...
def evaluate_rows(
    ...
    gate_results = {
        ...
        "ordered_stock_ids_exact": all(
            row["manifest_stock_ids"] == stock_ids for row in rows
        ),
        "execution_identity_exact": all(
            row["render_execution_id"] == controls[f"{row['mode']}_execution_id"]
            for row in rows
        ),
...
def main() -> int:
    ...
    if decoded is None or decoded.shape[:2] != tuple(
        config["input"]["expected_dimensions"]
    ):
    ...
    try:
        ...
        identity_payload = {
            ...
            "execution_rows": [
                {
                    "mode": row["mode"],
                    "render_execution_id": row["render_execution_id"],
                    "rows": row["rows"],
        ...
        report = {
            "schema_version": "neuro-film.u7-6f-three-stock-row-stream-render-result.v1",
            "node_id": "U7.6F",
