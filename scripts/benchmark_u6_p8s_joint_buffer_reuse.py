#!/usr/bin/env python3
"""Benchmark exact U6.P8S tied-phase buffer reuse."""
...
SCHEMA = "neuro_film.u6_p8s_joint_buffer_reuse_resources_contract.v1"
...
def validate_contract(config: dict[str, Any]) -> dict[str, Any]:
    expected = {
        "source_context_float32_cast": "row-local-only",
        "source_context_lab_storage": "one-full-frame-float32",
        "physical_source_halo": "one-exact-directional-carry",
        "physical_output": (
            "destructively-reuses-consumed-linear-buffer"
        ),
        "physical_tile_rows": 128,
        "forward_and_reverse_exact_required": True,
    ...
    if (
        ...
        raise ValueError("unsupported U6.P8S contract")
    ...
    if (
        not decision["next_leaf"].startswith("U6.P8S")
        ...
        raise ValueError("U6.P8S parent decision drift")
    return measurement
...
def evaluate(
    ...
    stable = {
        "schema": (
            "neuro_film.u6_p8s_joint_buffer_reuse_stable_evidence.v1"
        ),
    ...
    return {
        "schema": (
            "neuro_film.u6_p8s_joint_buffer_reuse_resources_report.v1"
        ),
        "node": config["node"],
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=ROOT
        / "configs/u6_p8s_joint_buffer_reuse_resources_v1.json",
    )
