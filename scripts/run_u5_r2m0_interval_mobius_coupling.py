#!/usr/bin/env python
"""Run the frozen U5.R2M0 interval-Möbius coupling audit."""
...
from src.roll2film.interval_mobius_coupling import (  # noqa: E402
...
def run_audit(config: dict[str, Any], config_sha256: str) -> dict[str, Any]:
    ...
    for target_name, function in functions.items():
        ...
        operator = fit_interval_mobius_coupling(
            fit_source,
            fit_target,
            stage_channels=candidate["stage_channels"],
            maximum_lower_lift=float(candidate["maximum_lower_lift"]),
            maximum_upper_compression=float(
                candidate["maximum_upper_compression"]
            ),
            maximum_log_odds_shift=float(candidate["maximum_log_odds_shift"]),
            maximum_absolute_coefficient=float(
                candidate["maximum_absolute_coefficient"]
            ),
            endpoint_head_bias_initialization=float(
                fit["endpoint_head_bias_initialization"]
            ),
            log_odds_head_initialization=float(
                fit["log_odds_head_initialization"]
            ),
            seed=int(fit["seed"]),
            steps=int(fit["steps"]),
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/u5_r2m0_interval_mobius_coupling_v1.json",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "outputs/u5_r2m0_interval_mobius_coupling_v1/report.json",
    )
