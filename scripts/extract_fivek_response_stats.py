#!/usr/bin/env python3
"""Extract compact response statistics from a FiveK RAW-derived cache."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extract FiveK compact response statistics.")
    parser.add_argument(
        "--manifest",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "outputs" / "fivek_auto_optimize" / "response_stats_v1_mini64",
    )
...
def load_rgb(path: Path, stride: int) -> np.ndarray:
...
def response_curves(rows: list[dict[str, str]], bins: int, stride: int) -> tuple[dict, list[dict], dict[str, np.ndarray]]:
    ...
    for row in rows:
        raw = load_rgb(ROOT / row["raw_render"], stride)
        target = load_rgb(ROOT / row["target"], stride)
        raw_l = luma(raw)
...
def scene_buckets(per_image: list[dict]) -> dict[str, dict[str, float | int]]:
    buckets = {
        "dark": [row for row in per_image if row["raw_luma_mean"] < 0.25],
        "midtone": [row for row in per_image if 0.25 <= row["raw_luma_mean"] < 0.55],
        "bright": [row for row in per_image if row["raw_luma_mean"] >= 0.55],
        "low_chroma": [row for row in per_image if row["raw_chroma_mean"] < 0.08],
        "high_chroma": [row for row in per_image if row["raw_chroma_mean"] >= 0.08],
    }
    ...
    for name, items in buckets.items():
        ...
        result[name] = {
            "count": len(items),
            "luma_delta_mean": float(np.mean([row["luma_delta_mean"] for row in items])),
            "chroma_delta_mean": float(np.mean([row["chroma_delta_mean"] for row in items])),
            "rgb_delta_mean_r": float(np.mean([row["rgb_delta_mean_r"] for row in items])),
            "rgb_delta_mean_g": float(np.mean([row["rgb_delta_mean_g"] for row in items])),
            "rgb_delta_mean_b": float(np.mean([row["rgb_delta_mean_b"] for row in items])),
        }
    return result
...
def make_diagnostic_plot(curves: dict, output: Path) -> None:
    ...
    draw.text((margin, 16), "FiveK compact response stats: raw luma -> target response", fill="black", font=font)
...
def main() -> int:
    ...
    buckets = scene_buckets(per_image)
        ...
        json.dumps(
            {
                ...
                "note": "Compact response stats are fit from RAW/default preview renders, not full linear RAW tensors.",
            },
            indent=2,
        ),
        encoding="utf-8",
