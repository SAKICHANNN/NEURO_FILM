"""Build the non-mutating P96 integration manifest and strict schema."""
...
if __package__:
    ...
    from scripts.build_reference_match_integration_manifest_v13 import (
    ...
    from build_reference_match_integration_manifest_v13 import (
        REQUIRED_CONTRACT_SCHEMA_PATHS,
        REQUIRED_PUBLIC_EXPORTS,
    )
...
SCHEMA_ID = "neuro-film.reference-match-main-integration-manifest.v14"
CLAIM_CEILING = "review-ready-not-merged"
PAYLOAD_SCOPE = "P1-P95"
PRIOR_MANIFEST_PATH = (
    "configs/reference_match_main_integration_manifest_v13.json"
)
PRIOR_MANIFEST_SCHEMA_ID = (
    "neuro-film.reference-match-main-integration-manifest.v13"
)
...
V13_SCHEMA_PATH = (
    ...
    / "configs"
    / "schemas"
    / "reference_match_main_integration_manifest_v13.schema.json"
)
DEFAULT_SCHEMA_PATH = (
    ROOT
    / "configs"
...
def build_schema(*, v13_schema_path: Path = V13_SCHEMA_PATH) -> dict[str, Any]:
    try:
        schema = json.loads(v13_schema_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise IntegrationManifestError(
            f"v13 integration schema unavailable: {v13_schema_path}"
        ) from exc
    result = deepcopy(schema)
    result["$id"] = (
        "https://neuro-film.local/schemas/"
        "reference-match-main-integration-manifest-v14"
    )
    result["title"] = "Neuro-Film Reference Match Main Integration Manifest V14"
