#!/usr/bin/env python3
"""Evaluate distilled Neural LUT V2 variant outputs."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate distilled Neural LUT V2 variants.")
...
def contact_sheet(rows: list[dict], path: Path, title: str) -> None:
    ...
    for row in rows:
        ...
        draw.text((428, 178), "variant", fill="black", font=font)
        thumbs.append(tile)
...
def apply_model(model: np.lib.npyio.NpzFile, source: np.ndarray, style_index: int, strength: float, output_margin: int) -> tuple[np.ndarray, float]:
    scheme = str(model["scheme"].tolist()[0])
    if scheme == "nilut":
        return apply_distilled_nilut(
            source,
    ...
    if scheme == "context4d":
        return apply_distilled_context4d(
            source,
            style_index=style_index,
            residual4d=model["residual4d"],
            gate_weights=model["gate_weights"],
            strength=strength,
            output_margin=output_margin,
        )
...
def main() -> int:
    ...
    run_prefix = args.run_prefix or f"scheme_{'b' if scheme == 'nilut' else 'c'}_{scheme}_distilled_v1"
    ...
    for label, strength in strengths:
        ...
        for style in styles:
            ...
            for index, input_path in enumerate(paths, start=1):
                ...
                out, gate = apply_model(model, source, style_index, strength, args.output_margin)
                after_path = style_dir / "after" / f"{index:02d}_{style}_{scheme}.png"
    ...
    (output_root / f"{run_prefix}_all_strengths_summary.json").write_text(json.dumps(summaries, indent=2), encoding="utf-8")
    return 0
