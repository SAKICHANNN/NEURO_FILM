"""Build the non-mutating P77 integration manifest and strict schema."""
...
if __package__:
    ...
    from scripts.build_reference_match_integration_manifest_v7 import (
        REQUIRED_CONTRACT_SCHEMA_PATHS as V7_SCHEMA_PATHS,
        REQUIRED_PUBLIC_EXPORTS as V7_PUBLIC_EXPORTS,
    )
    ...
    from build_reference_match_integration_manifest_v7 import (
        REQUIRED_CONTRACT_SCHEMA_PATHS as V7_SCHEMA_PATHS,
        REQUIRED_PUBLIC_EXPORTS as V7_PUBLIC_EXPORTS,
    )
...
SCHEMA_ID = "neuro-film.reference-match-main-integration-manifest.v8"
CLAIM_CEILING = "review-ready-not-merged"
PAYLOAD_SCOPE = "P1-P76"
PRIOR_MANIFEST_PATH = (
    "configs/reference_match_main_integration_manifest_v7.json"
)
PRIOR_MANIFEST_SCHEMA_ID = (
    "neuro-film.reference-match-main-integration-manifest.v7"
)
...
DEFAULT_SCHEMA_PATH = (
    ...
    / "configs"
    / "schemas"
    / "reference_match_main_integration_manifest_v8.schema.json"
)
SRGB_ICC_PROFILE_SCHEMA_PATH = (
    "configs/schemas/reference_srgb_icc_profile_conformance_v1.schema.json"
)
NEW_PUBLIC_EXPORTS = (
    "srgb_icc_profile_conformance_v1",
    "srgb_icc_profile_v1",
)
REQUIRED_PUBLIC_EXPORTS = (*V7_PUBLIC_EXPORTS, *NEW_PUBLIC_EXPORTS)
...
def build_schema(*, v7_schema_path: Path = V7_SCHEMA_PATH) -> dict[str, Any]:
    try:
        schema = json.loads(v7_schema_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise IntegrationManifestError(
            f"v7 integration schema unavailable: {v7_schema_path}"
        ) from exc
    result = deepcopy(schema)
    result["$id"] = (
        "https://neuro-film.local/schemas/"
        "reference-match-main-integration-manifest-v8"
    )
    result["title"] = "Neuro-Film Reference Match Main Integration Manifest V8"
    ...
    contract["prefixItems"][6:6] = [
        _schema_prefix(SRGB_ICC_PROFILE_SCHEMA_PATH)
    ]
    enum = result["$defs"]["contract_schema"]["properties"]["path"]["enum"]
    enum[6:6] = [SRGB_ICC_PROFILE_SCHEMA_PATH]
