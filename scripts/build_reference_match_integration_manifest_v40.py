"""Build the P166 integration manifest."""
...
SCHEMA_ID = "neuro-film.reference-match-main-integration-manifest.v40"
CLAIM_CEILING = "review-ready-not-merged"
PAYLOAD_SCOPE = "P1-P166"
PRIOR_MANIFEST_PATH = "configs/reference_match_main_integration_manifest_v39.json"
PRIOR_MANIFEST_SCHEMA_ID = (
    "neuro-film.reference-match-main-integration-manifest.v39"
)
...
def build_schema(
    *,
    v39_schema_path: Path = V39_SCHEMA_PATH,
) -> dict[str, Any]:
    try:
        result = deepcopy(
            json.loads(v39_schema_path.read_text(encoding="utf-8"))
        )
    except (OSError, json.JSONDecodeError) as exc:
        raise IntegrationManifestError(
            "v39 integration schema unavailable"
        ) from exc
    result["$id"] = (
        "https://neuro-film.local/schemas/"
        "reference-match-main-integration-manifest-v40"
    )
    result["title"] = "Neuro-Film Reference Match Main Integration Manifest V40"
...
def build_manifest(
    *,
    ...
    return build_versioned_manifest(
        ...
        required_public_exports=V34_REQUIRED_PUBLIC_EXPORTS,
        required_contract_schema_paths=V34_REQUIRED_CONTRACT_SCHEMA_PATHS,
