"""Build the P146 integration manifest."""
...
SCHEMA_ID="neuro-film.reference-match-main-integration-manifest.v31";CLAIM_CEILING="review-ready-not-merged";PAYLOAD_SCOPE="P1-P145"
PRIOR_MANIFEST_PATH="configs/reference_match_main_integration_manifest_v30.json";PRIOR_MANIFEST_SCHEMA_ID="neuro-film.reference-match-main-integration-manifest.v30"
ROOT=Path(__file__).resolve().parents[1];V30_SCHEMA_PATH=ROOT/"configs/schemas/reference_match_main_integration_manifest_v30.schema.json";DEFAULT_SCHEMA_PATH=ROOT/"configs/schemas/reference_match_main_integration_manifest_v31.schema.json"
...
V31_REQUIRED_PUBLIC_EXPORTS=REQUIRED_PUBLIC_EXPORTS+("REFERENCE_FILE_OUTPUT_CAPABILITIES_ID","FileReferenceOutputCapability","reference_file_output_capabilities")
def build_schema(*,v30_schema_path:Path=V30_SCHEMA_PATH)->dict[str,Any]:
    try:result=deepcopy(json.loads(v30_schema_path.read_text(encoding="utf-8")))
    except (OSError,json.JSONDecodeError) as exc:raise IntegrationManifestError("v30 integration schema unavailable") from exc
    result["$id"]="https://neuro-film.local/schemas/reference-match-main-integration-manifest-v31";result["title"]="Neuro-Film Reference Match Main Integration Manifest V31";result["properties"]["schema_id"]["const"]=SCHEMA_ID;result["properties"]["payload_scope"]["const"]=PAYLOAD_SCOPE;result["properties"]["required_public_exports"]["const"]=list(V31_REQUIRED_PUBLIC_EXPORTS)
...
def build_manifest(*,repo:Path,payload_commit:str,base_commit:str,main_commit:str)->dict[str,Any]:return build_versioned_manifest(repo=repo,payload_commit=payload_commit,base_commit=base_commit,main_commit=main_commit,schema_id=SCHEMA_ID,payload_scope=PAYLOAD_SCOPE,required_public_exports=V31_REQUIRED_PUBLIC_EXPORTS,required_contract_schema_paths=REQUIRED_CONTRACT_SCHEMA_PATHS,prior_manifest_path=PRIOR_MANIFEST_PATH,prior_manifest_schema_id=PRIOR_MANIFEST_SCHEMA_ID,verification_commands=VERIFICATION_COMMANDS,claim_ceiling=CLAIM_CEILING)
