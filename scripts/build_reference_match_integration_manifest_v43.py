"""Build the P172 integration manifest."""
...
if __package__:
    ...
    from build_reference_match_integration_manifest_v42 import (
        V42_REQUIRED_CONTRACT_SCHEMA_PATHS,
        V42_REQUIRED_PUBLIC_EXPORTS,
    )
...
SCHEMA_ID = "neuro-film.reference-match-main-integration-manifest.v43"
CLAIM_CEILING = "review-ready-not-merged"
PAYLOAD_SCOPE = "P1-P172"
PRIOR_MANIFEST_PATH = "configs/reference_match_main_integration_manifest_v42.json"
PRIOR_MANIFEST_SCHEMA_ID = (
    "neuro-film.reference-match-main-integration-manifest.v42"
)
...
V43_REQUIRED_PUBLIC_EXPORTS = V42_REQUIRED_PUBLIC_EXPORTS
V43_REQUIRED_CONTRACT_SCHEMA_PATHS = V42_REQUIRED_CONTRACT_SCHEMA_PATHS
...
def build_schema(
    *,
    v42_schema_path: Path = V42_SCHEMA_PATH,
) -> dict[str, Any]:
    try:
        result = deepcopy(
            json.loads(v42_schema_path.read_text(encoding="utf-8"))
        )
    except (OSError, json.JSONDecodeError) as exc:
        raise IntegrationManifestError(
            "v42 integration schema unavailable"
        ) from exc
    result["$id"] = (
        "https://neuro-film.local/schemas/"
        "reference-match-main-integration-manifest-v43"
    )
    result["title"] = "Neuro-Film Reference Match Main Integration Manifest V43"
...
def build_manifest(
    *,
    ...
    return build_versioned_manifest(
        ...
        required_public_exports=V43_REQUIRED_PUBLIC_EXPORTS,
        required_contract_schema_paths=V43_REQUIRED_CONTRACT_SCHEMA_PATHS,
