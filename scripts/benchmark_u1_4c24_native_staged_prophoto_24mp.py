#!/usr/bin/env python3
"""Measure the exact native-assisted staged renderer on the frozen 24MP fixture."""
...
from src.inference.romm_rec2020_velvia_staged_native import (
    build_native_staged_velvia_runtime,
    render_supported_prophoto_velvia_rec2020_staged_native,
)

CONTRACT_SCHEMA = "neuro-film.u1-4c24-native-staged-prophoto-24mp-contract.v1"
REPORT_SCHEMA = "neuro-film.u1-4c24-native-staged-prophoto-24mp-report.v1"
...
def _bound_file(root: Path, row: dict[str, Any]) -> Path:
    ...
    if relative.is_absolute() or ".." in relative.parts:
        raise ValueError("C24 contract paths must be repository-relative")
    path = root / relative
    if not path.is_file() or _sha256(path) != row["sha256"]:
        raise ValueError(f"C24 bound file drift: {relative.as_posix()}")
    return path


def load_contract(path: Path, *, root: Path = ROOT) -> tuple[dict[str, Any], str]:
    payload = json.loads(Path(path).read_text(encoding="utf-8"))
    if payload.get("schema") != CONTRACT_SCHEMA or payload.get("experiment_id") != "U1.4C24":
        raise ValueError("unsupported U1.4C24 contract")
    ...
    if (
        ...
        or payload["candidate"]["thread_count"] != 8
        or payload["candidate"]["c21_gamut_kernel_allowed"]
        or payload["measurement"]["fresh_processes"] != 2
        or not payload["measurement"]["run_workers_sequentially"]
    ):
        raise ValueError("C24 frozen execution drift")
...
def worker(contract_path: Path, output_dir: Path, *, root: Path = ROOT) -> dict[str, Any]:
    ...
    if output_dir.exists():
        raise ValueError("C24 worker output must be create-only")
    output_dir.mkdir(parents=True)
    try:
        runtime = build_native_staged_velvia_runtime(
            root=root, build_dir=output_dir / "build"
        )
        output_path = output_dir / "render.png"
        started = time.perf_counter()
        receipt = render_supported_prophoto_velvia_rec2020_staged_native(
...
def benchmark(contract_path: Path, output_dir: Path, *, root: Path = ROOT) -> dict[str, Any]:
    contract, contract_sha256 = load_contract(contract_path, root=root)
    if output_dir.exists():
        raise ValueError("C24 benchmark output must be create-only")
    output_dir.mkdir(parents=True)
    try:
        runs = []
        ...
        builds = {
            json.dumps(run["worker"]["build_receipt"], sort_keys=True)
            for run in runs
        }
        ...
        report = {
            ...
            **stable,
            "measurements": {
                ...
                "worker_wall_seconds": [
                    run["worker"]["wall_seconds"] for run in runs
                ],
