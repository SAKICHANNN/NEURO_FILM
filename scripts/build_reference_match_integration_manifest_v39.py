"""Build the P158 integration manifest."""
...
SCHEMA_ID = "neuro-film.reference-match-main-integration-manifest.v39"
CLAIM_CEILING = "review-ready-not-merged"
PAYLOAD_SCOPE = "P1-P158"
PRIOR_MANIFEST_PATH = "configs/reference_match_main_integration_manifest_v38.json"
PRIOR_MANIFEST_SCHEMA_ID = (
    "neuro-film.reference-match-main-integration-manifest.v38"
)
ROOT = Path(__file__).resolve().parents[1]
V38_SCHEMA_PATH = (
    ROOT / "configs/schemas/reference_match_main_integration_manifest_v38.schema.json"
)
DEFAULT_SCHEMA_PATH = (
    ROOT / "configs/schemas/reference_match_main_integration_manifest_v39.schema.json"
)
...
def build_schema(
    *,
    v38_schema_path: Path = V38_SCHEMA_PATH,
) -> dict[str, Any]:
    try:
        result = deepcopy(
            json.loads(v38_schema_path.read_text(encoding="utf-8"))
        )
    except (OSError, json.JSONDecodeError) as exc:
        raise IntegrationManifestError(
            "v38 integration schema unavailable"
        ) from exc
    result["$id"] = (
        "https://neuro-film.local/schemas/"
        "reference-match-main-integration-manifest-v39"
    )
    result["title"] = "Neuro-Film Reference Match Main Integration Manifest V39"
