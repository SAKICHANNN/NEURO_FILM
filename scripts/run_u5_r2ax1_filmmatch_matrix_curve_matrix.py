"""Run positive matrix-curve-matrix FilmMatch capacity."""
...
from src.eval.filmmatch_matrix_curve_matrix_capacity import (  # noqa: E402
    evaluate_matrix_curve_matrix_capacity,
)
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/u5_r2ax1_filmmatch_matrix_curve_matrix_v1.json",
    )
    parser.add_argument(
        "--output",
        type=Path,
        ...
        "matrix_curve_matrix_report.json",
    )
    ...
    if (
        ...
        raise ValueError("AX0 parent identity drift")
    sample_config = json.loads(
        (ROOT / config["parent"]["sample_config"]).read_text(encoding="utf-8")
    )
    report = evaluate_matrix_curve_matrix_capacity(
