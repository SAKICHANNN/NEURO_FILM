...
def run(root: Path, config_path: Path) -> None:
    ...
    archive_path = root / config["frozen_descriptor_archive"]
    calibration_path = root / config["calibration_receipt"]
    for path, expected in ((archive_path, config["frozen_descriptor_archive_sha256"]), (calibration_path, config["calibration_receipt_sha256"]), (input_path, config["frozen_input_manifest_sha256"]), (root / "src/color_match/research/local_reference_field_v1.py", config["frozen_extractor_sha256"])):
        if sha256(path) != expected:
            raise ValueError(f"Frozen V2 binding mismatch: {path}")
    calibration = json.loads(calibration_path.read_text())
    if calibration["tau"] != config["calibrated_distance_threshold"]:
        raise ValueError("Reference-only threshold binding mismatch")
    descriptor_cache = np.load(archive_path)
    cached_reference_features = descriptor_cache["reference_features"]
    cached_reference_files = descriptor_cache["reference_files"]
    cached_reference_centers = descriptor_cache["reference_centers_xy"]
    ...
    report = {
        "schema": "local-reference-field-report-v2", "status": "RUNNING",
        ...
        "code_sha256": {p: sha256(root / p) for p in ["src/color_match/research/local_reference_field_v1.py", "scripts/run_local_reference_field_v2.py", "scripts/calibrate_local_reference_matcher_v2.py"]},
    ...
    try:
        ...
        with torch.no_grad():
            for index, row in enumerate(refs["rows"]):
                ...
                selected_cache = cached_reference_files == index
                if not np.array_equal(centers.cpu().numpy(), cached_reference_centers[selected_cache]):
                    raise ValueError("Reference patch coordinates differ from frozen descriptors")
                features.append(torch.from_numpy(cached_reference_features[selected_cache]).to(device))
        ...
        tau = config["calibrated_distance_threshold"]
        ...
        for source_index, source in enumerate(inputs):
            ...
            centers = centers_for(x, config["grid_spacing"])
            with torch.no_grad():
                if not np.array_equal(centers.cpu().numpy(), descriptor_cache[f"{source_id}_centers_xy"]):
                    raise ValueError("Source patch coordinates differ from frozen descriptors")
                source_features = torch.from_numpy(descriptor_cache[f"{source_id}_features"]).to(device)
        ...
        report["completed_arm_receipts"] = sum(len(row["arms"]) for row in report["rows"])
        report["trained_fits"] = sum(arm["steps"] > 0 for row in report["rows"] for arm in row["arms"].values())
        report["actual_updates"] = sum(arm["steps"] for row in report["rows"] for arm in row["arms"].values())
        descriptor_cache.close()
        write_json(output / "report.json", report)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="configs/local_reference_field_v2.json")
