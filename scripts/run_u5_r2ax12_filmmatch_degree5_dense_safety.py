"""Run dense numerical and visual stress on the AX10 degree-5 operator."""
...
def main() -> int:
    ...
    validation_raw = (ROOT / config["parent"]["validation_report"]).read_bytes()
    if (
        hashlib.sha256(validation_raw).hexdigest()
        != config["parent"]["validation_report_sha256"]
    ):
        raise ValueError("AX11 validation identity drift")
    ...
    if (
        ...
        raise ValueError("AX10 operator report identity drift")
    parent = json.loads(parent_raw)
    if (
        parent["stable_evidence_id"]
        != config["parent"]["operator_stable_evidence_id"]
    ):
        raise ValueError("AX10 operator stable identity drift")
