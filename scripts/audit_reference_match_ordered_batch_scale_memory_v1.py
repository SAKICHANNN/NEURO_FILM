"""Run the frozen P160 24 MP ordered three-source batch audit."""
...
CONFIG_PATH = (
    ROOT / "configs" / "reference_match_ordered_batch_scale_memory_v1.json"
)


def load_config(path: Path) -> dict[str, Any]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("schema_version") != 1 or payload.get("node") != "P160":
        raise ValueError("config must be the P160 ordered-batch contract")
    ...
    if (
        ...
        or len(set(image["source_seeds"])) != 3
    ):
        raise ValueError("image contract must contain three distinct 24 MP sources")
    file_match = payload.get("file_match")
    if (
        not isinstance(file_match, dict)
        or file_match.get("source_count") != 3
        or file_match.get("execution_semantics")
        != "ordered-sequential-source-processing"
    ):
        raise ValueError("file_match differs from the frozen ordered-batch contract")
...
def generate_inputs(config: dict[str, Any], input_dir: Path) -> dict[str, Any]:
    ...
    rows = [
        ("reference", input_dir / "reference.png", image["reference_seed"]),
        *[
            (f"source-{index}", input_dir / f"source-{index}.png", seed)
            for index, seed in enumerate(image["source_seeds"])
        ],
    ]
    hashes: dict[str, str] = {}
    for role, path, seed in rows:
    ...
    return {
        ...
        "sources": [hashes[f"source-{index}"] for index in range(3)],
    }
...
def worker(
    *,
    ...
    source_paths = [input_dir / f"source-{index}.png" for index in range(3)]
    output_paths = [run_dir / f"output-{index}.png" for index in range(3)]
...
def evaluate_runs(
    ...
    ordered_binding_pass = bool(
        ...
        and all(
            ...
            and len(row["worker_result"]["output_sha256"]) == 3
            and len(row["worker_result"]["normalized_report"]["outputs"]) == 3
            and [
                item["source_path"]
                for item in row["worker_result"]["normalized_report"]["outputs"]
            ]
    ...
    return {
        ...
        "all_identity_fallback_pass": all_identity,
