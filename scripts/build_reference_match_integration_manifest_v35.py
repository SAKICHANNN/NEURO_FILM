"""Build the P150 integration manifest."""
...
SCHEMA_ID="neuro-film.reference-match-main-integration-manifest.v35";CLAIM_CEILING="review-ready-not-merged";PAYLOAD_SCOPE="P1-P150"
PRIOR_MANIFEST_PATH="configs/reference_match_main_integration_manifest_v34.json";PRIOR_MANIFEST_SCHEMA_ID="neuro-film.reference-match-main-integration-manifest.v34"
ROOT=Path(__file__).resolve().parents[1];V34_SCHEMA_PATH=ROOT/"configs/schemas/reference_match_main_integration_manifest_v34.schema.json";DEFAULT_SCHEMA_PATH=ROOT/"configs/schemas/reference_match_main_integration_manifest_v35.schema.json"
...
def build_schema(*,v34_schema_path:Path=V34_SCHEMA_PATH)->dict[str,Any]:
    try:result=deepcopy(json.loads(v34_schema_path.read_text(encoding="utf-8")))
    except (OSError,json.JSONDecodeError) as exc:raise IntegrationManifestError("v34 integration schema unavailable") from exc
    result["$id"]="https://neuro-film.local/schemas/reference-match-main-integration-manifest-v35";result["title"]="Neuro-Film Reference Match Main Integration Manifest V35";result["properties"]["schema_id"]["const"]=SCHEMA_ID;result["properties"]["payload_scope"]["const"]=PAYLOAD_SCOPE
