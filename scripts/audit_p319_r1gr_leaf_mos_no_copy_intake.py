#!/usr/bin/env python3
"""Audit exact R1GR Leaf MOS unpacking without copying producer code."""
...
SCHEMA = "neuro-film.p319-r1gr-leaf-mos-no-copy-intake.v1"


class P319Error(RuntimeError):
    pass
...
def _verify(repo: Path, commit: str, binding: dict[str, Any]) -> bytes:
    ...
    if observed != expected:
        raise P319Error(f"producer artifact differs: {binding['path']}")
    return value


def _load(site: Path, module_bytes: bytes, dependency_bytes: bytes) -> Any:
    package = site / "zhuise"
    package.mkdir(parents=True)
    (package / "__init__.py").write_bytes(b"")
    (package / "lossless_jpeg.py").write_bytes(dependency_bytes)
    (package / "leaf_mos.py").write_bytes(module_bytes)
    sys.path.insert(0, str(site))
    try:
        return importlib.import_module("zhuise.leaf_mos")
    finally:
...
def _reject_truncation(module: Any, source: bytes) -> bool:
    try:
        module.decode_leaf_mos(source[:-1])
    except ValueError:
        return True
    return False
...
def execute(config_path: Path, producer_repo: Path, order: str) -> dict[str, Any]:
    if order not in {"forward", "reverse"}:
        raise P319Error("order must be forward or reverse")
    config_bytes = config_path.read_bytes()
    config = json.loads(config_bytes)
    if config["status"] != "FROZEN_BEFORE_FIRST_CONSUMER_DECODE":
        raise P319Error("P319 is not frozen")
    ...
    for row in source_bindings:
        ...
        if source_before[row["id"]] != expected:
            raise P319Error(f"source differs: {row['id']}")

    temporary = Path(tempfile.mkdtemp(prefix="neuro-film-p319-"))
    module_names = ("zhuise.leaf_mos", "zhuise.lossless_jpeg", "zhuise")
    try:
        module = _load(
            temporary / "site",
            artifacts["callable_module"],
            artifacts["lossless_jpeg_dependency"],
        )
        ...
        for binding in invocation:
            ...
            metadata = module.parse_leaf_mos_metadata(source)
            output = module.decode_leaf_mos(source)
            rows.append(
                {
                    "id": binding["id"],
        ...
        dependency_binding = config["artifacts"]["lossless_jpeg_dependency"]
        gates = {
            ...
            == "SOURCE_STORAGE_AND_DECODER_BRANCH_LOCKED_BEFORE_RAW_SAMPLE_DECODE"
            and execution_lock["fixed_identity"]["git_blobs"]["core"]
            == core_binding["git_blob"]
            and execution_lock["fixed_identity"]["git_blobs"]["lossless_jpeg"]
            == dependency_binding["git_blob"],
            "producer-evidence-exact": evidence["status"]
            == "PASS_PRIVATE_LEAF_MOS_DUAL_STORAGE_UNPACK_CALLABLE"
            ...
            and evidence["fixed_identity"]["lossless_jpeg_git_blob"]
            == dependency_binding["git_blob"],
        ...
        report = {
            "schema": SCHEMA,
            "experiment_id": "P319",
            "status": "PASS_PRIVATE_R1GR_LEAF_MOS_NO_COPY_INTAKE"
            if all(gates.values())
            else "FAIL_CLOSED_R1GR_LEAF_MOS_NO_COPY_INTAKE",
            "bindings": {
                "artifacts": verified,
        ...
        if not all(gates.values()):
            ...
            raise P319Error(f"P319 gates failed: {', '.join(failed)}")
        return report
    finally:
        ...
        if temporary.exists():
            raise P319Error("temporary residue remains")
