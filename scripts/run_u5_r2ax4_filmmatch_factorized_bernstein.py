"""Run monotone-base plus safe Bernstein residual FilmMatch capacity."""
...
from src.eval.filmmatch_factorized_bernstein_capacity import (  # noqa: E402
    evaluate_factorized_bernstein_capacity,
)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=ROOT
        / "configs/u5_r2ax4_filmmatch_factorized_bernstein_v1.json",
    )
    parser.add_argument(
        "--output",
        type=Path,
        ...
        "factorized_bernstein_report.json",
    )
    ...
    if (
        ...
        raise ValueError("AX3 parent identity drift")
    sample_config = json.loads(
        (ROOT / config["parent"]["sample_config"]).read_text(encoding="utf-8")
    )
    report = evaluate_factorized_bernstein_capacity(
