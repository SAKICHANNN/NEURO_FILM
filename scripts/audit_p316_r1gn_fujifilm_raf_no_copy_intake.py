#!/usr/bin/env python3
"""Audit exact corrected R1GN RAF unpacking without copying producer code."""
...
SCHEMA = "neuro-film.p316-r1gn-fujifilm-raf-no-copy-intake.v1"


class P316Error(RuntimeError):
    pass
...
def _verify(repo: Path, commit: str, binding: dict[str, Any]) -> bytes:
    ...
    if observed != expected:
        raise P316Error(f"producer artifact differs: {binding['path']}")
    return value
...
def _load(site: Path, module_bytes: bytes) -> Any:
    ...
    (package / "fujifilm_raf.py").write_bytes(module_bytes)
    sys.path.insert(0, str(site))
    try:
        return importlib.import_module("zhuise.fujifilm_raf")
    finally:
...
def _reject_truncation(module: Any, source: bytes) -> bool:
    try:
        module.decode_fujifilm_raf(source[:-1])
    except ValueError:
        return True
    return False
...
def execute(config_path: Path, producer_repo: Path, order: str) -> dict[str, Any]:
    if order not in {"forward", "reverse"}:
        raise P316Error("order must be forward or reverse")
    config_bytes = config_path.read_bytes()
    config = json.loads(config_bytes)
    if config["status"] != "FROZEN_BEFORE_FIRST_CONSUMER_DECODE":
        raise P316Error("P316 is not frozen")

    correction = config["identity_correction"]
    corrected_resolves = _commit_resolves(
        producer_repo, correction["corrected_implementation_commit"]
    )
    superseded_rejects = not _commit_resolves(
        producer_repo, correction["superseded_unresolvable_implementation_commit"]
    )
    ...
    for row in source_bindings:
        ...
        if source_before[row["id"]] != expected:
            raise P316Error(f"source differs: {row['id']}")

    temporary = Path(tempfile.mkdtemp(prefix="neuro-film-p316-"))
    module_names = ("zhuise.fujifilm_raf", "zhuise")
    try:
        ...
        for binding in invocation:
            ...
            metadata = module.parse_fujifilm_raf_metadata(source)
            output = module.decode_fujifilm_raf(source)
            rows.append(
                {
                    "id": binding["id"],
                    ...
                    "fuji_layout_hex": metadata.fuji_layout.hex(),
                    "layout_selector": int(metadata.layout_selector),
                    "stored_uint16_order": metadata.stored_uint16_order,
        ...
        for row, binding in zip(rows, source_bindings, strict=True):
            ...
            expected_selector = 0 if binding["stored_uint16_order"] == "big-endian" else 1
            exact_rows.append(
                ...
                and row["fuji_layout_hex"]
                == binding["fuji_layout_hex"]
                == fixture_row["fuji_layout_hex"]
                and row["stored_uint16_order"]
                == binding["stored_uint16_order"]
                == fixture_row["stored_uint16_order"]
                and row["layout_selector"] == expected_selector
        ...
        gates = {
            "identity-correction-exact": corrected_resolves
            and superseded_rejects
            and _git_blob(
                producer_repo,
                correction["corrected_implementation_commit"],
                core_binding["path"],
            )
            == core_binding["git_blob"]
            and correction["science_source_fixture_gates_and_formal_reports_changed"]
            is False,
            ...
            and contract["source_lock"]["implementation_git_blob"]
            == core_binding["git_blob"],
            "producer-locks-exact": preregistration["candidate"]["callable_id"]
            == config["protocol"]
            and execution_lock["fixed_identity"]["core_git_blob"]
            == core_binding["git_blob"],
            "producer-evidence-exact": evidence["status"]
            == "PASS_PRIVATE_FUJIFILM_RAF_DUAL_LAYOUT_UNPACK_CALLABLE"
        ...
        scientific = {
            ...
            "identity_correction": {
                "corrected_commit_resolves": corrected_resolves,
                "superseded_commit_rejects": superseded_rejects,
            },
            "rows": rows,
            "controls": controls,
            "gates": gates,
        ...
        report = {
            "schema": SCHEMA,
            "experiment_id": "P316",
            "status": "PASS_PRIVATE_R1GN_FUJIFILM_RAF_NO_COPY_INTAKE"
            if all(gates.values())
            else "FAIL_CLOSED_R1GN_FUJIFILM_RAF_NO_COPY_INTAKE",
            "bindings": {
                "artifacts": verified,
        ...
        if not all(gates.values()):
            ...
            raise P316Error(f"P316 gates failed: {', '.join(failed)}")
        return report
    finally:
        ...
        if temporary.exists():
            raise P316Error("temporary residue remains")
