...
from src.eval.physical_native_ordered_chain_conformance import (  # noqa: E402
    run_native_ordered_chain_conformance,
    write_report,
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        default="configs/u6_p8aa_native_ordered_chain_v1.json",
    )
    parser.add_argument(
        "--output-dir",
        default="outputs/u6_p8aa_native_ordered_chain_v1",
    )
    arguments = parser.parse_args()
    config = json.loads((ROOT / arguments.config).read_text())
    output_dir = ROOT / arguments.output_dir
    report = run_native_ordered_chain_conformance(
        root=ROOT,
        config=config,
