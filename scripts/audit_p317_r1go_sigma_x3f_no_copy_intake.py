#!/usr/bin/env python3
"""Audit exact R1GO Sigma X3F unpacking without copying producer code."""
...
SCHEMA = "neuro-film.p317-r1go-sigma-x3f-no-copy-intake.v1"


class P317Error(RuntimeError):
    pass
...
def _verify(repo: Path, commit: str, binding: dict[str, Any]) -> bytes:
    ...
    if observed != expected:
        raise P317Error(f"producer artifact differs: {binding['path']}")
    return value
...
def _load(site: Path, module_bytes: bytes) -> Any:
    ...
    (package / "sigma_x3f.py").write_bytes(module_bytes)
    sys.path.insert(0, str(site))
    try:
        return importlib.import_module("zhuise.sigma_x3f")
    finally:
...
def _reject_truncation(module: Any, source: bytes) -> bool:
    try:
        module.decode_sigma_x3f(source[:-1])
    except ValueError:
        return True
    return False
...
def execute(config_path: Path, producer_repo: Path, order: str) -> dict[str, Any]:
    if order not in {"forward", "reverse"}:
        raise P317Error("order must be forward or reverse")
    config_bytes = config_path.read_bytes()
    config = json.loads(config_bytes)
    if config["status"] != "FROZEN_BEFORE_FIRST_CONSUMER_DECODE":
        raise P317Error("P317 is not frozen")
    ...
    for row in source_bindings:
        ...
        if source_before[row["id"]] != expected:
            raise P317Error(f"source differs: {row['id']}")

    temporary = Path(tempfile.mkdtemp(prefix="neuro-film-p317-"))
    module_names = ("zhuise.sigma_x3f", "zhuise")
    try:
        ...
        for binding in invocation:
            ...
            metadata = module.parse_sigma_x3f_metadata(source)
            output = module.decode_sigma_x3f(source)
            rows.append(
                {
                    "id": binding["id"],
                    "source_bytes": len(source),
                    "source_sha256": source_hash,
                    "geometry_hwc": [int(value) for value in output.shape],
                    "type_format_hex": f"0x{int(metadata.type_format):08x}",
        ...
        for row, binding in zip(rows, source_bindings, strict=True):
            ...
            exact_rows.append(
                ...
                and row["geometry_hwc"]
                == binding["geometry_hwc"]
                == fixture_row["geometry_hwc"]
                and row["type_format_hex"]
                == binding["type_format_hex"]
                == fixture_row["type_format_hex"]
                and row["codec"] == binding["codec"] == fixture_row["codec"]
        ...
        gates = {
            ...
            == "LOCKED_AFTER_HEADER_ONLY_AUDIT_BEFORE_FIRST_RAW_BODY_DECODE"
            and execution_lock["fixed_identity"]["git_blobs"]["core"]
            == core_binding["git_blob"],
            "producer-evidence-exact": evidence["status"]
            == "PASS_PRIVATE_SIGMA_X3F_DUAL_CODEC_UNPACK_CALLABLE"
        ...
        report = {
            "schema": SCHEMA,
            "experiment_id": "P317",
            "status": "PASS_PRIVATE_R1GO_SIGMA_X3F_NO_COPY_INTAKE"
            if all(gates.values())
            else "FAIL_CLOSED_R1GO_SIGMA_X3F_NO_COPY_INTAKE",
            "bindings": {
                "artifacts": verified,
        ...
        if not all(gates.values()):
            ...
            raise P317Error(f"P317 gates failed: {', '.join(failed)}")
        return report
    finally:
        ...
        if temporary.exists():
            raise P317Error("temporary residue remains")
