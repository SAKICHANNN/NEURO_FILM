"""Build the file-identity-hardened post-main-merge integration manifest."""
...
if __package__:
    ...
    from build_reference_match_integration_manifest_v46 import (
        V46_REQUIRED_CONTRACT_SCHEMA_PATHS,
        V46_REQUIRED_PUBLIC_EXPORTS,
    )
...
SCHEMA_ID = "neuro-film.reference-match-main-integration-manifest.v47"
CLAIM_CEILING = "review-ready-file-identity-hardened-not-merged"
PAYLOAD_SCOPE = "P173-P177-file-identity-hardened-post-main-merge"
PRIOR_MANIFEST_PATH = "configs/reference_match_main_integration_manifest_v46.json"
PRIOR_MANIFEST_SCHEMA_ID = (
    "neuro-film.reference-match-main-integration-manifest.v46"
)
...
V47_REQUIRED_PUBLIC_EXPORTS = V46_REQUIRED_PUBLIC_EXPORTS
V47_REQUIRED_CONTRACT_SCHEMA_PATHS = V46_REQUIRED_CONTRACT_SCHEMA_PATHS
VERIFICATION_COMMANDS = (
    "git merge-tree --write-tree <payload_commit> <main_commit>",
    "git diff --check <base_commit> <payload_commit>",
    "python -m pytest -q tests/test_color_match_files.py "
    "tests/test_color_match_file_replay.py",
...
def build_schema(
    *,
    v46_schema_path: Path = V46_SCHEMA_PATH,
) -> dict[str, Any]:
    try:
        result = deepcopy(
            json.loads(v46_schema_path.read_text(encoding="utf-8"))
        )
    except (OSError, json.JSONDecodeError) as exc:
        raise IntegrationManifestError(
            "v46 integration schema unavailable"
        ) from exc
    result["$id"] = (
        "https://neuro-film.local/schemas/"
        "reference-match-main-integration-manifest-v47"
    )
    result["title"] = "Neuro-Film Reference Match Main Integration Manifest V47"
    result["properties"]["schema_id"]["const"] = SCHEMA_ID
    result["properties"]["payload_scope"]["const"] = PAYLOAD_SCOPE
    result["properties"]["claim_ceiling"]["const"] = CLAIM_CEILING
...
def build_manifest(
    *,
    ...
    return build_versioned_manifest(
        ...
        required_public_exports=V47_REQUIRED_PUBLIC_EXPORTS,
        required_contract_schema_paths=V47_REQUIRED_CONTRACT_SCHEMA_PATHS,
