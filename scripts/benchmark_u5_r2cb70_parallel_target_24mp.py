#!/usr/bin/env python3
"""Measure exact parallel target construction in the complete 24MP renderer."""
...
def _worker(config: dict, output: Path, scratch: Path) -> dict:
    ...
    runtime = load_analytic_y_chromaticity_profile(
        ROOT / "configs/render_profiles/analytic_y_chromaticity_cb66_v3.json",
        root=ROOT,
    )
    candidate, facts = render_analytic_y_chromaticity_parallel_target_candidate(
        working,
        runtime,
        scratch_root=scratch,
        row_chunk=int(config["candidate"]["row_chunk"]),
        workers=int(config["candidate"]["workers"]),
    )
    save_srgb16_png(composite_layers(candidate, [], output_margin=0), output)
    return {
...
def _launch(config_path: Path, work_dir: Path, index: int) -> dict:
    ...
    while process.poll() is None:
        try:
            ...
            pass
        if perf_counter() - started > 240.0:
            for child in reversed(root.children(recursive=True)):
                child.kill()
            root.kill()
            raise TimeoutError("CB70 worker exceeded 240 seconds")
        sleep(0.02)
...
def _parent(config_path: Path, output: Path, work_dir: Path) -> dict:
    ...
    checks = {
        ...
        and all(
            result["output_sha256"] == baseline["output_sha256"]
            for result in results
        ),
        "selector_facts_exact": complete
        and all(
            result["selector_facts"] == baseline["selector_facts"]
            for result in results
        ),
