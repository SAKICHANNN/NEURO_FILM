"""Build the non-mutating P89 integration manifest and strict schema."""
...
if __package__:
    ...
    from build_reference_match_integration_manifest_v12 import (
        REQUIRED_CONTRACT_SCHEMA_PATHS,
        REQUIRED_PUBLIC_EXPORTS,
    )
...
SCHEMA_ID = "neuro-film.reference-match-main-integration-manifest.v13"
CLAIM_CEILING = "review-ready-not-merged"
PAYLOAD_SCOPE = "P1-P88"
PRIOR_MANIFEST_PATH = (
    "configs/reference_match_main_integration_manifest_v12.json"
)
PRIOR_MANIFEST_SCHEMA_ID = (
    "neuro-film.reference-match-main-integration-manifest.v12"
)
...
def build_schema(*, v12_schema_path: Path = V12_SCHEMA_PATH) -> dict[str, Any]:
    try:
        schema = json.loads(v12_schema_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise IntegrationManifestError(
            f"v12 integration schema unavailable: {v12_schema_path}"
        ) from exc
    result = deepcopy(schema)
    result["$id"] = (
        "https://neuro-film.local/schemas/"
        "reference-match-main-integration-manifest-v13"
    )
    result["title"] = "Neuro-Film Reference Match Main Integration Manifest V13"
