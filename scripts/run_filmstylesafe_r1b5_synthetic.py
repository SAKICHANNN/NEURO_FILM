"""Run the R1B5 explicit highlight-chroma-speckle synthetic failure prototype."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs" / "filmstylesafe_r1b5_synthetic_operator_v1.json",
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=ROOT / "outputs" / "filmstylesafe" / "r1b5" / "report.json",
    )
    ...
    params = {
        ...
        "speckle_density": config["speckle_density"],
        "blob_radius_px": config["blob_radius_px"],
    }
    result = run_highlight_chroma_speckle_v1(
