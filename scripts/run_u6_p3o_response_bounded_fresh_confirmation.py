#!/usr/bin/env python
"""Run the U6.P3O fresh response-bounded residual confirmation."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=ROOT
        / "configs/u6_p3o_response_bounded_fresh_confirmation_v1.json",
    )
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--visual-root", type=Path, required=True)
    args = parser.parse_args()
    contract = load_contract(args.config)
    report = evaluate_response_bounded_fresh_confirmation(
        root=ROOT,
        contract=contract,
