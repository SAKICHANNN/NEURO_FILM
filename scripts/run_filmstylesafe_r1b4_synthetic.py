"""Run the R1B4 explicit highlight-chroma-island synthetic failure prototype."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs" / "filmstylesafe_r1b4_synthetic_operator_v0.json",
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=ROOT / "outputs" / "filmstylesafe" / "r1b4" / "report.json",
    )
    ...
    params = {
        ...
        "seed": config["seed"],
        "center_xy_norm": config["center_xy_norm"],
        "radius_norm": config["radius_norm"],
        "luma_threshold": config["luma_threshold"],
        "chroma_boost": config["chroma_boost"],
    }
    result = run_highlight_chroma_island_v0(
    ...
    print(json.dumps({k: report[k] for k in (
        "operator_id", "input_hash", "output_hash", "parameter_hash",
        "output_path", "visual_adjudication_status"
    )}, indent=2))
    return 0
