#!/usr/bin/env python3
"""Run the P4GM capacity-matched chart artifact confirmation."""
...
def _visual(path: Path, source: np.ndarray, physics: np.ndarray, residual: np.ndarray) -> None:
    ...
    for label, value in (
        ...
        ("capacity-matched physics", physics),
        ("physics + AO6 residual", residual),
    ):
...
def evaluate(contract_path: Path, visual: Path | None = None) -> dict:
    ...
    for parent in contract["parents"].values():
        ...
        if _sha(path) != parent["sha256"] or decision != parent["required_decision"]:
            raise RuntimeError("P4GM parent drift")
    ...
    physics_outputs: list[np.ndarray] = []
    receipts = []
        ...
        try:
            ...
            for tile_rows in fixture["tile_rows"]:
                ...
                cloud.render_to_sink(
                    source, output_sink=lambda _a, _b, value: scans.append(value.copy())
                )
                physics_outputs.append(
                    np.ascontiguousarray(
                        linear_srgb_to_encoded(np.concatenate(scans).astype(np.float64)),
                        np.float32,
                    )
                )
                for _ in range(2):
    ...
    physics_metrics = _metrics(physics_outputs[0], patches, contract)
    residual_metrics = _metrics(outputs[0], patches, contract)
    ...
    checks = {
        ...
        "residual_neutral_increment": residual_metrics["neutral_chroma_p99"] - physics_metrics["neutral_chroma_p99"] <= gates["maximum_residual_neutral_chroma_increase"],
        "residual_overshoot_increment": residual_metrics["edge_overshoot"] - physics_metrics["edge_overshoot"] <= gates["maximum_residual_edge_overshoot_increase"],
        "residual_undershoot_increment": residual_metrics["edge_undershoot"] - physics_metrics["edge_undershoot"] <= gates["maximum_residual_edge_undershoot_increase"],
