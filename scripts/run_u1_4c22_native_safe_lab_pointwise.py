...
def _load_contract(path: Path, *, root: Path) -> tuple[dict[str, Any], str]:
    ...
    if payload.get("schema") != "neuro-film.u1-4c22-native-safe-lab-pointwise.v1":
        raise ValueError("C22 contract schema is invalid")
    for binding in payload["bindings"]:
        bound = root / binding["path"]
        if _sha256(bound) != binding["sha256"]:
            raise ValueError(f"C22 binding drift: {binding['path']}")
...
def evaluate(
    ...
    if output_dir.exists():
        raise ValueError("C22 output directory must be create-only")
    ...
    with tempfile.TemporaryDirectory(prefix="u1_4c22_", dir=output_dir) as temporary:
        ...
        with tifffile.TiffFile(input_path) as document:
            ...
            if (
                ...
                raise ValueError("C22 decoded source contract drift")
            encoded = np.memmap(
        ...
        result: dict[str, Any] = {
            "schema": "neuro-film.u1-4c22-native-safe-lab-pointwise-result.v1",
