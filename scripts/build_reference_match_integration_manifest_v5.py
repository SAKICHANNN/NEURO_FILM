"""Build the non-mutating P70 integration manifest and strict schema."""
...
if __package__:
    ...
    from scripts.build_reference_match_integration_manifest_v4 import (
        REQUIRED_CONTRACT_SCHEMA_PATHS as V4_SCHEMA_PATHS,
        REQUIRED_PUBLIC_EXPORTS as V4_PUBLIC_EXPORTS,
    )
    ...
    from build_reference_match_integration_manifest_v4 import (
        REQUIRED_CONTRACT_SCHEMA_PATHS as V4_SCHEMA_PATHS,
        REQUIRED_PUBLIC_EXPORTS as V4_PUBLIC_EXPORTS,
    )
...
SCHEMA_ID = "neuro-film.reference-match-main-integration-manifest.v5"
CLAIM_CEILING = "review-ready-not-merged"
PAYLOAD_SCOPE = "P1-P69"
PRIOR_MANIFEST_PATH = (
    "configs/reference_match_main_integration_manifest_v4.json"
)
PRIOR_MANIFEST_SCHEMA_ID = (
    "neuro-film.reference-match-main-integration-manifest.v4"
)
...
MATCH_VIEW_SCHEMA_PATH = (
    "configs/schemas/reference_runtime_staging_match_view_bridge_v1.schema.json"
)
MATCH_VIEW_EXPORTS = (
...
REQUIRED_PUBLIC_EXPORTS = (*V4_PUBLIC_EXPORTS, *MATCH_VIEW_EXPORTS)
...
def build_schema(*, v4_schema_path: Path = V4_SCHEMA_PATH) -> dict[str, Any]:
    try:
        schema = json.loads(v4_schema_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise IntegrationManifestError(
            f"v4 integration schema unavailable: {v4_schema_path}"
        ) from exc
    result = deepcopy(schema)
    result["$id"] = (
        "https://neuro-film.local/schemas/"
        "reference-match-main-integration-manifest-v5"
    )
    result["title"] = "Neuro-Film Reference Match Main Integration Manifest V5"
    ...
    contract["prefixItems"].insert(
        6,
        {
            "allOf": [
                ...
                {
                    "properties": {
                        "path": {"const": MATCH_VIEW_SCHEMA_PATH}
                    }
                },
            ]
        },
