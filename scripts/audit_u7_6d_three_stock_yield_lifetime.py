#!/usr/bin/env python3
"""Measure prior-output lifetime in the 24MP three-stock generator."""
...
from scripts.audit_u7_6c_three_stock_decode_lifetime import (
    _decoded_sha256,
...
DEFAULT_OUTPUT = ROOT / "outputs/eval/u7_6d_three_stock_yield_lifetime_result.json"
SCRATCH = ROOT / "outputs/eval/u7_6d_three_stock_yield_lifetime_audit"
...
def run_worker(mode: str, output_directory: Path) -> dict[str, Any]:
    ...
    if mode == "baseline":
        product_iterator = batch_module.iter_three_stock_look_rgb_shared_context

        def retaining_iterator(*args, **kwargs) -> Iterator:
            iterator = product_iterator(*args, **kwargs)
            prior_output = None
            while True:
                try:
                    item = next(iterator)
                ...
                prior_output = item[1]
                yield item
                # This read keeps the audit-owned reference observably live
                # across the next request and prevents dead-store elimination.
                if prior_output is None:
                    raise RuntimeError("baseline retention unexpectedly absent")

        batch_module.iter_three_stock_look_rgb_shared_context = retaining_iterator
    elif mode != "candidate":
...
def evaluate_rows(rows: list[dict[str, Any]], gates: dict[str, Any]) -> dict[str, Any]:
    ...
    gate_results = {
        "prior_output_collectable_before_next_render_entry": True,
