#!/usr/bin/env python3
"""Render and evaluate the frozen U5.R2I0 sensitometry-print frontier."""
...
def main(default_config: Path | None = None) -> int:
    ...
    if args.render_dir is not None:
        ...
        print(json.dumps({"manifest": str(result["manifest_path"]), "sha256": result["manifest_sha256"], "records": len(result["manifest"]["records"])}, indent=2, sort_keys=True))
        return 0
    ...
    if args.build_visual and result["shortlist"]:
        visual = {"status": "pending_blind_review", **build_blind_sheets(root=ROOT, config=config, manifest_path=manifest, shortlist=result["shortlist"], output_dir=output.parent / "visual")}
    commit = subprocess.run(["git", "rev-parse", "HEAD"], cwd=ROOT, check=True, capture_output=True, text=True).stdout.strip()
    report = {"schema_version": 1, "experiment_id": config["experiment_id"], "software_commit": commit, "config_sha256": sha256_file(args.config), "manifest_sha256": sha256_file(manifest), **result, "visual": visual, "claim_ceiling": config["claim_ceiling"]}
    encoded = (json.dumps(report, indent=2, sort_keys=True) + "\n").encode()
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_bytes(encoded)
    print(json.dumps({"report": str(output), "sha256": hashlib.sha256(encoded).hexdigest(), "automatic_survivors": result["automatic_survivors"], "shortlist": result["shortlist"], "automatic_decision": result["automatic_decision"]}, indent=2, sort_keys=True))
    return 0
