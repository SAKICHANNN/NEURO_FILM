#!/usr/bin/env python
"""Run the frozen U5.R2AG1 bounded convex-gradient representation audit."""
...
from src.roll2film.bounded_convex_gradient import (  # noqa: E402
...
def _target_functions(
    ...
    for name, target in config["targets"].items():
        ...
        if family == "identity":
            ...
            operator = operator_from_config(density["witnesses"][target["witness"]])
            functions[name] = (
...
def _fit_operator(
    *,
    ...
    initial_anchors = np.asarray(candidate["initial_anchors"], dtype=np.float64)
    common = {
        "temperature": float(candidate["temperature"]),
        "maximum_strength": float(candidate["maximum_strength"]),
        "maximum_absolute_centered_bias": float(
            candidate["maximum_absolute_centered_bias"]
        ),
    }
    if target_name == "identity":
        return BoundedConvexGradientMap.identity(
            anchors=initial_anchors,
            **common,
        )
    fit = config["fit"]
    return fit_bounded_convex_gradient_map(
        fit_source,
        fit_target,
        ...
        bias_l2=float(fit["bias_l2"]),
        anchor_l2_to_initial=float(fit["anchor_l2_to_initial"]),
...
def run_audit(config: dict[str, Any], config_sha256: str) -> dict[str, Any]:
    ...
    jacobian_axis = np.linspace(
        0.0, 1.0, int(config["jacobian_grid_axis_size"]), dtype=np.float64
    )
    ...
    for target_name, function in functions.items():
        ...
        jacobians = operator.jacobians(jacobian_points)
        eigenvalues = np.linalg.eigvalsh(jacobians)
        ...
        restored = operator.inverse(
            prediction,
            maximum_iterations=int(inverse["maximum_iterations"]),
            residual_tolerance=float(inverse["residual_tolerance"]),
            minimum_line_search_scale=float(inverse["minimum_line_search_scale"]),
        )
        reports[target_name] = {
            "anchor_count": int(operator.anchors.shape[0]),
            "raw_fitted_scalar_count": operator.raw_fitted_scalar_count,
            "strength": operator.strength,
            "temperature": operator.temperature,
            "maximum_absolute_centered_bias": float(
                np.max(np.abs(operator.centered_biases))
            ),
            ...
            "improvement_over_global_affine_fraction": (
                ...
                if affine_rmse > 1e-15
                else 0.0
            ),
            "output_minimum": float(np.min(prediction)),
            "output_maximum": float(np.max(prediction)),
            "minimum_analytic_jacobian_eigenvalue": float(np.min(eigenvalues)),
            "minimum_analytic_jacobian_determinant": float(np.min(determinants)),
            "maximum_analytic_jacobian_spectral_norm": float(np.max(eigenvalues)),
    ...
    checks = {
        "parameter_budget": all(
            report["raw_fitted_scalar_count"]
            <= gates["fitted_scalar_count_maximum"]
        ...
        "bounded_without_clamp": all(
            report["output_minimum"] >= gates["output_minimum"]
            and report["output_maximum"] <= gates["output_maximum"]
            for report in reports.values()
        ),
        "bias_bound": all(
            report["maximum_absolute_centered_bias"]
            <= gates["maximum_absolute_centered_bias"]
            for report in reports.values()
        ),
        "strength_bound": all(
            report["strength"] <= gates["maximum_strength"]
            for report in reports.values()
        ),
        "positive_definite_jacobian": all(
            report["minimum_analytic_jacobian_eigenvalue"]
            >= gates["minimum_analytic_jacobian_eigenvalue"]
            - 1e-12
            for report in reports.values()
        ),
        "positive_jacobian_determinant": all(
            report["minimum_analytic_jacobian_determinant"]
            >= gates["minimum_analytic_jacobian_determinant"]
            - 1e-12
            for report in reports.values()
        ),
        "bounded_jacobian_norm": all(
            report["maximum_analytic_jacobian_spectral_norm"]
            <= gates["maximum_analytic_jacobian_spectral_norm"]
            for report in reports.values()
        ),
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=(
            ROOT / "configs/u5_r2ag1_bounded_convex_gradient_representation_v1.json"
        ),
    )
    parser.add_argument(
        "--output",
        ...
        default=(
            ROOT
            / "outputs/u5_r2ag1_bounded_convex_gradient_representation_v1/report.json"
        ),
    )
