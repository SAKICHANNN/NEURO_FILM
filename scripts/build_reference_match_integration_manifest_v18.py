"""Build the non-mutating P112 integration manifest and strict schema."""
...
if __package__:
    ...
    from scripts.build_reference_match_integration_manifest_v17 import (
        REQUIRED_CONTRACT_SCHEMA_PATHS as V17_CONTRACT_SCHEMA_PATHS,
        REQUIRED_PUBLIC_EXPORTS as V17_PUBLIC_EXPORTS,
    )
    ...
    from build_reference_match_integration_manifest_v17 import (
        REQUIRED_CONTRACT_SCHEMA_PATHS as V17_CONTRACT_SCHEMA_PATHS,
        REQUIRED_PUBLIC_EXPORTS as V17_PUBLIC_EXPORTS,
    )
...
SCHEMA_ID = "neuro-film.reference-match-main-integration-manifest.v18"
CLAIM_CEILING = "review-ready-not-merged"
PAYLOAD_SCOPE = "P1-P111"
PRIOR_MANIFEST_PATH = (
...
REQUIRED_CONTRACT_SCHEMA_PATHS = (
    *V17_CONTRACT_SCHEMA_PATHS,
    "configs/schemas/reference_hdr_shot_reuse_veto_decision_v1.schema.json",
)
...
def build_schema(*, v17_schema_path: Path = V17_SCHEMA_PATH) -> dict[str, Any]:
    try:
        schema = json.loads(v17_schema_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise IntegrationManifestError(
            f"v17 integration schema unavailable: {v17_schema_path}"
        ) from exc
    result = deepcopy(schema)
    result["$id"] = (
        "https://neuro-film.local/schemas/"
        "reference-match-main-integration-manifest-v18"
    )
    result["title"] = (
