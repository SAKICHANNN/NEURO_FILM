"""Run U6.P8BT dual-compiler conformance and 12MP resource evidence."""
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise TypeError("P8BT JSON must be an object")
    return payload


def _worker(contract_path: Path, dll: Path, result_path: Path) -> None:
    contract = _load_json(contract_path)
    p8bs = _load_json(ROOT / "configs/u6_p8bs_native_thomas_field_v1.json")
    profile = profile_from_contract(p8bs)
    ...
    output, raw_mean, workspace_bytes = render_native_thomas_transmittance(
        library, profile, base, sigma
    )
    elapsed = time.perf_counter() - started
    result = {
        "schema": "neuro_film.u6_p8bt_native_thomas_density_worker.v1",
        "shape": list(shape),
    ...
    while process.poll() is None:
        if time.perf_counter() - started > timeout_seconds:
            for child in root_process.children(recursive=True):
                child.kill()
            root_process.kill()
            raise TimeoutError("P8BT worker exceeded timeout")
        try:
...
def evaluate(contract_path: Path, output_dir: Path, clang: Path) -> dict[str, Any]:
    ...
    stable = {
        "schema": "neuro_film.u6_p8bt_native_density_thomas_result.v1",
...
def main() -> None:
    ...
    parser.add_argument(
        "--contract",
        type=Path,
        default=ROOT / "configs/u6_p8bt_native_density_thomas_transmittance_v1.json",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "outputs/eval/u6_p8bt_native_density_thomas_v1",
    )
    parser.add_argument(
        "--clang",
        type=Path,
