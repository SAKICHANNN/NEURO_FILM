#!/usr/bin/env python3
"""Benchmark exact U6.P8I per-halo-tile physical linear decoding."""
...
SCHEMA = "neuro_film.u6_p8i_tile_linear_decode_resources_contract.v1"
...
def validate_contract(config: dict[str, Any]) -> dict[str, Any]:
    expected = {
        "full_frame_linear_decode_allowed": False,
        "linear_decode_scope": "physical-halo-tile",
        "encoded_source_retained_for_exact_source_context": True,
        "per_element_decode_arithmetic_change_allowed": False,
    ...
    if (
        ...
        raise ValueError("unsupported U6.P8I contract")
    ...
    if (
        not decision["next_leaf"].startswith("U6.P8I")
        or decision["performance_target_pass"]
        or decision["production_default_changed"]
    ):
        raise ValueError("U6.P8I parent decision drift")
    return parent
...
def evaluate(
    ...
    stable = {
        "schema": (
            "neuro_film.u6_p8i_tile_linear_decode_stable_evidence.v1"
        ),
        "decode_mode": config["implementation"],
    ...
    return {
        "schema": (
            "neuro_film.u6_p8i_tile_linear_decode_resources_report.v1"
        ),
        "node": config["node"],
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=ROOT
        / "configs/u6_p8i_tile_linear_decode_resources_v1.json",
    )
