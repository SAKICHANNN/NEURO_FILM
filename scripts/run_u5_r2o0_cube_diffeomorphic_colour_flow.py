#!/usr/bin/env python
"""Run the frozen U5.R2O0 cube-diffeomorphic colour-flow audit."""
...
def _fit_operator(
    *,
    ...
    if target_name == "identity":
        return CubeDiffeomorphicColourFlow.identity(
    ...
    return fit_cube_diffeomorphic_colour_flow(
        ...
        maximum_absolute_coefficient=float(
            candidate["maximum_absolute_velocity_grid_coefficient"]
        ),
        seed=int(fit["seed"]),
        steps=int(fit["steps"]),
...
def run_audit(config: dict[str, Any], config_sha256: str) -> dict[str, Any]:
    ...
    for target_name, function in functions.items():
        ...
        corner_error = float(np.max(np.abs(operator.apply(_corners()) - _corners())))
        reports[target_name] = {
            "velocity_grid_axis_size": int(operator.velocity_grid.shape[0]),
            "parameter_count": int(operator.velocity_grid.size),
            "integration_steps": operator.integration_steps,
            "maximum_absolute_velocity_grid_coefficient": float(
                np.max(np.abs(operator.velocity_grid))
            ),
            ...
            "endpoint_maximum_absolute_error": corner_error,
    ...
    checks = {
        ...
        "coefficient_bound": all(
            report["maximum_absolute_velocity_grid_coefficient"]
            <= gates["maximum_absolute_velocity_grid_coefficient"]
            for report in reports.values()
        ),
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/u5_r2o0_cube_diffeomorphic_colour_flow_v1.json",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "outputs/u5_r2o0_cube_diffeomorphic_colour_flow_v1/report.json",
    )
