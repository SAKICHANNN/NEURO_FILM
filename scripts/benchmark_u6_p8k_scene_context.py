#!/usr/bin/env python3
"""Benchmark exact U6.P8K scene-row context and encoded-buffer removal."""
...
SCHEMA = "neuro_film.u6_p8k_scene_context_resources_contract.v1"
...
def validate_contract(config: dict[str, Any]) -> dict[str, Any]:
    expected = {
        "context_input": "scene-linear-row-staged",
        "context_encoded_float32_semantics": "exact-p8g",
        "retained_full_encoded_source": False,
        "physical_input": "one-full-frame-encoded-roundtrip-linear",
        "gauged_output": "one-reused-full-frame-float64",
    ...
    if (
        ...
        raise ValueError("unsupported U6.P8K contract")
    ...
    if (
        not decision["next_leaf"].startswith("U6.P8K")
        or decision["standalone_resource_improvement"]
        or not decision["retained_only_for_next_combination"]
        or decision["production_default_changed"]
    ):
        raise ValueError("U6.P8K parent decision drift")
    return measurement
...
def evaluate(
    ...
    stable = {
        "schema": (
            "neuro_film.u6_p8k_scene_context_stable_evidence.v1"
        ),
    ...
    return {
        "schema": (
            "neuro_film.u6_p8k_scene_context_resources_report.v1"
        ),
        "node": config["node"],
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/u6_p8k_scene_context_resources_v1.json",
    )
