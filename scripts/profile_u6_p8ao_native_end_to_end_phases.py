#!/usr/bin/env python3
"""Attribute 12MP native end-to-end runtime by execution phase."""
...
SCHEMA = "neuro_film.u6_p8ao_native_end_to_end_phases_contract.v1"
...
def validate_contract(config: dict[str, Any]) -> None:
    if (
        ...
        raise ValueError("unsupported P8AO contract")
    parent = _load_exact_json(
        ROOT / config["parent_decision"],
        config["parent_decision_sha256"],
    )
    if not str(parent.get("next_leaf", "")).startswith("U6.P8AO"):
        raise ValueError("P8AO parent decision drift")
    _load_exact_json(
...
def _profile_once(
    *,
    ...
    context_library = _load_context(Path(builds["context"]["dll_path"]))
    display_library = _load_display(Path(builds["display"]["dll_path"]))
    ...
    phases = {
        ...
        "context_native_update_seconds": 0.0,
        ...
        "ao6_native_display_seconds": 0.0,
    ...
    if context_library.nf_ao6_context_f32_init_v1(
        ...
        raise RuntimeError("P8AO context init failed")
    for y0 in range(0, height, tile_rows):
        ...
        phases["context_native_update_seconds"] += (
            time.perf_counter() - phase_started
        )
        if status != 0:
            raise RuntimeError(f"P8AO context update failed: {status}")
    ...
    if context_library.nf_ao6_context_f32_finalize_v1(
        ...
        raise RuntimeError("P8AO context finalize failed")
    ...
    def output_sink(y0: int, y1: int, rows: np.ndarray) -> None:
        ...
        phases["ao6_native_display_seconds"] += (
            time.perf_counter() - phase_started
        )
        phase_started = time.perf_counter()
    ...
    if consumed_rows != height:
        raise RuntimeError("P8AO did not consume every output row")
...
def profile(config: dict[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    output_exact = len({run["output_sha256"] for run in runs}) == 1
    ...
    stable = {
        "schema": "neuro_film.u6_p8ao_native_end_to_end_phases_result.v1",
        "scenario": config["scenario"]["scenario_id"],
        "height": int(config["scenario"]["height"]),
        "width": int(config["scenario"]["width"]),
        "tile_rows": int(config["tile_rows"]),
        "repeat_count": len(runs),
        "output_repeat_exact": output_exact,
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        default="configs/u6_p8ao_native_end_to_end_phases_v1.json",
    )
    parser.add_argument(
        "--output-dir",
        default="outputs/u6_p8ao_native_end_to_end_phases_v1",
    )
