"""Run the frozen U2.2B composition audit twice."""
...
from src.eval.sensitometry_print_composition import evaluate_composition, result_hashes  # noqa: E402
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=ROOT / "configs/u2_2b_sensitometry_print_composition_v1.json")
    ...
    parser.add_argument("--output-dir", type=Path, default=ROOT / "outputs/u2_2b_sensitometry_print_composition/formal")
    args = parser.parse_args()
    config = load_json(args.config)
    parent = load_json(args.parent_config)
    print_config = load_json(args.print_config)
    repeats = []
    for _ in range(int(config["gates"]["exact_repeats"])):
        report, arrays = evaluate_composition(config, parent, print_config)
        repeats.append((report, arrays, result_hashes(report, arrays)))
    if any(value[2] != repeats[0][2] for value in repeats[1:]):
        raise RuntimeError("U2.2B exact repeats differ")
