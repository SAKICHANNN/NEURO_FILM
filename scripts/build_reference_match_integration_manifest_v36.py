"""Build the P151 integration manifest."""
...
if __package__:
    ...
    from build_reference_match_integration_manifest_v34 import V34_REQUIRED_CONTRACT_SCHEMA_PATHS, V34_REQUIRED_PUBLIC_EXPORTS
...
SCHEMA_ID="neuro-film.reference-match-main-integration-manifest.v36";CLAIM_CEILING="review-ready-not-merged";PAYLOAD_SCOPE="P1-P151"
PRIOR_MANIFEST_PATH="configs/reference_match_main_integration_manifest_v35.json";PRIOR_MANIFEST_SCHEMA_ID="neuro-film.reference-match-main-integration-manifest.v35"
ROOT=Path(__file__).resolve().parents[1];V35_SCHEMA_PATH=ROOT/"configs/schemas/reference_match_main_integration_manifest_v35.schema.json";DEFAULT_SCHEMA_PATH=ROOT/"configs/schemas/reference_match_main_integration_manifest_v36.schema.json"
...
def build_schema(*,v35_schema_path:Path=V35_SCHEMA_PATH)->dict[str,Any]:
    try:result=deepcopy(json.loads(v35_schema_path.read_text(encoding="utf-8")))
    except (OSError,json.JSONDecodeError) as exc:raise IntegrationManifestError("v35 integration schema unavailable") from exc
    result["$id"]="https://neuro-film.local/schemas/reference-match-main-integration-manifest-v36";result["title"]="Neuro-Film Reference Match Main Integration Manifest V36";result["properties"]["schema_id"]["const"]=SCHEMA_ID;result["properties"]["payload_scope"]["const"]=PAYLOAD_SCOPE
...
def build_manifest(*,repo:Path,payload_commit:str,base_commit:str,main_commit:str)->dict[str,Any]:return build_versioned_manifest(repo=repo,payload_commit=payload_commit,base_commit=base_commit,main_commit=main_commit,schema_id=SCHEMA_ID,payload_scope=PAYLOAD_SCOPE,required_public_exports=V34_REQUIRED_PUBLIC_EXPORTS,required_contract_schema_paths=V34_REQUIRED_CONTRACT_SCHEMA_PATHS,prior_manifest_path=PRIOR_MANIFEST_PATH,prior_manifest_schema_id=PRIOR_MANIFEST_SCHEMA_ID,verification_commands=VERIFICATION_COMMANDS,claim_ceiling=CLAIM_CEILING)
