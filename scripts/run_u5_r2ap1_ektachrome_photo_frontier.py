#!/usr/bin/env python
"""Render and evaluate the frozen AP1 Ektachrome photo frontier."""
...
CONFIG_SHA256 = "bcf9ba1068b695243d7386dab4ddd5e4f1afdf137cdfa70ca1b1970977f19c8d"
...
def load_config(path: Path, *, expected_sha256: str) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("AP1 frozen contract mismatch")
    return config
...
def _require_clean_tracked_worktree() -> None:
    for command in (
        ...
        if subprocess.run(command, cwd=ROOT, check=False).returncode != 0:
            raise RuntimeError("AP1 requires a clean tracked worktree")
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/u5_r2ap1_ektachrome_photo_frontier_v1.json",
    )
