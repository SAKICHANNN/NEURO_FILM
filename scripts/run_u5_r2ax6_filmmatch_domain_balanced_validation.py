"""Render the fixed AX5 operator on the fit-forbidden validation scene."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=ROOT
        / "configs/u5_r2ax6_filmmatch_domain_balanced_validation_v1.json",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    if (
        ...
        raise ValueError("AX5 parent report hash drift")
    parent = json.loads(parent_raw)
    if (
        parent["stable_evidence_id"]
        != config["parent"]["stable_evidence_id"]
    ):
        raise ValueError("AX5 parent stable identity drift")
