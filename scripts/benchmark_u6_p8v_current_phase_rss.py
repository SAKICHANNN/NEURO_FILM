#!/usr/bin/env python3
"""Re-attribute RSS through the exact current P8U production entry."""
...
SCHEMA = "neuro_film.u6_p8v_current_phase_rss_contract.v1"
...
def validate_contract(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("unsupported U6.P8V contract")
...
def _run_worker(config: dict[str, Any], output: Path) -> None:
    ...
    sampler = _PhaseSampler(
        [str(value) for value in config["measurement"]["phase_order"]],
    ...
    original_encode = profile_consumer._linear_srgb_to_encoded_row_staged
    original_context = (
        profile_consumer.build_density_source_context_inplace_packed_lab
    )
    original_reconstruct = profile_consumer.reconstruct_standalone_runtime
    original_refill = (
        profile_consumer._refill_roundtrip_linear_inplace_row_staged
    )
    ...
    def context_wrapper(
        ...
        *,
        ...
        return original_context(payload, source, tile_rows=tile_rows)
    ...
    def refill_wrapper(
        ...
        *,
        ...
        return original_refill(
            consumed, original_linear, tile_rows=tile_rows
        )
    ...
    try:
        ...
        working = WorkingImage(
            ...
            source_profile=SourceProfile(
                "raw_metadata", "U6.P8V analytic scene"
            ),
            hdr_metadata={},
            orientation_applied=True,
            alpha_policy="absent",
            bit_depth_in=32,
            source_path=Path("u6-p8v-analytic.scene-linear"),
        )
        rendered, receipt = (
        ...
        if output_sha256 != receipt["output"]["array_sha256"]:
            raise RuntimeError("P8V receipt/output identity mismatch")
    finally:
...
def evaluate(config: dict[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    if available < int(
        ...
        raise RuntimeError("available memory is below P8V launch floor")
    scenario = config["scenario"]
    ...
    for repeat in range(int(scenario["repeats"])):
        ...
        command = [
            ...
            str(worker_output),
            "--config",
            str(ROOT / "configs/u6_p8v_current_phase_rss_v1.json"),
        ]
        observed = _monitor(
            command,
    ...
    stable = {
        "schema": "neuro_film.u6_p8v_current_phase_rss_stable_evidence.v1",
    ...
    return {
        "schema": "neuro_film.u6_p8v_current_phase_rss_report.v1",
        "node": config["node"],
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/u6_p8v_current_phase_rss_v1.json",
    )
