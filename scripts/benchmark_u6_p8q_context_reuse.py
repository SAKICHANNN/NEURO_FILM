#!/usr/bin/env python3
"""Benchmark exact U6.P8Q context-first encoded-buffer reuse."""
...
SCHEMA = "neuro_film.u6_p8q_context_reuse_resources_contract.v1"
...
def validate_contract(config: dict[str, Any]) -> dict[str, Any]:
    expected = {
        "source_context_schedule": "before-physical-execution",
        "encoded_source_allocation": (
            "destructively-reused-for-roundtrip-linear"
        ),
        "roundtrip_linear_execution": "row-bounded-exact-eotf",
        "roundtrip_linear_tile_rows": 128,
        "display_source_context": "exact-precomputed-safe-lab-context",
        "display_output_reuses_consumed_float64_input": True,
        "pointwise_arithmetic_change_allowed": False,
    ...
    if (
        ...
        raise ValueError("unsupported U6.P8Q contract")
    ...
    if (
        not decision["next_leaf"].startswith("U6.P8Q")
        ...
        raise ValueError("U6.P8Q parent decision drift")
    return measurement
...
def evaluate(
    ...
    stable = {
        "schema": "neuro_film.u6_p8q_context_reuse_stable_evidence.v1",
    ...
    return {
        "schema": "neuro_film.u6_p8q_context_reuse_resources_report.v1",
        "node": config["node"],
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=ROOT
        / "configs/u6_p8q_context_reuse_resources_v1.json",
    )
