#!/usr/bin/env python
"""Run the frozen U5.R2K2 rational-quadratic coupling audit."""
...
from src.roll2film.rational_quadratic_coupling import (  # noqa: E402
...
def _best_global_affine(
    ...
    confirm_design = np.column_stack(
        (np.ones(len(confirm_source)), confirm_source)
    )
...
def run_audit(config: dict[str, Any], config_sha256: str) -> dict[str, Any]:
    ...
    for target_name, function in functions.items():
        ...
        operator = fit_rational_quadratic_coupling(
            fit_source,
            fit_target,
            ...
            minimum_bin_width=float(candidate["minimum_bin_width"]),
            minimum_bin_height=float(candidate["minimum_bin_height"]),
            minimum_derivative=float(candidate["minimum_derivative"]),
        ...
        stage_statistics = operator.stage_statistics(jacobian_points)
        reports[target_name] = {
            "stage_count": len(operator.stage_channels),
            "bin_count": operator.bin_count,
            "parameter_count": int(operator.coefficients.size),
            "maximum_absolute_coefficient": float(
                np.max(np.abs(operator.coefficients))
            ),
            "minimum_observed_bin_width": stage_statistics[
                "minimum_bin_width"
            ],
            "minimum_observed_bin_height": stage_statistics[
                "minimum_bin_height"
            ],
            ...
            "minimum_analytic_stage_derivative": stage_statistics[
                "minimum_stage_derivative"
            ],
    ...
    checks = {
        ...
        "bin_floors": all(
            report["minimum_observed_bin_width"]
            >= gates["minimum_observed_bin_width"]
            and report["minimum_observed_bin_height"]
            >= gates["minimum_observed_bin_height"]
            for report in reports.values()
        ),
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/u5_r2k2_rational_quadratic_coupling_v1.json",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT
        / "outputs/u5_r2k2_rational_quadratic_coupling_v1/report.json",
    )
