#!/usr/bin/env python
"""Run the frozen U5.R2E0 density-domain primitive audit."""
...
from src.roll2film.density_domain import (  # noqa: E402
...
def run_audit(config: dict[str, Any], config_sha256: str) -> dict[str, Any]:
    ...
    for name, payload in config["witnesses"].items():
        ...
        replay = DensityDomainNegativePrintOperator.from_dict(
    ...
    non_neutral = [
        ...
        if name != "neutral_density_reference"
    ]
    checks = {
        "endpoints": all(
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/u5_r2e0_density_domain_operator_v1.json",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "outputs/u5_r2e0_density_domain_operator_v1/audit.json",
    )
