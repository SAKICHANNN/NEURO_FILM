"""Run dense numerical and visual stress on the fixed AX5 operator."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/u5_r2ax7_filmmatch_dense_safety_stress_v1.json",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    if (
        ...
        raise ValueError("AX6 decision identity drift")
    ...
    if (
        ...
        raise ValueError("AX5 operator report identity drift")
    parent = json.loads(parent_raw)
    if (
        parent["stable_evidence_id"]
        != config["parent"]["operator_stable_evidence_id"]
    ):
        raise ValueError("AX5 operator stable identity drift")
