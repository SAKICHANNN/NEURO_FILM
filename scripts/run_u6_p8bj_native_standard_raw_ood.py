#!/usr/bin/env python3
"""Render a bounded multi-camera RAW cohort and build a review sheet."""
...
SCHEMA = "neuro_film.u6_p8bj_native_standard_raw_ood_contract.v1"


def run(config: dict[str, Any], output_dir: Path) -> dict[str, Any]:
    if config.get("schema") != SCHEMA or len(config["rows"]) != 4:
        raise ValueError("unsupported P8BJ contract")
    ...
    if (
        parent["result"]["status"] != "pass"
        or not str(parent["next_leaf"]).startswith("U6.P8BJ")
    ):
        raise ValueError("P8BJ parent drift")
    for row in config["rows"]:
        if hashlib.sha256(
            (ROOT / row["path"]).read_bytes()
        ).hexdigest() != row["sha256"]:
            raise ValueError("P8BJ RAW identity drift")
    p8aw._patch_runtime()
    ...
    for row in config["rows"]:
        ...
        input_preview.thumbnail((640, 420))
        paths = {
        ...
        output_preview.thumbnail((640, 420))
    ...
    report = {
        "schema": "neuro_film.u6_p8bj_native_standard_raw_ood_result.v1",
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        default="configs/u6_p8bj_native_standard_raw_ood_v1.json",
    )
    parser.add_argument(
        "--output-dir",
        default="outputs/u6_p8bj_native_standard_raw_ood_v1",
    )
