"""Build the non-mutating P114 integration manifest and strict schema."""
...
if __package__:
    ...
    from build_reference_match_integration_manifest_v18 import (
        REQUIRED_CONTRACT_SCHEMA_PATHS,
        REQUIRED_PUBLIC_EXPORTS,
    )
...
SCHEMA_ID = "neuro-film.reference-match-main-integration-manifest.v19"
CLAIM_CEILING = "review-ready-not-merged"
PAYLOAD_SCOPE = "P1-P113"
PRIOR_MANIFEST_PATH = (
    "configs/reference_match_main_integration_manifest_v18.json"
)
PRIOR_MANIFEST_SCHEMA_ID = (
    "neuro-film.reference-match-main-integration-manifest.v18"
)
...
def build_schema(*, v18_schema_path: Path = V18_SCHEMA_PATH) -> dict[str, Any]:
    try:
        schema = json.loads(v18_schema_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise IntegrationManifestError(
            f"v18 integration schema unavailable: {v18_schema_path}"
        ) from exc
    result = deepcopy(schema)
    result["$id"] = (
        "https://neuro-film.local/schemas/"
        "reference-match-main-integration-manifest-v19"
    )
    result["title"] = (
        "Neuro-Film Reference Match Main Integration Manifest V19"
    )
