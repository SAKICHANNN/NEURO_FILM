...
from src.eval.physical_interimage_photographic_stress import (  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=(
            ROOT / "configs" / "u6_p5g_interimage_photographic_stress_v1.json"
        ),
    )
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--overview", type=Path, required=True)
    parser.add_argument("--worst-patches", type=Path, required=True)
    ...
    _exact_json(
        ...
        contract["parents"]["p5d_decision_sha256"],
        "P5D decision",
    )
    p5d_contract = _exact_json(
        ROOT / contract["input"]["reuse_p5d_contract"],
        contract["input"]["reuse_p5d_contract_sha256"],
        "P5D contract",
    )
    ...
    report = evaluate_interimage_photographic_stress(
        contract,
        manifest,
        p5d_contract,
        p5c_contract,
        ...
        worst_patches_path=args.worst_patches,
    )
    ...
    print(f"overview_sha256={report['overview_sha256']}")
    print(f"worst_patches_sha256={report['worst_patches_sha256']}")
