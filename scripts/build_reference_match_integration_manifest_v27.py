"""Build the P136 integration manifest."""
...
if __package__:
    ...
    from scripts.build_reference_match_integration_manifest_v25 import (
    ...
    from build_reference_match_integration_manifest_v25 import (
        REQUIRED_CONTRACT_SCHEMA_PATHS,
        REQUIRED_PUBLIC_EXPORTS,
    )
...
SCHEMA_ID = "neuro-film.reference-match-main-integration-manifest.v27"
CLAIM_CEILING = "review-ready-not-merged"
PAYLOAD_SCOPE = "P1-P135"
PRIOR_MANIFEST_PATH = "configs/reference_match_main_integration_manifest_v26.json"
PRIOR_MANIFEST_SCHEMA_ID = "neuro-film.reference-match-main-integration-manifest.v26"
ROOT = Path(__file__).resolve().parents[1]
V26_SCHEMA_PATH = (
    ROOT / "configs/schemas/reference_match_main_integration_manifest_v26.schema.json"
)
DEFAULT_SCHEMA_PATH = (
...
def build_schema(*, v26_schema_path: Path = V26_SCHEMA_PATH) -> dict[str, Any]:
    try:
        result = deepcopy(json.loads(v26_schema_path.read_text(encoding="utf-8")))
    except (OSError, json.JSONDecodeError) as exc:
        raise IntegrationManifestError("v26 integration schema unavailable") from exc
    result["$id"] = (
        "https://neuro-film.local/schemas/"
        "reference-match-main-integration-manifest-v27"
    )
    result["title"] = "Neuro-Film Reference Match Main Integration Manifest V27"
...
def build_manifest(
    *, repo: Path, payload_commit: str, base_commit: str, main_commit: str
) -> dict[str, Any]:
...
def validate_manifest(
    *, repo: Path, manifest: dict[str, Any], schema_path: Path = DEFAULT_SCHEMA_PATH
) -> None:
    validate_versioned_manifest(
        repo=repo,
        manifest=manifest,
...
if __name__ == "__main__":
    ...
    if args.output_schema:
        args.output_schema.write_text(
            encode_json(build_schema()), encoding="utf-8", newline="\n"
        )
