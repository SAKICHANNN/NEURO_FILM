"""Build the P173 integration manifest."""
...
if __package__:
    ...
    from scripts.build_reference_match_integration_manifest_v43 import (
        V43_REQUIRED_CONTRACT_SCHEMA_PATHS,
    ...
    from build_reference_match_integration_manifest_v43 import (
        V43_REQUIRED_CONTRACT_SCHEMA_PATHS,
        V43_REQUIRED_PUBLIC_EXPORTS,
    )
...
SCHEMA_ID = "neuro-film.reference-match-main-integration-manifest.v44"
CLAIM_CEILING = "review-ready-not-merged"
PAYLOAD_SCOPE = "P1-P173"
PRIOR_MANIFEST_PATH = "configs/reference_match_main_integration_manifest_v43.json"
PRIOR_MANIFEST_SCHEMA_ID = (
    "neuro-film.reference-match-main-integration-manifest.v43"
)
ROOT = Path(__file__).resolve().parents[1]
V43_SCHEMA_PATH = (
    ROOT
    / "configs/schemas/reference_match_main_integration_manifest_v43.schema.json"
)
DEFAULT_SCHEMA_PATH = (
    ROOT
    / "configs/schemas/reference_match_main_integration_manifest_v44.schema.json"
)
V44_REQUIRED_PUBLIC_EXPORTS = V43_REQUIRED_PUBLIC_EXPORTS
V44_REQUIRED_CONTRACT_SCHEMA_PATHS = V43_REQUIRED_CONTRACT_SCHEMA_PATHS
...
def build_schema(
    *,
    v43_schema_path: Path = V43_SCHEMA_PATH,
) -> dict[str, Any]:
    try:
        result = deepcopy(
            json.loads(v43_schema_path.read_text(encoding="utf-8"))
        )
    except (OSError, json.JSONDecodeError) as exc:
        raise IntegrationManifestError(
            "v43 integration schema unavailable"
        ) from exc
    result["$id"] = (
        "https://neuro-film.local/schemas/"
        "reference-match-main-integration-manifest-v44"
    )
    result["title"] = "Neuro-Film Reference Match Main Integration Manifest V44"
...
def build_manifest(
    *,
    ...
    return build_versioned_manifest(
        ...
        required_public_exports=V44_REQUIRED_PUBLIC_EXPORTS,
        required_contract_schema_paths=V44_REQUIRED_CONTRACT_SCHEMA_PATHS,
