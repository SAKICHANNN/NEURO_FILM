#!/usr/bin/env python3
"""Audit v3 NumPy-compatible native safe-Lab pointwise execution."""
...
CONTRACT_SCHEMA = (
    "neuro-film.u7-6j-three-stock-native-pointwise-exact-contract.v1"
)
WORKER_SCHEMA = "neuro-film.u7-6j-three-stock-native-pointwise-exact-worker.v1"
REPORT_SCHEMA = "neuro-film.u7-6j-three-stock-native-pointwise-exact-result.v1"
...
def _load_contract(path: Path) -> tuple[dict[str, Any], str]:
    ...
    if payload.get("schema") != CONTRACT_SCHEMA:
        raise ValueError("U7.6J contract schema drift")
    for binding in payload["bindings"]:
        if _sha256(ROOT / binding["path"]) != binding["sha256"]:
            raise ValueError(f"U7.6J binding drift: {binding['path']}")
    parent = payload["parent_negative"]
    if _sha256(ROOT / parent["evidence_path"]) != parent["evidence_sha256"]:
        raise ValueError("U7.6J parent evidence drift")
    source = payload["source"]
    if _sha256(ROOT / source["path"]) != source["sha256"]:
        raise ValueError("U7.6J source identity drift")
...
def _unload(library: ctypes.CDLL) -> None:
    ...
    if not free_library(library._handle):
        raise RuntimeError("U7.6J failed to unload native library")
...
def run_worker(contract_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    build = build_native_safe_lab_pointwise_v3(root=ROOT, output_dir=build_dir)
    ...
    try:
        ...
        for style_id in contract["styles"]:
            ...
            if resolved_style != style_id:
                raise ValueError("U7.6J stock/style mapping drift")
...
def _stable_id(report: dict[str, Any]) -> str:
    stable = {
        ...
        "gates": {
            ...
            if key not in {"three_style_native_wall", "native_to_python_wall_ratio"}
        },
...
def evaluate(contract_path: Path, output_path: Path) -> dict[str, Any]:
    if output_path.exists():
        raise ValueError("U7.6J output must be create-only")
    contract, contract_sha256 = _load_contract(contract_path)
    scratch = Path(tempfile.mkdtemp(prefix="u7_6j_", dir=ROOT / "tmp"))
    try:
        ...
        for index in range(int(contract["execution"]["formal_processes"])):
            ...
            if completed.stderr:
                raise RuntimeError("U7.6J worker wrote stderr")
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/u7_6j_three_stock_native_pointwise_exact_v1.json",
    )
