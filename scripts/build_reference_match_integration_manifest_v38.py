"""Build the P153 integration manifest."""
...
SCHEMA_ID="neuro-film.reference-match-main-integration-manifest.v38";CLAIM_CEILING="review-ready-not-merged";PAYLOAD_SCOPE="P1-P153"
PRIOR_MANIFEST_PATH="configs/reference_match_main_integration_manifest_v37.json";PRIOR_MANIFEST_SCHEMA_ID="neuro-film.reference-match-main-integration-manifest.v37"
ROOT=Path(__file__).resolve().parents[1];V37_SCHEMA_PATH=ROOT/"configs/schemas/reference_match_main_integration_manifest_v37.schema.json";DEFAULT_SCHEMA_PATH=ROOT/"configs/schemas/reference_match_main_integration_manifest_v38.schema.json"
...
def build_schema(*,v37_schema_path:Path=V37_SCHEMA_PATH)->dict[str,Any]:
    try:result=deepcopy(json.loads(v37_schema_path.read_text(encoding="utf-8")))
    except (OSError,json.JSONDecodeError) as exc:raise IntegrationManifestError("v37 integration schema unavailable") from exc
    result["$id"]="https://neuro-film.local/schemas/reference-match-main-integration-manifest-v38";result["title"]="Neuro-Film Reference Match Main Integration Manifest V38";result["properties"]["schema_id"]["const"]=SCHEMA_ID;result["properties"]["payload_scope"]["const"]=PAYLOAD_SCOPE
