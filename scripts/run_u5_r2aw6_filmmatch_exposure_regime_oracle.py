"""Run the FilmMatch negative/zero/positive exposure-regime Oracle."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=ROOT
        / "configs/u5_r2aw6_filmmatch_exposure_regime_oracle_v1.json",
    )
    parser.add_argument(
        "--output",
        type=Path,
        ...
        "exposure_regime_oracle_report.json",
    )
    ...
    parent_raw = (ROOT / config["parent"]["oracle_report"]).read_bytes()
    if (
        hashlib.sha256(parent_raw).hexdigest()
        != config["parent"]["oracle_report_sha256"]
        or json.loads(parent_raw)["stable_evidence_id"]
        != config["parent"]["oracle_stable_evidence_id"]
    ):
        raise ValueError("AW5 Oracle parent identity drift")
    report = evaluate_exposure_regime_oracle(
        ...
        json.dumps(
            {
                ...
                "luma_proxy_router_research_opened": report[
                    "luma_proxy_router_research_opened"
                ],
