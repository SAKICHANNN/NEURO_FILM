#!/usr/bin/env python3
"""Run U6.P8BY exact non-retaining row-sink evidence."""
...
def _json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise TypeError("P8BY JSON must be an object")
    return payload
...
def _validate_contract(contract_path: Path, contract: dict[str, Any]) -> None:
    if (
        contract.get("schema")
        != "neuro_film.u6_p8by_native_thomas_row_sink_contract.v1"
        ...
        or contract["candidate"].get("output_array_allowed") is not False
        or contract["candidate"].get("model_or_profile_change_allowed") is not False
    ):
        raise RuntimeError("P8BY contract drift")
    del contract_path
    for binding in contract["parents"].values():
        ...
        if (
            ...
            raise RuntimeError("P8BY parent drift")
...
class _HashSink:
    ...
    def __call__(self, channel: int, row_start: int, values: np.ndarray) -> None:
        ...
        self.total.update(payload)
        self.channels[channel].update(payload)
...
def _worker(
    ...
    means, workspace_bytes, calls = stream_native_exposure_thomas_rgb_rows(
    ...
    if hashlib.sha256(exposure.tobytes()).hexdigest() != input_sha:
        raise RuntimeError("P8BY row sink mutated its input")
    result = {
        "schema": "neuro_film.u6_p8by_native_thomas_row_sink_worker.v1",
        "shape_chw": list(shape),
        "input_sha256": input_sha,
        "output_sha256": sink.total.hexdigest(),
        ...
        "sink_calls": calls,
        "sink_order_exact": sink.order == sorted(sink.order),
    ...
    while process.poll() is None:
        if time.perf_counter() - started > timeout:
            for child in root_process.children(recursive=True):
                child.kill()
            root_process.kill()
            raise TimeoutError("P8BY worker exceeded timeout")
        try:
            total = 0
...
def evaluate(contract_path: Path, output_dir: Path, clang: Path) -> dict[str, Any]:
    contract = _json(contract_path)
    _validate_contract(contract_path, contract)
    ...
    for name, rows_library in row_libraries.items():
        ...
        exact = exact and row["output_sha256"] == hashlib.sha256(
            expected.tobytes()
        ).hexdigest()
    ...
    def fail_sink(channel: int, row_start: int, values: np.ndarray) -> None:
        nonlocal failure_calls
        del channel, row_start, values
        failure_calls += 1
        if failure_calls == 2:
            raise RuntimeError("injected P8BY sink failure")
    ...
    try:
        ...
        sink_failure = str(error) == "injected P8BY sink failure" and failure_calls == 2
    ...
    hashes = [str(row["worker"]["output_sha256"]) for row in runs]
    limits = contract["performance"]
    reduction = int(limits["p8bx_reference_peak_rss_bytes"]) - max(peaks)
    gates = {
        "exact_p8bw_stream": bool(exact),
        ...
        "minimum_rss_reduction": reduction
        >= int(limits["minimum_rss_reduction_bytes_vs_p8bx"]),
    ...
    report = {
        "schema": "neuro_film.u6_p8by_native_thomas_row_sink_result.v1",
        ...
        "performance": {
            ...
            "rss_reduction_bytes_vs_p8bx": reduction,
...
def main() -> None:
    ...
    parser.add_argument(
        "--contract",
        type=Path,
        default=ROOT / "configs/u6_p8by_native_thomas_row_sink_v1.json",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "outputs/eval/u6_p8by_native_thomas_row_sink_v1",
    )
    parser.add_argument(
        "--clang",
        type=Path,
