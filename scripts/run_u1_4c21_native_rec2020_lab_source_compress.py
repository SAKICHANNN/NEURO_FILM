...
def _load_contract(path: Path, *, root: Path) -> tuple[dict[str, Any], str]:
    ...
    if payload.get("schema") != "neuro-film.u1-4c21-native-rec2020-lab-source-compress.v1":
        raise ValueError("C21 contract schema is invalid")
    for binding in payload["bindings"]:
        bound = root / binding["path"]
        if _sha256(bound) != binding["sha256"]:
            raise ValueError(f"C21 binding drift: {binding['path']}")
...
def evaluate(
    ...
    if output_dir.exists():
        raise ValueError("C21 output directory must be create-only")
    ...
    compress_build = build_native_rec2020_lab_compress(
        root=root, output_dir=output_dir / "build_compress"
    )
    compress_library = load_native_rec2020_lab_compress(
    ...
    with tempfile.TemporaryDirectory(prefix="u1_4c21_", dir=output_dir) as temporary:
        scratch = Path(temporary)
        with tifffile.TiffFile(input_path) as document:
            if len(document.pages) != 1:
                raise ValueError("C21 input must contain one TIFF page")
            ...
            if (
                ...
                raise ValueError("C21 decoded source contract drift")
            encoded = np.memmap(
        ...
        result: dict[str, Any] = {
            "schema": "neuro-film.u1-4c21-native-rec2020-lab-source-compress-result.v1",
