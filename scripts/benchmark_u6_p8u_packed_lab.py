#!/usr/bin/env python3
"""Benchmark exact U6.P8U packed-Lab context storage."""
...
SCHEMA = "neuro_film.u6_p8u_packed_lab_resources_contract.v1"
...
def validate_contract(config: dict[str, Any]) -> dict[str, Any]:
    expected = {
        "source_context_lab_storage": (
            "first-half-of-consumed-float64-encoded-allocation"
        ),
        "source_context_lab_dtype": "float32",
        "source_context_reduction": "exact-legacy-numpy-mean-std",
        "roundtrip_linear_refill_source": "retained-working-image-float32",
        "roundtrip_linear_refill": "row-local-exact-oetf-then-eotf",
    ...
    if (
        ...
        raise ValueError("unsupported U6.P8U contract")
    ...
    if (
        not decision["next_leaf"].startswith("U6.P8U")
        or decision["dominant_phase"] != "source-context-build"
        or not decision["attribution_complete"]
        or decision["performance_target_pass"]
        or decision["production_default_changed"]
    ):
        raise ValueError("U6.P8U parent decision drift")
    return measurement
...
def evaluate(
    ...
    stable = {
        "schema": "neuro_film.u6_p8u_packed_lab_stable_evidence.v1",
    ...
    return {
        "schema": "neuro_film.u6_p8u_packed_lab_resources_report.v1",
        "node": config["node"],
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/u6_p8u_packed_lab_resources_v1.json",
    )
