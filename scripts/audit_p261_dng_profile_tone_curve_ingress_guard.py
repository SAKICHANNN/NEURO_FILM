"""Run the frozen P261 DNG ProfileToneCurve no-silent-drop audit."""
...
def _sdk_code(config: dict[str, object]) -> dict[int, str]:
    ...
    if len(matching) != 1 or f"= {code}" not in matching[0]:
        raise RuntimeError("SDK tag-code authority mismatch for ProfileToneCurve")
    return {code: name}
...
def _loader_rejection_control(code: int) -> dict[str, object]:
    decode_calls = 0
    ...
    with tempfile.TemporaryDirectory(prefix="p261-guard-") as directory:
        source = Path(directory) / "profile.dng"
        source.write_bytes(b"P261 metadata-only rejection control")
        with (
            patch.object(
                raster.tifffile, "TiffFile", lambda _path: _fake_document((code,))
            ),
            ...
            try:
                ...
                raise AssertionError("guard accepted ProfileToneCurve")
    return {"code": code, "decode_calls": decode_calls, "message": message}


def _direct_guard_control(
    code: int,
    guard_name: str,
    guard,
) -> dict[str, object]:
    pages = [("0", _fake_document((code,)).pages[0])]
    try:
        guard(pages)
    except raster.DngForwardRasterError as exc:
        message = str(exc)
    else:
        raise AssertionError(f"{guard_name} accepted an unsupported tag")
    return {"code": code, "guard": guard_name, "message": message}
...
def build_report(config_path: Path, order: str) -> dict[str, object]:
    ...
    guarded = _sdk_code(config)
    code = next(iter(guarded))
    ...
    for row in rows:
        ...
        results.append(
            {
                "profile_tone_curve_tags": present,
    ...
    rejection = _loader_rejection_control(code)
    preserved = [
        _direct_guard_control(
            50937, "ProfileHueSatMap", raster._guard_unsupported_profile_huesatmap
        ),
        _direct_guard_control(
            52525,
            "ProfileGainTableMap",
    ...
    gate_results = {
        "all_real_rows_tag_free": all(
            not row["profile_tone_curve_tags"] for row in results
        ),
        ...
        "p244_p257_guards_unchanged": all(
            control["guard"] in control["message"] for control in preserved
        ),
        "parent_bindings_exact": all(parent_bindings.values()),
        "row_count": len(results) == int(gates["required_rows"]),
        "sdk_tag_code_exact": guarded == {50940: "ProfileToneCurve"},
        "source_hashes_unchanged": all(row["source_unchanged"] for row in results),
        "tag_rejection": "ProfileToneCurve(50940)@0" in rejection["message"],
        "zero_decode_calls_on_rejection": rejection["decode_calls"] == 0,
    }
    ...
    return {
        "bindings": {
            ...
            "test": _bound(
                Path("tests/test_p261_dng_profile_tone_curve_ingress_guard.py")
            ),
        },
        "claim_ceiling": config["claim_ceiling"],
        "decision": (
            "PASS_PRIVATE_DNG_PROFILE_TONE_CURVE_INGRESS_GUARD"
            if passed
            else "FAIL_CLOSED_DNG_PROFILE_TONE_CURVE_INGRESS_GUARD"
        ),
        "execution_commit": _git_head(),
        "experiment_id": "P261",
        "gate_results": gate_results,
        "guarded_tag": {"code": code, "name": guarded[code]},
        "parent_bindings": parent_bindings,
        "preserved_guard_controls": preserved,
        "rejection_control": rejection,
        "rows": results,
        "schema": "neuro-film.p261-dng-profile-tone-curve-ingress-guard-result.v1",
    }
