"""Build the non-mutating P100 integration manifest and strict schema."""
...
if __package__:
    ...
    from build_reference_match_integration_manifest_v14 import (
        REQUIRED_CONTRACT_SCHEMA_PATHS,
        REQUIRED_PUBLIC_EXPORTS,
    )
...
SCHEMA_ID = "neuro-film.reference-match-main-integration-manifest.v15"
CLAIM_CEILING = "review-ready-not-merged"
PAYLOAD_SCOPE = "P1-P99"
PRIOR_MANIFEST_PATH = (
    "configs/reference_match_main_integration_manifest_v14.json"
)
PRIOR_MANIFEST_SCHEMA_ID = (
    "neuro-film.reference-match-main-integration-manifest.v14"
)
...
def build_schema(*, v14_schema_path: Path = V14_SCHEMA_PATH) -> dict[str, Any]:
    try:
        schema = json.loads(v14_schema_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise IntegrationManifestError(
            f"v14 integration schema unavailable: {v14_schema_path}"
        ) from exc
    result = deepcopy(schema)
    result["$id"] = (
        "https://neuro-film.local/schemas/"
        "reference-match-main-integration-manifest-v15"
    )
    result["title"] = "Neuro-Film Reference Match Main Integration Manifest V15"
