"""Build the exact-threshold sRGB OETF quantizer under factual ceilings."""
...
SOURCE = ROOT / "native" / "reference_srgb_oetf_quantize_v1.c"
HEADER = ROOT / "native" / "reference_srgb_oetf_quantize_v1.h"
ABI_SYMBOLS = {
    "nf_srgb_oetf_quantize_apply_v1",
    "nf_srgb_oetf_quantize_thresholds_sha256_v1",
}
DLL_ENTRY_SOURCE = (
...
def build_msvc_dll(output: Path) -> dict[str, Any]:
    ...
    exports = {
        ...
        if line.split()
        and line.split()[-1].startswith("nf_srgb_oetf_quantize_")
    }
    if exports != ABI_SYMBOLS:
        raise ValueError("MSVC OETF quantizer DLL exports mismatch")
    return {
        "protocol": "neuro-film.srgb-oetf-quantize-msvc-runtime.v1",
        "claim_scope": "Windows x86_64 dynamic OETF quantizer execution",
        **_base_report(),
...
def build_llvm_mingw_dll(
    ...
    exports_path = output.with_suffix(".def")
    entry_source = output.with_name("srgb_oetf_quantize_dll_entry.c")
    entry_object = output.with_name("srgb_oetf_quantize_dll_entry.o")
    _write_exports(exports_path)
    ...
    exports = {
        ...
        and line.split(":", 1)[1]
        .strip()
        .startswith("nf_srgb_oetf_quantize_")
    }
    if exports != ABI_SYMBOLS:
        raise ValueError("LLVM OETF quantizer DLL exports mismatch")
    return {
        "protocol": "neuro-film.srgb-oetf-quantize-llvm-runtime.v1",
        "claim_scope": "independent Windows x86_64 dynamic quantizer execution",
...
def build_android(ndk: Path, output: Path) -> dict[str, Any]:
    ...
    for abi, (target, expected_machine) in ANDROID_TARGETS.items():
        library = output / f"libneuro_film_srgb_oetf_quantize_{abi}.so"
        subprocess.run(
            [
                compiler,
                f"--target={target}",
        ...
        if symbols != ABI_SYMBOLS:
            raise ValueError(f"{abi} exported OETF symbols mismatch")
        artifacts[abi] = {
            "target": target,
            "elf_machine": machine,
    ...
    return {
        "protocol": "neuro-film.srgb-oetf-quantize-android-link.v1",
...
def build_apple_objects(toolchain: Path, output: Path) -> dict[str, Any]:
    ...
    for name, target in APPLE_TARGETS.items():
        object_path = output / f"reference_srgb_oetf_quantize_{name}.o"
        subprocess.run(
            [
                compiler,
                f"--target={target}",
        ...
        symbols = {
            ...
            for line in symbols_output.splitlines()
            if line.split()
            and line.split()[-1].startswith("_nf_srgb_oetf_quantize_")
        }
        if symbols != ABI_SYMBOLS:
            raise ValueError(f"{name} defined OETF symbols mismatch")
        artifacts[name] = {
            "target": target,
            "format": "Mach-O arm64",
    ...
    return {
        "protocol": "neuro-film.srgb-oetf-quantize-apple-object.v1",
