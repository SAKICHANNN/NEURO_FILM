#!/usr/bin/env python3
"""Benchmark exact U6.P8J source-context-first profile scheduling."""
...
SCHEMA = "neuro_film.u6_p8j_context_first_resources_contract.v1"
...
def validate_contract(config: dict[str, Any]) -> dict[str, Any]:
    expected = {
        "source_context_schedule": (
            "before-linear-and-gauged-full-frame-allocation"
        ),
        "source_context_rebuilt_during_display": False,
        "physical_linear_decode_mode": "restored-full-frame-p8h",
        "source_context_semantics_change_allowed": False,
    ...
    if (
        ...
        raise ValueError("unsupported U6.P8J contract")
    ...
    if (
        not decision["next_leaf"].startswith("U6.P8J")
        or decision["candidate_retained"]
        or decision["production_default_changed"]
    ):
        raise ValueError("U6.P8J parent decision drift")
    return measurement
...
def evaluate(
    ...
    stable = {
        "schema": (
            "neuro_film.u6_p8j_context_first_stable_evidence.v1"
        ),
        "schedule": config["implementation"],
    ...
    return {
        "schema": (
            "neuro_film.u6_p8j_context_first_resources_report.v1"
        ),
        "node": config["node"],
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/u6_p8j_context_first_resources_v1.json",
    )
