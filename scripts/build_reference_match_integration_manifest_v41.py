"""Build the P168 integration manifest."""
...
if __package__:
    ...
    from build_reference_match_integration_manifest_v34 import (
        V34_REQUIRED_CONTRACT_SCHEMA_PATHS,
        V34_REQUIRED_PUBLIC_EXPORTS,
    )
...
SCHEMA_ID = "neuro-film.reference-match-main-integration-manifest.v41"
CLAIM_CEILING = "review-ready-not-merged"
PAYLOAD_SCOPE = "P1-P168"
PRIOR_MANIFEST_PATH = "configs/reference_match_main_integration_manifest_v40.json"
PRIOR_MANIFEST_SCHEMA_ID = (
    "neuro-film.reference-match-main-integration-manifest.v40"
)
...
def build_schema(
    *,
    v40_schema_path: Path = V40_SCHEMA_PATH,
) -> dict[str, Any]:
    try:
        result = deepcopy(
            json.loads(v40_schema_path.read_text(encoding="utf-8"))
        )
    except (OSError, json.JSONDecodeError) as exc:
        raise IntegrationManifestError(
            "v40 integration schema unavailable"
        ) from exc
    result["$id"] = (
        "https://neuro-film.local/schemas/"
        "reference-match-main-integration-manifest-v41"
    )
    result["title"] = "Neuro-Film Reference Match Main Integration Manifest V41"
                ...
                {"$ref": "#/$defs/contract_schema"},
                {
                    "properties": {
                        "path": {
                            "const": (
                                "configs/schemas/"
                                "reference_file_output_metadata_policy_v1."
                                "schema.json"
                            )
                        }
                    }
...
def build_manifest(
    *,
    ...
    return build_versioned_manifest(
        ...
        required_public_exports=V41_REQUIRED_PUBLIC_EXPORTS,
        required_contract_schema_paths=V41_REQUIRED_CONTRACT_SCHEMA_PATHS,
