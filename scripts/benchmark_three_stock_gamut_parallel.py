#!/usr/bin/env python3
"""Measure exact row-parallel gamut compression on the three-stock baseline."""
...
def main() -> int:
    args = _parse_args()
    if args.width < 1 or args.height < 1 or args.workers < 2:
        raise ValueError("width/height must be positive and workers must be at least 2")
    source_bytes = args.source.read_bytes()
    with Image.open(args.source) as image:
        rgb8 = np.asarray(
            image.convert("RGB").resize((args.width, args.height), Image.Resampling.LANCZOS),
            dtype=np.uint8,
        )
    ...
    for style in styles:
        ...
        parallel = render_resolved_safe_lab_rgb(
            source, **kwargs, gamut_workers=args.workers
        )
    ...
    stable = {
        "schema": "neuro-film.three-stock-exact-gamut-parallel-benchmark.v1",
