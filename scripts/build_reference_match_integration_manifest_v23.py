"""Build the non-mutating P127 integration manifest and strict schema."""
...
if __package__:
    ...
    from build_reference_match_integration_manifest_v22 import (
        REQUIRED_CONTRACT_SCHEMA_PATHS,
        REQUIRED_PUBLIC_EXPORTS,
    )
...
SCHEMA_ID = "neuro-film.reference-match-main-integration-manifest.v23"
CLAIM_CEILING = "review-ready-not-merged"
PAYLOAD_SCOPE = "P1-P126"
PRIOR_MANIFEST_PATH = (
...
def build_schema(*, v22_schema_path: Path = V22_SCHEMA_PATH) -> dict[str, Any]:
    try:
        schema = json.loads(v22_schema_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise IntegrationManifestError(
            f"v22 integration schema unavailable: {v22_schema_path}"
        ) from exc
    result = deepcopy(schema)
    result["$id"] = (
        "https://neuro-film.local/schemas/"
        "reference-match-main-integration-manifest-v23"
    )
    result["title"] = (
