"""Run triangular conditional monotone FilmMatch capacity."""
...
from src.eval.filmmatch_triangular_conditional_capacity import (  # noqa: E402
    evaluate_triangular_conditional_capacity,
)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=ROOT
        / "configs/u5_r2ax2_filmmatch_triangular_conditional_v1.json",
    )
    parser.add_argument(
        "--output",
        type=Path,
        ...
        "triangular_conditional_report.json",
    )
    ...
    if (
        ...
        raise ValueError("AX1 parent identity drift")
    sample_config = json.loads(
        (ROOT / config["parent"]["sample_config"]).read_text(encoding="utf-8")
    )
    report = evaluate_triangular_conditional_capacity(
