#!/usr/bin/env python3
"""Re-attribute RSS through the exact current P8S production entry."""
...
from scripts.benchmark_u6_p8r_current_phase_rss import (  # noqa: E402
    _run_worker,
)
...
SCHEMA = "neuro_film.u6_p8t_current_phase_rss_contract.v1"
...
def validate_contract(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("unsupported U6.P8T contract")
...
def evaluate(config: dict[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    if available < int(
        ...
        raise RuntimeError("available memory is below P8T launch floor")
    scenario = config["scenario"]
    ...
    for repeat in range(int(scenario["repeats"])):
        ...
        command = [
            ...
            str(worker_output),
            "--config",
            str(ROOT / "configs/u6_p8t_current_phase_rss_v1.json"),
        ]
        observed = _monitor(
            command,
    ...
    stable = {
        "schema": "neuro_film.u6_p8t_current_phase_rss_stable_evidence.v1",
    ...
    return {
        "schema": "neuro_film.u6_p8t_current_phase_rss_report.v1",
        "node": config["node"],
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/u6_p8t_current_phase_rss_v1.json",
    )
