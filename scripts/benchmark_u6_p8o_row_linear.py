#!/usr/bin/env python3
"""Benchmark exact U6.P8O row-staged roundtrip-linear construction."""
...
SCHEMA = "neuro_film.u6_p8o_row_linear_resources_contract.v1"
...
def validate_contract(config: dict[str, Any]) -> dict[str, Any]:
    expected = {
        "roundtrip_linear_output": "one-preallocated-full-frame-float64",
        "roundtrip_linear_execution": "row-bounded-eotf",
        "roundtrip_linear_tile_rows": 128,
        "upstream_source_encoding": "exact-p8m",
        "downstream_execution": "exact-p8m",
        "pointwise_eotf_change_allowed": False,
    ...
    if (
        ...
        raise ValueError("unsupported U6.P8O contract")
    ...
    if (
        not decision["next_leaf"].startswith("U6.P8O")
        or decision["dominant_phase"] != "roundtrip-linear"
        or not decision["attribution_complete"]
        or decision["production_default_changed"]
    ):
        raise ValueError("U6.P8O parent decision drift")
    return measurement
...
def evaluate(
    ...
    stable = {
        "schema": (
            "neuro_film.u6_p8o_row_linear_stable_evidence.v1"
        ),
    ...
    return {
        "schema": "neuro_film.u6_p8o_row_linear_resources_report.v1",
        "node": config["node"],
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/u6_p8o_row_linear_resources_v1.json",
    )
