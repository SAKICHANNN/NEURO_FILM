"""Build the P152 integration manifest."""
...
SCHEMA_ID="neuro-film.reference-match-main-integration-manifest.v37";CLAIM_CEILING="review-ready-not-merged";PAYLOAD_SCOPE="P1-P152"
PRIOR_MANIFEST_PATH="configs/reference_match_main_integration_manifest_v36.json";PRIOR_MANIFEST_SCHEMA_ID="neuro-film.reference-match-main-integration-manifest.v36"
ROOT=Path(__file__).resolve().parents[1];V36_SCHEMA_PATH=ROOT/"configs/schemas/reference_match_main_integration_manifest_v36.schema.json";DEFAULT_SCHEMA_PATH=ROOT/"configs/schemas/reference_match_main_integration_manifest_v37.schema.json"
...
def build_schema(*,v36_schema_path:Path=V36_SCHEMA_PATH)->dict[str,Any]:
    try:result=deepcopy(json.loads(v36_schema_path.read_text(encoding="utf-8")))
    except (OSError,json.JSONDecodeError) as exc:raise IntegrationManifestError("v36 integration schema unavailable") from exc
    result["$id"]="https://neuro-film.local/schemas/reference-match-main-integration-manifest-v37";result["title"]="Neuro-Film Reference Match Main Integration Manifest V37";result["properties"]["schema_id"]["const"]=SCHEMA_ID;result["properties"]["payload_scope"]["const"]=PAYLOAD_SCOPE
