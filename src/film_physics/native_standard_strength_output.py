"""Final PNG16 transaction for verified native Standard strength staging."""
...
STRENGTH_OUTPUT_VERIFICATION_SCHEMA = (
    "neuro_film.native_standard_strength_png16_verification.v1"
)
_MAX_REPORT_BYTES = 2 * 1024 * 1024


def commit_verified_native_standard_strength_png16(
    *,
    ...
    """Quantize one verified strength stage once and commit report last."""

    verification = verify_native_standard_strength_staging(
    ...
    if hashlib.sha256(staging_raw).hexdigest() != (
        ...
        raise ValueError(
            "native Standard strength staging changed after verification"
        )
    ...
    if (
        ...
        raise ValueError(
            "native Standard strength raw staging shape drift"
        )
    ...
    if (
        ...
        raise ValueError(
            "native Standard strength PNG/report must be absent "
            "canonical siblings"
        )
    ...
    try:
        ...
        if (
            ...
            raise ValueError(
                "native Standard strength final values require clipping"
            )
        save_srgb16_png(pixels, output_temp)
        del pixels
        if _sha256_file(raw_output) != staging["output_sha256"]:
            raise ValueError(
                "native Standard strength staging changed during encoding"
            )
        png_sha = _sha256_file(output_temp)
        core = {
            "schema": STRENGTH_OUTPUT_SCHEMA,
            "state": "committed-native-standard-strength-png16",
            "claim_ceiling": (
                "bounded-display-look-strength-srgb16-png-committed-"
                "not-delivered"
            ),
            "strength_staging_verification_id": (
                verification["verification_id"]
            ),
            "strength_staging_run_id": expected_staging_run_id,
            "strength_staging_report_sha256": (
                expected_staging_report_sha256
            ),
            "strength_staging_output_sha256": staging["output_sha256"],
            "full_strength_output_sha256": (
                staging["full_strength_output_sha256"]
            ),
            "strength": staging["strength"],
...
def verify_native_standard_strength_png16(
    *,
    ...
    """Restart-verify a final encoded strength PNG."""

    if not _is_sha256(expected_report_sha256) or not _is_sha256(
        expected_delivery_id
    ):
        raise ValueError(
            "expected strength PNG identities must be SHA-256"
        )
    raw = _bounded_read(
    ...
    if hashlib.sha256(raw).hexdigest() != expected_report_sha256:
        raise ValueError(
            "native Standard strength PNG report hash mismatch"
        )
    ...
    expected_keys = {
        ...
        "claim_ceiling",
        "strength_staging_verification_id",
        "strength_staging_run_id",
        "strength_staging_report_sha256",
        "strength_staging_output_sha256",
        "full_strength_output_sha256",
        "strength",
        "strength_domain",
        "output_path",
    ...
    if (
        ...
        or payload["schema"] != STRENGTH_OUTPUT_SCHEMA
        or payload["state"]
        != "committed-native-standard-strength-png16"
        ...
        raise ValueError(
            "native Standard strength PNG report contract drift"
        )
    core = {
        key: value
    ...
    if hashlib.sha256(_canonical_bytes(core)).hexdigest() != (
        ...
        raise ValueError(
            "native Standard strength PNG delivery identity drift"
        )
    ...
    if (
        ...
        raise ValueError(
            "native Standard strength PNG output drift"
        )
    core_receipt = {
        "schema": STRENGTH_OUTPUT_VERIFICATION_SCHEMA,
        "state": "verified-native-standard-strength-png16",
        "delivery_id": expected_delivery_id,
        "report_sha256": expected_report_sha256,
        "output_sha256": payload["output_sha256"],
        "strength": payload["strength"],
        "claim_ceiling": (
            "verified-bounded-display-look-strength-srgb16-png-"
            "not-delivered"
        ),
    }
    return {
