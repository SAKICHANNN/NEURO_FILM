"""ctypes binding for the P4HO native inverse-Gamma density primitive."""
...
class NativeGammaDensityDiagnosticsV1(ctypes.Structure):
    _fields_ = [
...
def load_native_gamma_density_library(path: Path) -> ctypes.CDLL:
    library = ctypes.CDLL(str(path))
    library.nf_gamma_density_f64_abi_version_v1.argtypes = []
    library.nf_gamma_density_f64_abi_version_v1.restype = ctypes.c_uint32
    library.nf_gamma_density_f64_apply_v1.argtypes = [
        ...
        ctypes.POINTER(ctypes.c_double),
        ctypes.c_size_t,
        ctypes.POINTER(NativeGammaDensityDiagnosticsV1),
    ]
    library.nf_gamma_density_f64_apply_v1.restype = ctypes.c_int
    if library.nf_gamma_density_f64_abi_version_v1() != 1:
        raise RuntimeError("native gamma density ABI drift")
    return library


def apply_native_gamma_density(
    library: ctypes.CDLL,
    uniforms: np.ndarray,
    shapes: np.ndarray,
    scales: np.ndarray,
    *,
    ...
    if probability.ndim != 1 or shape.shape != probability.shape or scale.shape != probability.shape:
        raise ValueError("invalid native gamma density request")
    ...
    if status != 0:
        raise RuntimeError(f"native gamma density apply failed: {status}")
    return output, {
