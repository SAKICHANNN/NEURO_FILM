...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    body {{ margin:0; min-width:320px; overflow-x:clip; background:radial-gradient(circle at top left,#20261d 0,#101210 38rem); color:var(--text); font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif; line-height:1.5; }}
    .shell {{ width:min(1120px,calc(100vw - 32px)); max-width:100%; margin:0 auto; padding:48px 0 72px; }}
    ...
    .lede,.eyebrow {{ color:var(--muted); }} .lede {{ max-width:62ch; margin-bottom:0; overflow-wrap:anywhere; }} .eyebrow {{ margin-bottom:6px; font-size:.72rem; font-weight:750; letter-spacing:.14em; text-transform:uppercase; }}
    ...
    .recipe-grid {{ display:grid; min-width:0; max-width:100%; gap:16px; }} .recipe-card {{ min-width:0; max-width:100%; padding:22px; border:1px solid var(--line); border-radius:var(--radius); background:linear-gradient(145deg,var(--panel),var(--panel-2)); box-shadow:0 18px 40px rgba(0,0,0,.16); }} .recipe-card[hidden] {{ display:none; }} .recipe-card-error {{ border-color:#75483f; }}
    ...
    .recipe-details {{ display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:14px 22px; margin:22px 0 0; }} .recipe-details div {{ min-width:0; }} dt {{ margin-bottom:3px; color:var(--muted); font-size:.72rem; font-weight:750; letter-spacing:.06em; text-transform:uppercase; }} dd {{ min-width:0; margin:0; overflow-wrap:anywhere; }} code {{ white-space:normal; overflow-wrap:anywhere; word-break:break-all; color:#dfe8d8; font-family:"Cascadia Mono","SFMono-Regular",Consolas,monospace; font-size:.78rem; }}
    ...
    @media (max-width:720px) {{ .shell {{ width:min(calc(100vw - 20px),1120px); padding-top:28px; }} .app-header,.toolbar {{ grid-template-columns:1fr; }} .summary {{ justify-content:flex-start; }} .recipe-details {{ grid-template-columns:1fr; }} .card-heading {{ display:grid; }} .badges {{ justify-content:flex-start; }} }}
