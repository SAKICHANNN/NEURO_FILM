...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    .recipe-grid {{ display:grid; min-width:0; gap:16px; }} .recipe-card {{ min-width:0; padding:22px; border:1px solid var(--line); border-radius:var(--radius); background:linear-gradient(145deg,var(--panel),var(--panel-2)); box-shadow:0 18px 40px rgba(0,0,0,.16); }} .recipe-card[hidden] {{ display:none; }} .recipe-card-error {{ border-color:#75483f; }}
    ...
    @media (max-width:720px) {{ .shell {{ width:min(calc(100% - 20px),1120px); padding-top:28px; }} .app-header,.toolbar {{ grid-template-columns:1fr; }} .summary {{ justify-content:flex-start; }} .recipe-details {{ grid-template-columns:1fr; }} .card-heading {{ display:grid; }} .badges {{ justify-content:flex-start; }} }}
