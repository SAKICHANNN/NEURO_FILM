"""Host-native feasibility boundary for the pointwise safe-Lab stages."""
...
SOURCE = "native/color_engine/nf_safe_lab_pointwise_f32_v1.c"
HEADER = "native/color_engine/nf_safe_lab_pointwise_f32_v1.h"


class NativeSafeLabPointwiseError(RuntimeError):
    """Raised when the native pointwise safe-Lab kernel rejects an invocation."""


def build_native_safe_lab_pointwise(*, root: Path, output_dir: Path) -> dict[str, Any]:
    return build_msvc_c11_dll(
        root=root,
        output_dir=output_dir,
        source_relative=SOURCE,
        header_relative=HEADER,
        basename="nf_safe_lab_pointwise_f32_v1",
    )


def load_native_safe_lab_pointwise(dll_path: Path) -> ctypes.CDLL:
    library = ctypes.CDLL(str(Path(dll_path).resolve()))
    function = library.nf_safe_lab_pointwise_f32_v1
...
def apply_native_safe_lab_pointwise(
    library: ctypes.CDLL,
    ...
    *,
    ...
    status = library.nf_safe_lab_pointwise_f32_v1(
    ...
    if status != 0:
        raise NativeSafeLabPointwiseError(
