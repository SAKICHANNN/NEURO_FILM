"""Exact-output conformance for single-compute AO6 residual/display v4."""
...
def build_msvc_native_ao6_display_v4_dll(
    *, root: Path, output_dir: Path
    ...
    header_relatives = [
        "native/film_physics/nf_ao6_residual_f32_v2.h",
        "native/film_physics/nf_ao6_display_f32_v4.h",
    ]
    ...
    dll = (output_dir / "nf_ao6_display_f32_v4.dll").resolve()
    import_library = (
        output_dir / "nf_ao6_display_f32_v4.lib"
    ).resolve()
    batch = output_dir / "build_nf_ao6_display_f32_v4.bat"
    batch.write_text(
        "@echo off\r\n"
    ...
    if completed.returncode != 0 or not dll.is_file():
        raise RuntimeError(
            "MSVC native AO6 display v4 build failed:\n" + compiler_output
        )
    if "warning" in compiler_output.lower():
        raise RuntimeError(
            "MSVC native AO6 display v4 build warned:\n" + compiler_output
        )
    return {
...
def _load_display_v4(path: Path) -> ctypes.CDLL:
...
def _apply_display_v4(
    *,
    ...
    for y0 in range(0, height, tile_rows):
        ...
        if status != 0:
            raise RuntimeError(
                f"P8AV display v4 tile {tile_rows} failed: {status}"
            )
        output[y0:y1] = output_rows.reshape(y1 - y0, width, 3)
    return output


def run_native_ao6_display_v4_conformance(
    *, root: Path, config: dict[str, Any], output_dir: Path
    ...
    if not str(parent.get("next_leaf", "")).startswith("U6.P8AV"):
        raise ValueError("P8AV parent decision drift")
    ...
    if display_sha != config["expected_display_payload_sha256"]:
        raise ValueError("P8AV display payload drift")
    ...
    v3_build = build_msvc_native_ao6_display_v3_dll(
        root=root, output_dir=output_dir / "display_v3"
    )
    v4_builds = [
        build_msvc_native_ao6_display_v4_dll(
            root=root, output_dir=output_dir / f"display_v4_{index}"
        )
        for index in range(2)
    ]
    if v4_builds[0]["dll_sha256"] != v4_builds[1]["dll_sha256"]:
        raise RuntimeError("P8AV independent display v4 build drift")
    for build in v4_builds:
        if (
            build["sources"] != config["native_sources"]["sources"]
            or build["headers"] != config["native_sources"]["headers"]
        ):
            raise ValueError("P8AV native source identity drift")
    ...
    source[0, :2] = ((0.0, 0.0, 0.0), (1.0, 1.0, 1.0))
    encoded[0, :2] = source[0, :2]
    context_library = _load_context_v2(Path(context_build["dll_path"]))
    ...
    library_v3 = _load_display_v3(Path(v3_build["dll_path"]))
    library_v4 = _load_display_v4(Path(v4_builds[0]["dll_path"]))
    rows = []
    for tile_rows in config["tile_rows"]:
        ...
        if output_v3.tobytes() != output_v4.tobytes():
            raise RuntimeError("P8AV display v3/v4 output drift")
        rows.append(
            {
                "tile_rows": int(tile_rows),
    ...
    if len({row["output_sha256"] for row in rows}) != 1:
        raise RuntimeError("P8AV display v4 partition drift")
    ...
    status = library_v4.nf_ao6_display_f32_apply_v4(
        ctypes.byref(base_profile),
        ctypes.byref(context),
        ctypes.byref(residual_profile),
        _pointer(invalid),
    ...
    if (
        ...
        raise RuntimeError("P8AV invalid input changed scratch or final")

    stable = {
        "schema": "neuro_film.u6_p8av_native_ao6_display_v4.v1",
        "display_payload_sha256": display_sha,
        "context_v2_dll_sha256": context_build["dll_sha256"],
        "display_v3_dll_sha256": v3_build["dll_sha256"],
        "display_v4_dll_sha256": v4_builds[0]["dll_sha256"],
        "independent_display_v4_build_byte_exact": True,
        "context_sha256": context_sha,
        "display_rows": rows,
        "display_v3_v4_byte_exact": True,
        "display_v4_partition_byte_exact": True,
        "invalid_input_scratch_unchanged": True,
        "invalid_input_final_output_unchanged": True,
        "optimization": (
            "single residual pixel computation into expendable display scratch"
        ),
        "decision": (
            "pass AO6 display v4 under exact v3 output, partition, "
            "independent-build and invalid-input failure-atomic gates"
        ),
        "claim_ceiling": config["claim_ceiling"],
        "production_default_changed": False,
        "next_leaf": (
            "U6.P8AW rerun the unchanged 12MP resource gate with context "
            "v2 and display v4; if latency still fails, re-profile"
        ),
    }
    return {
        **stable,
