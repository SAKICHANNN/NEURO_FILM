...
@dataclass
class NativeDensityStageRuntime:
    """Loaded native developed-structure stage before paired scanner response.

    ``apply_source`` deliberately assumes that the caller has already validated the
    source colour rail.  It preserves the exact P4HR/P4HT/P4HU arithmetic and returns
    only the pre-scanner physical image plus compact deterministic receipts.
    """
    ...
    fast_gamma: bool
    native_spatial: bool
    native_toolchains: dict[str, dict[str, Any]]
    field_library: Any | None = None
    ...
    def apply_source(
        ...
        *,
        ...
        """Apply the bound stage to one already-validated linear-f32 source."""
    ...
    def apply_physical(
        ...
        """P4HE-compatible callback retaining exact component identity checks."""
        if (
            profile.identity() != self.profile_payload["density_profile_id"]
            or prior.identity() != self.profile_payload["manufacturer_prior_id"]
        ):
            raise ValueError("P4HR photographic component identity drift")
        height, width = source.shape[:2]
        if self.native_spatial:
            if self.field_library is None:
                raise ValueError("P4HR native field runtime missing")
            rendered = [
                render_native_thomas_field(
                    self.field_library,
                    NativeThomasFieldProfile(
            ...
            receipts = tuple(
                build_thomas_dc_receipt(
                    ...
                    canonical_row_block_height=(
                        self.components.canonical_row_block_height
                    ),
                )
                for seed in seeds
            )
        ...
        physical, developed, native = apply_profile_bound_native_density(
            ...
            copula_iterations=int(self.candidate["copula_iterations"]),
            gamma_inverse_iterations=int(
                self.candidate["gamma_inverse_iterations"]
            ),
            high_shape_threshold=float(self.candidate["high_shape_threshold"]),
            fast_newton_iterations=(
                int(self.candidate["gamma_newton_iterations"])
                if self.fast_gamma
                else None
            ),
            fast_direct_shape_upper=(
                float(self.candidate["gamma_direct_shape_upper"])
                if self.fast_gamma
                else None
            ),
        )
        ...
        return physical, {
            ...
            "rank_bins": self.components.rank_bins,
            "pass_count": int(self.candidate["copula_iterations"]) + 1,
            "canonical_row_block_height": (
                self.components.canonical_row_block_height
            ),
...
def build_native_density_stage_runtime(
    *,
    root: Path,
    build_dir: Path,
    candidate: dict[str, Any],
    ...
    """Build and load the exact native stage used by P4HR/P4HT/P4HU."""
    ...
    if field_build is not None:
        native_toolchains["field"] = _stable_toolchain(field_build)
    return NativeDensityStageRuntime(
        candidate=candidate,
        profile_payload=profile_payload,
        components=components,
        copula_library=copula_library,
        gamma_library=gamma_library,
        fast_gamma=fast_gamma,
        native_spatial=native_spatial,
        native_toolchains=native_toolchains,
...
def evaluate(
    ...
    *,
    ...
    result = _evaluate_paired(
        ...
        apply_physical_override=runtime.apply_physical,
    ...
    stable["native_toolchains"] = runtime.native_toolchains
