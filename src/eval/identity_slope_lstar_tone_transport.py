"""CB40 positive-source-slope L-star tone plus fixed CB32 chroma."""
...
SCHEMA = "neuro_film.u5_r2cb40_identity_slope_lstar_tone_development_contract.v1"
REPORT_SCHEMA = (
    "neuro_film.u5_r2cb40_identity_slope_lstar_tone_development_report.v1"
)
EXPERIMENT_ID = "U5.R2CB40"


class IdentitySlopeLstarToneTransportError(RuntimeError):
    pass
...
def load_contract(path: Path) -> dict[str, Any]:
    ...
    if payload.get("schema") != SCHEMA or payload.get("experiment_id") != EXPERIMENT_ID:
        raise IdentitySlopeLstarToneTransportError("CB40 contract structure drift")
    return payload


def select_identity_slope_candidate(
    ...
    *,
    ...
    tone_knot_count: int,
    source_identity_fraction: float,
    ...
    if (
        ...
        or not 0.0 < source_identity_fraction < 1.0
        or doses.ndim != 1
        or doses.size < 2
        or doses[0] != 1.0
        or doses[-1] != 0.0
        ...
        raise IdentitySlopeLstarToneTransportError("CB40 selector input drift")
    ...
    mapped_lstar = (
        source_identity_fraction * source_lstar.astype(np.float64)
        + (1.0 - source_identity_fraction) * mapped_lstar
    )
    ...
    if selected is None:
        ...
        raise IdentitySlopeLstarToneTransportError(
            "CB40 dose grid has no safe identity-slope candidate; "
    ...
    facts.update(
        {
            "source_identity_fraction": float(source_identity_fraction),
...
def evaluate(config: Mapping[str, Any], root: Path, output_dir: Path) -> dict[str, Any]:
    decision = _load_exact_json(
        root,
        config["parents"]["cb39_decision_path"],
        config["parents"]["cb39_decision_sha256"],
    )
    if decision.get("decision") != config["parents"]["cb39_required_decision"]:
        raise IdentitySlopeLstarToneTransportError("CB39 decision drift")
    ...
    if parent_source.get("decision") != source_decision["parent_required_decision"]:
        raise IdentitySlopeLstarToneTransportError("CB40 source parent drift")
    ...
    def candidate_builder(
        ...
        *,
        ...
        del weights
        try:
            candidate, scale, luma_error, row_facts = select_identity_slope_candidate(
                source_linear,
                safe_base_linear,
                full_target_linear,
                boundary_epsilon=boundary_epsilon,
                tone_knot_count=int(op["tone_knot_count"]),
                source_identity_fraction=float(op["source_identity_fraction"]),
        ...
        except IdentitySlopeLstarToneTransportError as exc:
    ...
    try:
        report = evaluate_direction_candidate(
            ...
            contract_filename="u5_r2cb40_identity_slope_lstar_tone_development_v1.json",
            blind_seed=int(config["blind_protocol"]["seed"]),
        )
    except IdentitySlopeLstarToneTransportError:
        if failure is None:
            raise
        ...
        diagnostic: dict[str, Any] = {
            ...
            "contract_sha256": hashlib.sha256(
                (
                    root
                    / "configs/u5_r2cb40_identity_slope_lstar_tone_development_v1.json"
                ).read_bytes()
            ).hexdigest(),
            ...
            "decision": "close_identity_slope_tone_before_complete_render",
    ...
    if len(facts) != len(report["rows"]):
        raise IdentitySlopeLstarToneTransportError("CB40 diagnostic count drift")
    ...
    report["decision"] = (
        "open_identity_slope_tone_severe_review_then_blind_development"
        if report["automatic_pass"]
        else "close_identity_slope_tone_without_rescue"
    )
...
__all__ = [
    "IdentitySlopeLstarToneTransportError",
    "evaluate",
    "load_contract",
