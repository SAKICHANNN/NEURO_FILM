...
def evaluate_assessment(model, plan: dict, *, assignment: dict, normalizer: dict,
    ...
    oracle = np.empty((32, 32, 4), dtype=np.float64)
    for j in range(32):
        for t in range(32):
            oracle[j, t] = decode_canonical_prior(
                {'mu': predictions['after_mu'][j, t], 'scale': predictions['after_scale'][j, t]},
                predictions['canonical_targets'][j, t], slope_limit=1.25, offset_limit=.35)['u']
    parameters['paired_oracle'] = oracle
    ...
    arrays = {key: predictions[key] for key in ['predicted_canonical', 'canonical_targets', 'after_mu', 'after_scale']}
    arrays['D'] = cases['D']
