// Private P248 ordered-scanline ACEScg OpenEXR research writer.
...
Arguments parse_arguments(int argc, char** argv) {
    ...
    if (args.row_block != 16) {
        throw std::invalid_argument("P248 row block must equal 16");
    }
    return args;
}
...
void write_file(const Arguments& args) {
    ...
    const auto temporary = args.output.native() + L".p248.tmp";
...
int main(int argc, char** argv) {
    try {
        ...
        std::cerr << "p248 writer error: " << error.what() << '\n';
        return 2;
    }
}
