// Private P255 ordered-scanline ACES2065-1 OpenEXR research writer.
...
constexpr double kAp1ToAp0[3][3] = {
...
Arguments parse_arguments(int argc, char** argv) {
    ...
    if (args.row_block != 16) {
        throw std::invalid_argument("P255 row block must equal 16");
    }
    return args;
}
...
void ap1_to_ap0(const float* ap1, float* ap0) {
    for (int output_channel = 0; output_channel < 3; ++output_channel) {
        ap0[output_channel] = static_cast<float>(
            static_cast<double>(ap1[0]) * kAp1ToAp0[output_channel][0] +
            static_cast<double>(ap1[1]) * kAp1ToAp0[output_channel][1] +
            static_cast<double>(ap1[2]) * kAp1ToAp0[output_channel][2]);
    }
}
...
void write_file(const Arguments& args) {
    ...
    const auto temporary = args.output.native() + L".p255.tmp";
    ...
    try {
        ...
        header.insert(
            "chromaticities",
            OPENEXR_IMF_NAMESPACE::ChromaticitiesAttribute(OPENEXR_IMF_NAMESPACE::Chromaticities(
                IMATH_NAMESPACE::V2f(0.7347F, 0.2653F),
                IMATH_NAMESPACE::V2f(0.0F, 1.0F),
                IMATH_NAMESPACE::V2f(0.0001F, -0.077F),
                IMATH_NAMESPACE::V2f(0.32168F, 0.33767F))));
        header.insert(
            "adoptedNeutral",
            OPENEXR_IMF_NAMESPACE::V2fAttribute(IMATH_NAMESPACE::V2f(0.32168F, 0.33767F)));
        header.insert("acesImageContainerFlag", OPENEXR_IMF_NAMESPACE::IntAttribute(1));
        header.insert(
            "colorInteropID", OPENEXR_IMF_NAMESPACE::StringAttribute("lin_ap0_scene"));
        ...
        std::vector<float> rows(
            static_cast<std::size_t>(args.row_block) * static_cast<std::size_t>(args.width) * 3U);
        ...
        for (int y0 = 0; y0 < args.height; y0 += args.row_block) {
            ...
            for (int local_y = 0; local_y < count; ++local_y) {
                ...
                for (int x_int = 0; x_int < args.width; ++x_int) {
                    ...
                    const std::size_t base =
                        (static_cast<std::size_t>(local_y) * static_cast<std::size_t>(args.width) +
                         static_cast<std::size_t>(x_int)) *
                        3U;
                    const float ap1[3] = {
                        sample_value(0, x, y, args.period),
                        sample_value(1, x, y, args.period),
                        sample_value(2, x, y, args.period),
                    };
                    ap1_to_ap0(ap1, rows.data() + base);
                }
            }
            ...
            frame_buffer.insert(
                "R", OPENEXR_IMF_NAMESPACE::Slice(
                         OPENEXR_IMF_NAMESPACE::FLOAT, logical_origin, pixel_stride, row_stride));
            frame_buffer.insert(
                "G", OPENEXR_IMF_NAMESPACE::Slice(
                         OPENEXR_IMF_NAMESPACE::FLOAT,
                         logical_origin + sizeof(float),
                         pixel_stride,
                         row_stride));
            frame_buffer.insert(
                "B", OPENEXR_IMF_NAMESPACE::Slice(
                         OPENEXR_IMF_NAMESPACE::FLOAT,
...
int main(int argc, char** argv) {
    try {
        ...
        std::cerr << "p255 writer error: " << error.what() << '\n';
        return 2;
    }
}
