"""Host-native v2 exact-semantics boundary for Rec.2020 Lab source compression."""
...
SOURCE = "native/color_engine/nf_rec2020_lab_source_compress_f32_v2.c"
HEADER = "native/color_engine/nf_rec2020_lab_source_compress_f32_v2.h"


class NativeRec2020LabCompressV2Error(RuntimeError):
    """Raised when the native Lab compression kernel rejects an invocation."""


def build_native_rec2020_lab_compress_v2(
    *, root: Path, output_dir: Path
    ...
    return build_msvc_c11_dll(
        ...
        basename="nf_rec2020_lab_source_compress_f32_v2",
    )


def load_native_rec2020_lab_compress_v2(dll_path: Path) -> ctypes.CDLL:
    library = ctypes.CDLL(str(Path(dll_path).resolve()))
    function = library.nf_rec2020_lab_source_compress_f32_v2
    function.argtypes = [
...
def apply_native_rec2020_lab_compress_v2(
    library: ctypes.CDLL,
    ...
    *,
    ...
    status = library.nf_rec2020_lab_source_compress_f32_v2(
    ...
    if status != 0:
        raise NativeRec2020LabCompressV2Error(
