"""Unchanged AY2 confirmation for the BN1 projection-curve operator."""
...
class FiveKProjectionCurveConfirmationError(ValueError):
    """Raised when BN2 confirmation inputs or policy drift."""
...
def validate_contract(root: Path, config: Mapping[str, Any]) -> dict[str, Any]:
    if config.get("status") != "contract_frozen_implementation_ready":
        raise FiveKProjectionCurveConfirmationError("contract is not frozen")
    if (
        ...
        raise FiveKProjectionCurveConfirmationError("confirmation boundary drift")
    ...
    if (
        ...
        raise FiveKProjectionCurveConfirmationError("BN1 decision drift")
    parent_report = root / str(parent["report"])
    if _sha256(parent_report) != parent["report_sha256"]:
        raise FiveKProjectionCurveConfirmationError("BN1 report drift")
    parent_config_path = root / str(parent["config"])
    if _sha256(parent_config_path) != parent["config_sha256"]:
        raise FiveKProjectionCurveConfirmationError("BN1 config drift")
    parent_config = json.loads(parent_config_path.read_text(encoding="utf-8"))
    parent_validated = validate_bn1_contract(root, parent_config)
    ...
    if (
        ...
        raise FiveKProjectionCurveConfirmationError("AY2 source drift")
    ...
    if (
        ...
        raise FiveKProjectionCurveConfirmationError("AY2 source is ineligible")
    ...
    if (
        ...
        raise FiveKProjectionCurveConfirmationError("AY2 leakage gate failed")
    ...
    if any(
        ...
        raise FiveKProjectionCurveConfirmationError("BN1 threshold drift")
    return {
...
def run_confirmation(
    *,
    ...
    directions = parent_validated["directions"]
    _fit_population(ay0, directions=directions, operator=operator)
    _fit_population(confirmation, directions=directions, operator=operator)
    predictions = _build_predictions(ay0, [confirmation], parent_config)
