"""U6.P7D1 frozen visual confirmation for the selected virtual-scan arm."""
...
from src.eval.physical_virtual_scan_sampling import validate_contract


SCHEMA = "neuro_film.u6_p7d1_virtual_scan_visual_confirmation_contract.v1"
ADJUDICATION_SCHEMA = (
    "neuro_film.u6_p7d1_virtual_scan_visual_adjudication_contract.v1"
)
...
def validate_visual_contract(
    ...
    if (
        ...
        raise ValueError("unsupported U6.P7D1 contract")
    parent_contract = _load_exact_json(
        root,
        config["parent_contract"],
        config["parent_contract_sha256"],
    )
    runtime_parent = validate_contract(root, parent_contract)
    ...
    if (
        first != second
        or first["selected_candidate_id"] != config["selected_candidate_id"]
        or first["automatic_eligible_candidate_ids"]
        != [config["selected_candidate_id"]]
        or not first["negative_control_reproduced_p7c"]
        or colour["node"] != "U6.P7B"
        or not colour["automatic_pass"]
    ):
        raise ValueError("U6.P7D1 parent evidence drift")
    ...
    if (
        ...
        or config["blind"]["arms"]
        != ["colour_only", "combined_scan_4000_dpi"]
    ):
        raise ValueError("U6.P7D1 visual protocol drift")
    return runtime, first, colour


def _expected_paths_and_hashes(
    *,
    root: Path,
    config: dict[str, Any],
    runtime: Any,
    ...
    combined_hashes = {
        ...
        if row["candidate_id"] == config["selected_candidate_id"]
    }
    colour_rows = {
        row["sample_id"]: row
    ...
    for sample_id in config["visual_ids"]:
        ...
        combined = (
            root
            / "outputs"
            / "u6_p7d_virtual_scan_sampling_audit_v1"
            / "run_a"
            / "renders"
            / config["selected_candidate_id"]
            / f"{sample_id}.png"
        ...
        expected = {
            ...
            combined: combined_hashes[sample_id],
        }
        ...
        paths[sample_id] = {
            ...
            "combined_scan_4000_dpi": combined,
        }
    return paths
...
def build_visual_evidence(
    *,
    ...
    paths = _expected_paths_and_hashes(
        root=root,
        config=config,
        runtime=runtime,
        parent=parent,
    ...
    core = {
        "schema": "neuro_film.u6_p7d1_virtual_scan_visual_evidence.v1",
        "node": config["node"],
...
def adjudicate_visual_evidence(
    *, root: Path, config: dict[str, Any]
    ...
    if config.get("schema") != ADJUDICATION_SCHEMA:
        raise ValueError("unsupported U6.P7D1 adjudication contract")
    ...
    if (
        ...
        raise ValueError("U6.P7D1 scoring or mapping drift")
    ...
    combined = "combined_scan_4000_dpi"
    combined_round_wins = sum(
        row["winner"] == combined for row in per_round.values()
    )
    severe_count = int(
        scoring["severe_review"]["confirmed_severe_combined_artifacts"]
    )
    if severe_count > 0:
        decision = "severe_fail"
    elif combined_round_wins < int(config["minimum_combined_round_wins"]):
        decision = "preference_fail"
    else:
        decision = "complete_pass"
    core = {
        "schema": "neuro_film.u6_p7d1_virtual_scan_visual_adjudication.v1",
        "node": config["node"],
        "claim_ceiling": config["claim_ceiling"],
        ...
        "per_round": per_round,
        "total_choices": total,
        "combined_round_wins": combined_round_wins,
        "decision": decision,
