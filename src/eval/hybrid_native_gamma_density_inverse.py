"""P4HP evaluation of the hybrid native inverse-Gamma density primitive."""
...
SCHEMA = "neuro-film.u6-p4hp-hybrid-native-gamma-density-inverse-contract.v1"


def _build_llvm(root: Path, output_dir: Path, clang: Path) -> dict[str, str]:
    source = root / "native/film_physics/nf_gamma_density_hybrid_f64_v1.c"
    header = root / "native/film_physics/nf_gamma_density_hybrid_f64_v1.h"
    dll = output_dir / "nf_gamma_density_hybrid_f64_llvm_v1.dll"
...
def _failure_atomic(
    ...
    bad = uniforms.copy()
    bad[len(bad) // 2] = 1.0
    output = np.full(uniforms.shape, -23.0, dtype=np.float64)
...
def evaluate(
    *, root: Path, contract: dict[str, Any], output_dir: Path, clang: Path
    ...
    if contract.get("schema") != SCHEMA:
        raise ValueError("unsupported P4HP contract")
    for name in ("p4ho_evidence", "profile"):
        binding = contract["parents"][name]
        path = root / binding["path"]
        if not path.is_file() or sha256_file(path) != binding["sha256"]:
            raise ValueError(f"P4HP parent drift: {name}")
    evidence = json.loads(
        (root / contract["parents"]["p4ho_evidence"]["path"]).read_text("utf-8")
    )
    if (
        evidence.get("decision")
        != contract["parents"]["p4ho_evidence"]["required_decision"]
    ):
        raise ValueError("P4HP P4HO decision drift")
    if (
        sha256_file(root / "native/film_physics/nf_gamma_density_f64_v1.c")
        != contract["parents"]["p4ho_native_source_sha256"]
    ):
        raise ValueError("P4HP P4HO native source drift")
    ...
    threshold = float(candidate["high_shape_threshold"])
    rows: dict[str, Any] = {}
    outputs: dict[str, np.ndarray] = {}
    group = int(candidate["uniform_grid_count"])
    for name, build in {"msvc": msvc, "llvm_mingw": llvm}.items():
        library = load_native_hybrid_gamma_density_library(Path(build["dll_path"]))
        first, diagnostics = apply_native_hybrid_gamma_density(
            library,
            uniforms,
            shapes,
            scales,
            inverse_iterations=80,
            high_shape_threshold=threshold,
        )
        second, second_diagnostics = apply_native_hybrid_gamma_density(
            library,
            uniforms,
            shapes,
            scales,
        ...
        rows[name] = {
            ...
            "positive_finite": bool(np.all(np.isfinite(first)) and np.all(first > 0)),
            "monotone": bool(
                all(
                    np.all(np.diff(first[start : start + group]) > 0)
                    for start in range(0, first.size, group)
                )
            ),
            "failure_atomic": _failure_atomic(
                library, uniforms, shapes, scales, 80, threshold
            ),
    ...
    checks = {
        ...
        "both_branches": all(
            row["diagnostics"]["direct_branch_count"] > 0
            and row["diagnostics"]["asymptotic_branch_count"] > 0
            for row in rows.values()
        ),
    }
    passed = all(checks.values())
    report: dict[str, Any] = {
        "schema": "neuro-film.u6-p4hp-hybrid-native-gamma-density-inverse-result.v1",
