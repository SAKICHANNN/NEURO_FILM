"""U6.P2AT interpolation confirmation for positive density parameters."""
...
class PositiveDensityParameterInterpolationError(RuntimeError):
    pass
...
def load_contract(path: Path) -> dict[str, Any]:
    ...
    if (
        payload.get("schema")
        != "neuro_film.u6_p2at_softplus_density_parameter_interpolation_contract.v1"
    ):
        raise PositiveDensityParameterInterpolationError("unsupported P2AT contract")
    return payload
...
def _load_parent(root: Path, binding: dict[str, Any]) -> dict[str, Any]:
    ...
    if _sha(path) != binding["sha256"]:
        raise PositiveDensityParameterInterpolationError("P2AT parent mismatch")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("automatic_pass") is not binding["required_automatic_pass"]:
        raise PositiveDensityParameterInterpolationError(
            "P2AT parent decision mismatch"
        )
    ...
    if (
        ...
        raise PositiveDensityParameterInterpolationError("P2AT parent profile mismatch")
    return payload


def run_audit(*, root: Path, contract: dict[str, Any]) -> dict[str, Any]:
    positive = _load_parent(root, contract["parents"]["positive_nodes"])
    _load_parent(root, contract["parents"]["historical_amplitude"])
    nodes = contract["parameter_nodes"]
    profile = SoftplusDensityParameterProfile(
        source_evidence_id=positive["stable_evidence_id"],
        densities=tuple(nodes["densities"]),
        a=tuple(nodes["a"]),
        b=tuple(nodes["b"]),
    )
    restored = SoftplusDensityParameterProfile.from_dict(profile.to_dict())
    ...
    parameter_a, parameter_b = profile.parameters(densities)
    ...
    stable = {
        "schema": "neuro_film.u6_p2at_softplus_density_parameter_interpolation_report.v1",
