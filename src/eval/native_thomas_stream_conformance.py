"""Build and load the U6.P8CI freestanding Thomas stream program."""
...
SOURCE_PATHS = (
    ...
    "native/film_physics/nf_thomas_stream_f32_v1.c",
)
HEADER_PATHS = (
    ...
    "native/film_physics/nf_thomas_stream_f32_v1.h",
)

StreamSink = ctypes.CFUNCTYPE(
    ctypes.c_int,
    ctypes.c_void_p,
    ctypes.c_size_t,
    ctypes.c_size_t,
...
class NativeThomasStreamConformanceError(RuntimeError):
    """Raised when the native stream program build or ABI fails."""


def load_library(path: Path) -> ctypes.CDLL:
    library = ctypes.CDLL(str(path))
    library.nf_thomas_stream_f32_abi_version_v1.argtypes = []
    library.nf_thomas_stream_f32_abi_version_v1.restype = ctypes.c_uint32
    library.nf_thomas_stream_f32_workspace_floats_v1.argtypes = [
        ctypes.c_size_t,
        ctypes.c_size_t,
        ctypes.POINTER(ctypes.c_size_t),
    ]
    library.nf_thomas_stream_f32_workspace_floats_v1.restype = ctypes.c_int
    library.nf_thomas_stream_f32_density_v1.argtypes = [
    ...
    library.nf_thomas_stream_f32_density_v1.restype = ctypes.c_int
    if library.nf_thomas_stream_f32_abi_version_v1() != 1:
        raise NativeThomasStreamConformanceError("Thomas stream ABI version mismatch")
    return library


def _build_report(root: Path, toolchain: str, binary: Path) -> dict[str, Any]:
    return {
        "toolchain": toolchain,
...
def build_msvc(root: Path, output_dir: Path) -> dict[str, Any]:
    ...
    dll = (output_dir / "nf_thomas_stream_msvc_v1.dll").resolve()
    import_library = (output_dir / "nf_thomas_stream_msvc_v1.lib").resolve()
    batch = output_dir / "build_nf_thomas_stream_msvc_v1.bat"
    ...
    if completed.returncode != 0 or not dll.is_file():
        raise NativeThomasStreamConformanceError(
            "MSVC Thomas stream build failed:\n"
            + (completed.stdout + completed.stderr).decode(errors="replace")
        )
    return _build_report(root, "msvc-x64-c11-strict", dll)


def build_llvm(root: Path, output_dir: Path, clang: Path) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    dll = (output_dir / "nf_thomas_stream_llvm_v1.dll").resolve()
    command = [
        str(clang),
    ...
    if completed.returncode != 0 or not dll.is_file():
        raise NativeThomasStreamConformanceError(
            "LLVM Thomas stream build failed:\n"
            + (completed.stdout + completed.stderr).decode(errors="replace")
        )
    report = _build_report(root, "llvm-mingw-x64-c11-strict", dll)
...
__all__ = [
    "NativeThomasStreamConformanceError",
    "StreamSink",
    "build_and_load",
    "build_llvm",
    "build_msvc",
