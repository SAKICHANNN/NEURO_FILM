...
def solve_cdfe_assignment(
    ...
    required_query_mask = 63
    categories = sorted(REQUIRED_COVERAGE)
    query_masks = {row["identity"]: sum(1 << i for i, category in enumerate(categories) if category in row.get("query_coverage", []) and row.get("query_coverage_reviewed") is True) for row in rows}
    indexed = {row["identity"]: row for row in rows}
    if not set(smoke_ids) <= indexed.keys() or any(indexed[i]["exposure"] != "SUBSTANTIVE_DEVELOPMENT" for i in smoke_ids):
        raise ValueError("Smoke captures must be known consumed sources")
    smoke_cameras = {indexed[i]["camera"] for i in smoke_ids}
    forbidden_components = {row["component"] for row in rows if row["exposure"] != "ADMISSION_ONLY_NOT_YET_FRESH_CERTIFIED"}
    ...
    if type(required_query_mask) is not int or not 0 <= required_query_mask <= 63:
        raise ValueError("Required query mask must be a six-bit integer")
    ...
    if set(query_masks) != set(identities) or any(
        type(mask) is not int or not 0 <= mask <= 63 for mask in query_masks.values()
    ):
        raise ValueError("Every component needs an explicitly bound six-bit query mask")
    if not rows:
    ...
    camera_fit = np.arange(5 * n + m, 5 * n + 2 * m)
    camera_rep = np.arange(5 * n + 2 * m, size)
    ...
    for i, row in enumerate(rows):
        components.setdefault(row["component"], []).append(i)
        fit_ok = row["exposure"] == "SUBSTANTIVE_DEVELOPMENT" and row["technical_eligible"] is True
        fresh_ok = (row["exposure"] == "ADMISSION_ONLY_NOT_YET_FRESH_CERTIFIED" and
                    row["technical_eligible"] is True and row["history_separation_established"] is True and
                    row["component"] not in forbidden_components)
        if not fit_ok:
            constrain([(fit[i], 1)], 0, 0)
        if not fresh_ok:
            constrain([(indices[i], 1) for indices in (val, rep, held, query)], 0, 0)
        if row.get("query_coverage_reviewed") is not True:
            constrain([(query[i], 1)], 0, 0)
    for members in components.values():
        constrain([(indices[i], 1) for i in members for indices in (val, rep, held, query)], 0, 1)
    camera_members = {}
    for j, camera in enumerate(cameras):
        ...
        constrain([(val[i], 1) for i in members] + [(rep[i], 1) for i in members] + [(query[i], 1) for i in members] +
                  [(camera_fit[j], -len(members))], -np.inf, 0)
        constrain([(camera_fit[j], 1), (camera_held[j], 1)], 0, 1)
        constrain([(rep[i], 1) for i in members] + [(camera_rep[j], -2)], 0, np.inf)
        constrain([(rep[i], 1) for i in members] + [(camera_rep[j], -16)], -np.inf, 0)
        if camera in smoke_cameras:
            constrain([(camera_held[j], 1)], 0, 0)
    for bit in range(6):
    ...
    validate_cdfe_assignment(list(rows), assignment, list(smoke_ids))
    return assignment
