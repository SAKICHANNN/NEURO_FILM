"""U6.P2AU fresh confirmation of mean-anchored density interpolation."""
...
class MeanAnchoredDensityInterpolationError(RuntimeError):
    pass
...
def load_contract(path: Path) -> dict[str, Any]:
    ...
    if (
        payload.get("schema")
        != "neuro_film.u6_p2au_softplus_mean_coordinate_interpolation_contract.v1"
    ):
        raise MeanAnchoredDensityInterpolationError("unsupported P2AU contract")
    return payload
...
def _load_parent(root: Path, binding: dict[str, Any]) -> dict[str, Any]:
    ...
    if _sha(path) != binding["sha256"]:
        raise MeanAnchoredDensityInterpolationError("P2AU parent mismatch")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("automatic_pass") is not binding["required_automatic_pass"]:
        raise MeanAnchoredDensityInterpolationError("P2AU parent decision mismatch")
    if "required_decision" in binding and (
        payload.get("decision") != binding["required_decision"]
    ):
        raise MeanAnchoredDensityInterpolationError("P2AU parent branch mismatch")
    return payload


def run_audit(*, root: Path, contract: dict[str, Any]) -> dict[str, Any]:
    node_fit = _load_parent(root, contract["parents"]["node_fit"])
    _load_parent(root, contract["parents"]["raw_parameter_interpolation_boundary"])
    ...
    restored = MeanAnchoredSoftplusDensityParameterProfile.from_dict(profile.to_dict())
    ...
    stable = {
        "schema": "neuro_film.u6_p2au_softplus_mean_coordinate_interpolation_report.v1",
