"""P4GX photographic test of retained Sigma-D on independent NPS topology."""
...
SCHEMA = (
    "neuro-film.u6-p4gx-density-compiled-independent-nps-"
    "photographic-development-contract.v1"
)
...
def load_contract(path: Path) -> dict[str, Any]:
    ...
    if payload.get("schema") != SCHEMA:
        raise ValueError("unsupported P4GX contract")
    return payload


def _load_parent(
    root: Path, binding: dict[str, Any], *, required_key: str
) -> dict[str, Any]:
    path = root / binding["path"]
    if not path.is_file() or sha256_file(path) != binding["sha256"]:
        raise ValueError(f"P4GX parent drift: {path}")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get(required_key) != binding[f"required_{required_key}"]:
        raise ValueError(f"P4GX parent decision drift: {path}")
    return payload
...
def evaluate(
    ...
    _load_parent(root, parents["p4gw_result"], required_key="decision")
    _load_parent(root, parents["p4bw_decision"], required_key="decision")
    _load_parent(root, parents["p4bx_decision"], required_key="decision")

    profile_path = root / parents["p4bw_bundle"]["path"]
    prior_path = root / parents["p2q_bundle"]["path"]
    if (
        sha256_file(profile_path) != parents["p4bw_bundle"]["sha256"]
        or sha256_file(prior_path) != parents["p2q_bundle"]["sha256"]
    ):
        raise ValueError("P4GX amplitude parent drift")
    ...
    if (
        profile.identity() != parents["p4bw_bundle"]["required_profile_id"]
        or prior.identity() != parents["p2q_bundle"]["required_prior_id"]
    ):
        raise ValueError("P4GX amplitude identity drift")
    ...
    if sha256_file(manifest_path) != source_contract["manifest_sha256"]:
        raise ValueError("P4GX source manifest drift")
    ...
    if (
        ...
        or any(
            ...
            or row.get("decoded_color_state")
            != source_contract["required_color_state"]
            for row in manifest
        )
    ):
        raise ValueError("P4GX source population drift")
    ...
    if (
        ...
        raise ValueError("P4GX frozen amplitude policy drift")
    ...
    with __import__("tempfile").TemporaryDirectory(
        ...
        for index, source_row in enumerate(manifest):
            ...
            physical, diagnostics = apply_density_compiled_independent_nps(
                source,
                profile=profile,
                prior=prior,
            ...
            row = {
                ...
                "minimum_compiled_sigma_d": diagnostics[
                    "minimum_compiled_sigma_d"
                ],
                "maximum_compiled_sigma_d": diagnostics[
                    "maximum_compiled_sigma_d"
                ],
                "mean_compiled_sigma_d": diagnostics["mean_compiled_sigma_d"],
