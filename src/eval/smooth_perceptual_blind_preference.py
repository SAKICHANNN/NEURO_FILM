"""Blinded autonomous comparison of fixed BK7, AO6 and safe-rich."""
...
def _validate_inputs(
    ...
    if (
        ...
        raise ValueError("BK9 render population drift")
...
def build_blind_sheets(
    *, root: Path, config: dict[str, Any], output_dir: Path
    ...
    if set(records) != expected_keys:
        raise ValueError("BK9 render inventory mismatch")
    ...
    for round_index in (1, 2, 3):
        ...
        for source_row in manifest:
            ...
            if sha256_file(source_path) != source_row["decoded_sha256"]:
                raise ValueError("BK9 source identity drift")
            ...
            for arm in ARMS:
                ...
                if sha256_file(path) != record["output_sha256"]:
                    raise ValueError("BK9 render identity drift")
                arm_paths[arm] = path
            with Image.open(source_path) as opened:
                source = opened.convert("RGB")
                source.thumbnail((340, 220), Image.Resampling.LANCZOS)
            candidates = []
            for arm in order:
                with Image.open(arm_paths[arm]) as opened:
                    image = opened.convert("RGB")
                    image.thumbnail((340, 220), Image.Resampling.LANCZOS)
                candidates.append(image)
            tile = Image.new("RGB", (1400, 270), "white")
            draw = ImageDraw.Draw(tile)
            font = ImageFont.load_default()
            draw.text((6, 4), source_id, fill="black", font=font)
            for index, (label, image) in enumerate(
                zip(("Source", *labels), (source, *candidates), strict=True)
            ):
                x = 6 + index * 348
                draw.text((x, 21), label, fill="black", font=font)
                tile.paste(image, (x, 39))
            tiles.append(tile)
        ...
        for part, start in enumerate(range(0, len(tiles), 4), start=1):
            ...
            sheet = Image.new(
                "RGB", (1400, len(selected) * 270 + 30), "white"
            )
            ImageDraw.Draw(sheet).text(
                (6, 7),
                f"BK9 blind round {round_index} part {part}",
                fill="black",
                font=ImageFont.load_default(),
            )
            for index, tile in enumerate(selected):
                sheet.paste(tile, (0, 30 + index * 270))
            sheet_path = (
                output_dir
                / f"blind_round_{round_index}_part_{part}.png"
            )
    ...
    receipt = {
        "schema": "neuro_film.u5_r2bk9_blind_build.v1",
...
def adjudicate_blind(
    *,
    ...
    if receipt.get("experiment_id") != config["experiment_id"]:
        raise ValueError("BK9 build receipt mismatch")
    expected_mappings = {
        int(row["round"]): row
    ...
    if set(expected_mappings) != {1, 2, 3}:
        raise ValueError("BK9 mapping inventory mismatch")
    ...
    for round_index in (1, 2, 3):
        ...
        if mapping_hash != expected_mappings[round_index]["mapping_sha256"]:
            raise ValueError("BK9 mapping identity drift")
        ...
        if int(observation["round"]) != round_index:
            raise ValueError("BK9 observation round mismatch")
        votes = observation["votes"]
        if set(votes) != set(by_id):
            raise ValueError("BK9 vote population mismatch")
        ...
        for source_id, label in votes.items():
            ...
            if arm not in round_counts:
                raise ValueError("invalid BK9 blind vote")
        ...
        round_wins[winner] += 1
        bk7_count = round_counts[ARMS[0]]
        ao6_count = round_counts[ARMS[1]]
        pairwise = (
            ARMS[0]
            if bk7_count > ao6_count
            else ARMS[1]
            if ao6_count > bk7_count
            else "tie"
        )
        ...
        decoded_rounds.append(
            {
                ...
                "counts": round_counts,
                "winner": winner,
                "bk7_vs_ao6_winner": pairwise,
            }
        )
    gate = config["blind_gate"]
    total_votes = sum(counts.values())
    bk7_share = counts[ARMS[0]] / total_votes
    pairwise_total = counts[ARMS[0]] + counts[ARMS[1]]
    pairwise_share = counts[ARMS[0]] / pairwise_total if pairwise_total else 0.0
    severe_count = int(observations["confirmed_severe_artifact_count"])
    passed = bool(
        severe_count == 0
        and round_wins[ARMS[0]]
        >= int(gate["minimum_bk7_overall_round_wins"])
        and bk7_share >= float(gate["minimum_bk7_total_choice_share"])
        and pairwise_round_wins[ARMS[0]]
        >= int(gate["minimum_bk7_round_wins_vs_ao6"])
        and pairwise_share
        >= float(gate["minimum_bk7_choice_share_vs_ao6"])
    )
    core = {
        "schema": "neuro_film.u5_r2bk9_blind_result.v1",
        "experiment_id": config["experiment_id"],
        ...
        "bk7_vs_ao6_round_wins": pairwise_round_wins,
        "bk7_total_choice_share": bk7_share,
        "bk7_choice_share_vs_ao6": pairwise_share,
