"""Output-exact row statistics for the CB32 nonexpansive target."""
...
def nonexpansive_fraction_transport_target_statistics_streamed(
    ...
    *,
    ...
    """Keep exact global reductions while bounding hue-unit intermediates by rows."""
    ...
    if (
        ...
        raise NonexpansiveFractionTransportError("CB63 input drift")
    ...
    for y0 in range(0, height, row_chunk):
        ...
        offset += count
    if offset < 2:
        raise NonexpansiveFractionTransportError("CB63 hue population failed")
    angle = float(
        np.arctan2(
            float(np.sum(cross_terms[:offset])),
            float(np.sum(dot_terms[:offset])),
        )
    )
    ...
    if (
        ...
        raise NonexpansiveFractionTransportError("CB63 target invariant failed")
    return target


__all__ = ["nonexpansive_fraction_transport_target_statistics_streamed"]
