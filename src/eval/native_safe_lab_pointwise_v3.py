"""Host-native v3 NumPy-compatible pointwise safe-Lab boundary."""
...
SOURCE = "native/color_engine/nf_safe_lab_pointwise_f32_v3.c"
HEADER = "native/color_engine/nf_safe_lab_pointwise_f32_v3.h"


class NativeSafeLabPointwiseV3Error(RuntimeError):
    """Raised when the v3 native pointwise kernel rejects an invocation."""


def build_native_safe_lab_pointwise_v3(*, root: Path, output_dir: Path) -> dict[str, Any]:
    return build_msvc_c11_dll(
        root=root,
        output_dir=output_dir,
        source_relative=SOURCE,
        header_relative=HEADER,
        basename="nf_safe_lab_pointwise_f32_v3",
    )


def load_native_safe_lab_pointwise_v3(dll_path: Path) -> ctypes.CDLL:
    library = ctypes.CDLL(str(Path(dll_path).resolve()))
    function = library.nf_safe_lab_pointwise_f32_v3
...
def apply_native_safe_lab_pointwise_v3(
    library: ctypes.CDLL,
    ...
    *,
    ...
    status = library.nf_safe_lab_pointwise_f32_v3(
    ...
    if status != 0:
        raise NativeSafeLabPointwiseV3Error(
            f"native pointwise safe-Lab v3 rejected status {status}"
        )
    return output
