...
def load_contract(path: Path, root: Path) -> dict[str, Any]:
    ...
    if arms[-1].get("stock_id") != "fujifilm_velvia_50" or arms[-1].get("role") != "velvia_display_proxy_baseline_only":
...
def _read_rgb16(path: Path) -> np.ndarray:
    ...
    if value is None or value.dtype != np.uint16 or value.ndim != 3 or value.shape[2] != 3:
...
def _delta_metrics(left: np.ndarray, right: np.ndarray) -> dict[str, float]:
    ...
    for start in range(0, left.shape[0], 128):
        ...
        distances.append(np.sqrt(np.sum((a - b) ** 2, axis=2, dtype=np.float32)).reshape(-1))
    distance = np.concatenate(distances)
    return {"median_delta_e76": float(np.median(distance)), "p95_delta_e76": float(np.quantile(distance, 0.95))}
...
def _boundary_metrics(source: np.ndarray, output: np.ndarray) -> dict[str, float]:
    ...
    return {
        ...
        "new_output_code_boundary_fraction": float(np.mean(output_boundary & ~source_boundary)),
    }
...
def evaluate(contract_path: Path, root: Path, output_dir: Path, order: str) -> dict[str, Any]:
    ...
    rows = list(p98["rows"])
    if len(rows) != contract["input"]["expected_rows"] or len({row["camera_make"] for row in rows}) != contract["input"]["expected_camera_makes"]:
    ...
    for row in processing:
        ...
        if working.working_space != contract["input"]["working_space"] or working.transfer_state != contract["input"]["transfer_state"]:
            raise DngThreeStockProxyError("P98 WorkingImage boundary drift")
        display = np.ascontiguousarray(
            apply_working_image_aces2_output(working, contract["input"]["display_target"]),
            dtype=np.float32,
        )
        if not np.isfinite(display).all() or np.any((display < 0.0) | (display > 1.0)):
            raise DngThreeStockProxyError("ACES 2 SDR output is non-finite or unbounded")
        ...
        for style in ("velvia_50", "portra_400", "ektar_100"):
            ...
            values = np.ascontiguousarray(
                style_transfer_rgb(display, stats[style], style, seed=1729, guardrails=guardrails, **_style_kwargs(profile)),
                dtype=np.float32,
            )
            if not np.isfinite(values).all() or np.any((values < 0.0) | (values > 1.0)):
                raise DngThreeStockProxyError(f"legacy proxy is non-finite or unbounded: {style}")
            ...
            inventory[arm["arm_id"]] = {"relative_path": relative.as_posix(), "png_sha256": png_sha, "pixel_sha256": hashlib.sha256(code.tobytes()).hexdigest(), **_boundary_metrics(source_code, code)}

        apply_base, apply_residual = build_source_context_display_look_stages(ao6_payload, display)
        ao6 = np.ascontiguousarray(apply_residual(apply_base(display)), dtype=np.float32)
        ...
        inventory[ao6_id] = {"relative_path": relative.as_posix(), "png_sha256": png_sha, "pixel_sha256": hashlib.sha256(ao6_code.tobytes()).hexdigest(), **_boundary_metrics(source_code, ao6_code)}
        ...
        report_rows.append({
            ...
            "outputs": inventory,
            "comparisons": {
                "velvia_vs_portra": _delta_metrics(rendered[velvia], rendered[portra]),
                "velvia_vs_ektar": _delta_metrics(rendered[velvia], rendered[ektar]),
                "portra_vs_ektar": _delta_metrics(rendered[portra], rendered[ektar]),
                "ao6_vs_legacy_velvia": _delta_metrics(rendered[ao6_id], rendered[velvia]),
            },
        })
    ...
    pairwise = {name: float(np.median([row["comparisons"][name]["median_delta_e76"] for row in report_rows])) for name in pair_names}
    ao6_sep = float(np.median([row["comparisons"]["ao6_vs_legacy_velvia"]["median_delta_e76"] for row in report_rows]))
    ...
    maximum_new_boundary = max(row["new_output_code_boundary_fraction"] for row in outputs)
    gates = contract["gates"]
    failed = [name for name, value in pairwise.items() if value < gates["minimum_pairwise_population_median_delta_e76"]]
    ...
    report = {**scientific, "execution_order": order, "stable_evidence_id": _canonical_sha(scientific)}
...
def main() -> int:
    ...
    report = evaluate(args.contract.resolve(), args.root.resolve(), args.output_dir.resolve(), args.order)
    print(json.dumps({"decision": report["decision"], "stable_evidence_id": report["stable_evidence_id"]}, sort_keys=True))
    return 0
