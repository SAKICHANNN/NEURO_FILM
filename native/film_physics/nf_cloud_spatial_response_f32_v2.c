...
static int nf_profile_valid(
    ...
    if (profile == NULL || profile->struct_size != sizeof(*profile) ||
        profile->abi_version != NF_CLOUD_SPATIAL_RESPONSE_F32_ABI_VERSION_V2 ||
    ...
    for (size_t channel = 0u; channel < 3u; ++channel) {
        if (!isfinite(profile->sigma_pixels_cmy[channel]) ||
            ...
            nf_radius(profile->sigma_pixels_cmy[channel], profile->truncate) >
                NF_CLOUD_SPATIAL_RESPONSE_F32_MAX_RADIUS_V2 ||
...
uint32_t nf_cloud_spatial_response_f32_abi_version_v2(void) {
    return NF_CLOUD_SPATIAL_RESPONSE_F32_ABI_VERSION_V2;
}

nf_cloud_spatial_response_f32_status_v2 nf_cloud_spatial_response_f32_apply_v2(
    const nf_cloud_spatial_response_f32_profile_v2* profile,
    ...
    if (!nf_profile_valid(profile) || counts == NULL || workspace == NULL ||
        ...
        return NF_CLOUD_SPATIAL_RESPONSE_F32_INVALID_ARGUMENT_V2;
    }
    ...
    if (workspace_doubles < extended_pixels ||
        core_pixels > SIZE_MAX / 3u || output_values < 3u * core_pixels) {
        return NF_CLOUD_SPATIAL_RESPONSE_F32_INVALID_ARGUMENT_V2;
    }
    for (size_t channel = 0u; channel < 3u; ++channel) {
        if (nf_radius(profile->sigma_pixels_cmy[channel], profile->truncate) > halo) {
            return NF_CLOUD_SPATIAL_RESPONSE_F32_DOMAIN_ERROR_V2;
        }
    }
    horizontal = workspace;
    for (size_t channel = 0u; channel < 3u; ++channel) {
        double weights[2u * NF_CLOUD_SPATIAL_RESPONSE_F32_MAX_RADIUS_V2 + 1u];
        ...
        for (size_t y = halo; y < halo + core_height; ++y) {
            for (size_t x = 0u; x < width; ++x) {
                double total = 0.0;
                for (size_t k = 0u; k <= 2u * radius; ++k) {
                ...
                const size_t output = ((y - halo) * width + x) * 3u + channel;
                const double density = total *
    ...
    return NF_CLOUD_SPATIAL_RESPONSE_F32_OK_V2;
}
