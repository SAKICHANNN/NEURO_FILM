#define NF_BOUNDED_ADJACENCY_F32_BUILD
...
#define NF_F32_TRANSMITTANCE_TOLERANCE_V1 2.0e-6
...
static int nf_profile_valid(
    ...
    if (
        profile == NULL ||
        profile->struct_size !=
            sizeof(nf_bounded_adjacency_f32_profile_v1) ||
        profile->abi_version !=
...
uint32_t nf_bounded_adjacency_f32_abi_version_v1(void) {
    return NF_BOUNDED_ADJACENCY_F32_ABI_VERSION_V1;
}
...
nf_bounded_adjacency_f32_validate_profile_v1(
    const nf_bounded_adjacency_f32_profile_v1* profile) {
    if (profile == NULL) {
        return NF_BOUNDED_ADJACENCY_F32_INVALID_ARGUMENT_V1;
    }
    return nf_profile_valid(profile)
        ? NF_BOUNDED_ADJACENCY_F32_OK_V1
        : NF_BOUNDED_ADJACENCY_F32_INVALID_PROFILE_V1;
}
...
nf_bounded_adjacency_f32_apply_v1(
    ...
    size_t index;
    size_t channel;
    const nf_bounded_adjacency_f32_status_v1 profile_status =
        nf_bounded_adjacency_f32_validate_profile_v1(profile);
    if (profile_status != NF_BOUNDED_ADJACENCY_F32_OK_V1) {
    ...
    for (index = 0; index < rgb_count; ++index) {
        for (channel = 0; channel < 3; ++channel) {
            ...
            const double blurred =
                (double)blurred_density_rgb[index * 3u + channel];
            if (
                !isfinite(density) ||
                !isfinite(blurred) ||
                density <
                ...
                return NF_BOUNDED_ADJACENCY_F32_INVALID_INPUT_V1;
            }
        }
    }
    for (index = 0; index < rgb_count; ++index) {
        for (channel = 0; channel < 3; ++channel) {
            ...
            const double limit =
                raw >= 0.0 ? positive_limit : negative_limit;
            ...
            rounded = (float)output;
            if (
                !isfinite((double)rounded) ||
                (double)rounded <
                    profile->black_reference_density[channel] -
                    NF_F32_DENSITY_TOLERANCE_V1 ||
                (double)rounded >
                    ...
                    NF_F32_DENSITY_TOLERANCE_V1 ||
                fabs(
                    pow(10.0, -(double)rounded) - transmittance
                ) >
                    profile->maximum_absolute_transmittance_delta +
                    NF_F32_TRANSMITTANCE_TOLERANCE_V1
            ) {
                return NF_BOUNDED_ADJACENCY_F32_NUMERIC_FAILURE_V1;
            }
            output_density_rgb[index * 3u + channel] = rounded;
        }
    }
    return NF_BOUNDED_ADJACENCY_F32_OK_V1;
}
