#define NF_PHYSICAL_DOMAINS_BUILD
#include "nf_physical_domains_v1.h"
...
static int nf_profile_numeric_valid(
    ...
    for (channel = 0; channel < 3; ++channel) {
        const uint32_t count = profile->knot_count[channel];
        if (count < 2 || count > NF_PHYSICAL_DOMAINS_MAX_KNOTS_V1) {
            return 0;
        }
...
uint32_t nf_physical_domains_abi_version_v1(void) {
    return NF_PHYSICAL_DOMAINS_ABI_VERSION_V1;
}

nf_physical_domains_status_v1 nf_physical_domains_validate_profile_v1(
    const nf_physical_domains_profile_v1* profile) {
    double raw_black[3];
    double raw_white[3];
    size_t channel;
    if (profile == NULL) {
        return NF_PHYSICAL_DOMAINS_INVALID_ARGUMENT_V1;
    }
    if (
        profile->struct_size != sizeof(nf_physical_domains_profile_v1) ||
        profile->abi_version != NF_PHYSICAL_DOMAINS_ABI_VERSION_V1 ||
        !nf_is_hex_sha256(profile->source_component_sha256) ||
        !nf_profile_numeric_valid(profile)
    ) {
        return NF_PHYSICAL_DOMAINS_INVALID_PROFILE_V1;
    }
    nf_raw_reflectance(
    ...
    return NF_PHYSICAL_DOMAINS_OK_V1;
}

nf_physical_domains_status_v1 nf_physical_sensitometry_apply_v1(
    const nf_physical_domains_profile_v1* profile,
    ...
    size_t index;
    size_t channel;
    const nf_physical_domains_status_v1 status =
        nf_physical_domains_validate_profile_v1(profile);
    if (status != NF_PHYSICAL_DOMAINS_OK_V1) {
        return status;
    }
    if (
    ...
    for (index = 0; index < rgb_count * 3u; ++index) {
        if (
            !isfinite(scene_linear_rgb[index]) ||
            scene_linear_rgb[index] < 0.0 ||
            scene_linear_rgb[index] > 1.0
        ) {
            return NF_PHYSICAL_DOMAINS_INVALID_INPUT_V1;
        }
    }
    for (index = 0; index < rgb_count; ++index) {
        for (channel = 0; channel < 3; ++channel) {
            const double exposure = log10(
                (
                    scene_linear_rgb[index * 3u + channel] +
...
nf_physical_domains_status_v1 nf_physical_interpretation_apply_v1(
    const nf_physical_domains_profile_v1* profile,
    ...
    for (index = 0; index < rgb_count; ++index) {
        double raw[3];
        nf_raw_reflectance(
            profile, &developed_density_rgb[index * 3u], raw);
        for (channel = 0; channel < 3; ++channel) {
            double value = (
            ...
            if (
                ...
                return NF_PHYSICAL_DOMAINS_NUMERIC_FAILURE_V1;
            }
            if (value < 0.0) {
                value = 0.0;
            } else if (value > 1.0) {
            ...
            scan_linear_rgb[index * 3u + channel] = value;
        }
    }
    return NF_PHYSICAL_DOMAINS_OK_V1;
}
