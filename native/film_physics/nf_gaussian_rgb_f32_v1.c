#define NF_GAUSSIAN_F32_BUILD
#include "nf_gaussian_rgb_f32_v1.h"

#include <math.h>

#define NF_GAUSSIAN_F32_MAX_DIAMETER_V1 \
    (2u * NF_GAUSSIAN_F32_MAX_RADIUS_V1 + 1u)
...
static uint32_t nf_radius(double sigma, double truncate) {
    ...
    if (
        !isfinite(scaled) ||
        scaled + 0.5 >=
            (double)(NF_GAUSSIAN_F32_MAX_RADIUS_V1 + 1u)
    ) {
        return NF_GAUSSIAN_F32_MAX_RADIUS_V1 + 1u;
    }
...
static int nf_profile_valid(
    ...
    if (
        profile == NULL ||
        profile->struct_size != sizeof(nf_gaussian_f32_profile_v1) ||
...
static void nf_build_weights(
    double sigma,
    uint32_t radius,
    double weights[NF_GAUSSIAN_F32_MAX_DIAMETER_V1]) {
    uint32_t index;
    double sum = 0.0;
    if (radius == 0u) {
        weights[0] = 1.0;
...
uint32_t nf_gaussian_f32_abi_version_v1(void) {
    return NF_GAUSSIAN_F32_ABI_VERSION_V1;
}

nf_gaussian_f32_status_v1 nf_gaussian_f32_validate_profile_v1(
    const nf_gaussian_f32_profile_v1* profile) {
    if (profile == NULL) {
    ...
    return nf_profile_valid(profile)
        ...
        : NF_GAUSSIAN_F32_INVALID_PROFILE_V1;
}

nf_gaussian_f32_status_v1 nf_gaussian_f32_required_halo_v1(
    const nf_gaussian_f32_profile_v1* profile,
    uint32_t* halo) {
    uint32_t maximum = 0u;
    size_t channel;
    const nf_gaussian_f32_status_v1 status =
        nf_gaussian_f32_validate_profile_v1(profile);
...
nf_gaussian_f32_status_v1 nf_gaussian_f32_apply_v1(
    const nf_gaussian_f32_profile_v1* profile,
    ...
    size_t x;
    uint32_t radii[3];
    double weights[3][NF_GAUSSIAN_F32_MAX_DIAMETER_V1];
    const nf_gaussian_f32_status_v1 profile_status =
        nf_gaussian_f32_validate_profile_v1(profile);
    if (profile_status != NF_GAUSSIAN_F32_OK_V1) {
    ...
    for (index = 0; index < count; ++index) {
        if (
            !isfinite((double)input_rgb[index]) ||
            input_rgb[index] < 0.0f
        ) {
            return NF_GAUSSIAN_F32_INVALID_INPUT_V1;
        }
    }
    for (channel = 0; channel < 3; ++channel) {
        ...
        for (x = 0; x < width; ++x) {
            for (channel = 0; channel < 3; ++channel) {
                ...
                for (kernel = 0; kernel <= 2u * radius; ++kernel) {
                    ...
                    value += weights[channel][kernel] * (double)
                        input_rgb[
                            (source_y * width + x) * 3u + channel
                        ];
                }
                workspace_rgb[(y * width + x) * 3u + channel] =
                    (float)value;
            }
        }
    }
    for (y = 0; y < height; ++y) {
        for (x = 0; x < width; ++x) {
            for (channel = 0; channel < 3; ++channel) {
                ...
                output_rgb[(y * width + x) * 3u + channel] =
                    (float)value;
            }
        }
    }
...
nf_gaussian_f32_status_v1 nf_gaussian_f32_apply_window_v2(
    const nf_gaussian_f32_profile_v1* profile,
    size_t full_height,
    size_t full_width,
