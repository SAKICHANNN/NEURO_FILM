...
uint32_t nf_density_conditioned_poisson_u16_abi_version_v3(void) {
    return NF_DENSITY_CONDITIONED_POISSON_U16_ABI_VERSION_V3;
}
...
nf_density_conditioned_poisson_u16_sample_region_v3(
    ...
    if (!nf_profile_valid(profile) || scale_cmy == NULL || output_cmy == NULL ||
        ...
        return NF_DENSITY_CONDITIONED_POISSON_U16_INVALID_ARGUMENT_V3;
    }
    pixels = height * width;
    ...
    for (size_t index = 0u; index < pixels * 3u; ++index) {
        if (!isfinite(scale_cmy[index]) || scale_cmy[index] < 0.0 ||
            scale_cmy[index] > 1.0) {
            return NF_DENSITY_CONDITIONED_POISSON_U16_DOMAIN_ERROR_V3;
        }
    }
    for (size_t local_y = 0u; local_y < height; ++local_y) {
        ...
        for (size_t local_x = 0u; local_x < width; ++local_x) {
            ...
            for (size_t index = 0u; index < 7u; ++index) {
                ...
                const double maximum = index < 4u ?
                    (index == 0u ? profile->shared_all_rate :
                        profile->shared_pair_rates_cm_cy_my[index - 1u]) :
                    profile->marginal_rates_cmy[index - 4u];
                const uint32_t pieces = maximum > 0.0 ?
                    (uint32_t)ceil(maximum / 64.0) : 1u;
                component[index] = 0u;
                for (uint32_t piece = 0u; piece < pieces; ++piece) {
                    const uint64_t piece_seed = seed +
                        (uint64_t)piece * UINT64_C(0x9E3779B97F4A7C15);
                    component[index] += (uint32_t)nf_poisson(
                        rates[index] / (double)pieces,
                        nf_uniform(coordinate + piece_seed));
                }
            }
    ...
    return NF_DENSITY_CONDITIONED_POISSON_U16_OK_V3;
}
