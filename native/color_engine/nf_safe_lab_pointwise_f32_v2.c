#include "nf_safe_lab_pointwise_f32_v2.h"
...
static int nf_apply_range(nf_safe_lab_context *context) {
    ...
    for (index = context->begin; index < context->end; ++index) {
        ...
        output[0] = source[0] + context->luma_strength *
        ...
        source_chroma = sqrtf(source[1] * source[1] + source[2] * source[2]);
        ...
        skin_weight = source[0] > 20.0F && source[0] < 92.0F &&
                          ...
                          ? fmin(1.0, fmax(0.0, context->skin_protect))
                          : 0.0F;
        output[1] = (float)((double)output[1] * (1.0 - skin_weight) + (double)source[1] * skin_weight);
        output[2] = (float)((double)output[2] * (1.0 - skin_weight) + (double)source[2] * skin_weight);
        ...
        output_chroma = fmaxf(sqrtf(output[1] * output[1] + output[2] * output[2]), 1e-6F);
...
int nf_safe_lab_pointwise_f32_v2(
    const float *source_lab,
    float *output_lab,
