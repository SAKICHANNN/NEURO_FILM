#include "nf_safe_lab_pointwise_f32_v3.h"
...
typedef struct nf_safe_lab_v3_context {
    ...
    int status;
} nf_safe_lab_v3_context;

static float nf_clip_f32(float value, float low, float high) {
    return value < low ? low : (value > high ? high : value);
}

static float nf_smoothstep_f32(float edge0, float edge1, float value) {
    float denominator = edge1 - edge0;
    float x;
    if (denominator < 1e-6F) {
        denominator = 1e-6F;
    }
    x = nf_clip_f32((value - edge0) / denominator, 0.0F, 1.0F);
    return x * x * (3.0F - 2.0F * x);
}

static double nf_clip_f64(double value, double low, double high) {
    return value < low ? low : (value > high ? high : value);
}

static double nf_smoothstep_f64(double edge0, double edge1, double value) {
    double denominator = edge1 - edge0;
    double x;
    if (denominator < 1e-6) {
        denominator = 1e-6;
    }
    x = nf_clip_f64((value - edge0) / denominator, 0.0, 1.0);
    return x * x * (3.0 - 2.0 * x);
}

static int nf_apply_range_v3(nf_safe_lab_v3_context *context) {
    uint64_t index;
    for (index = context->begin; index < context->end; ++index) {
        ...
        saturated_weight = nf_smoothstep_f32(24.0F, 60.0F, source_chroma);
        ...
        neutral_weight = 1.0F - nf_smoothstep_f32(4.0F, 14.0F, source_chroma);
        neutral_blend = nf_clip_f32(
        ...
        if (context->skin_protect > 0.0) {
            double output_a = (double)output[1];
            double output_b = (double)output[2];
            double skin_weight =
                source[0] > 20.0F && source[0] < 92.0F &&
                        source[1] > 4.0F && source[1] < 28.0F &&
                        source[2] > 4.0F && source[2] < 46.0F
                    ? fmin(1.0, fmax(0.0, context->skin_protect))
                    : 0.0;
            double output_chroma;
            double cap_scale;
            double knee;
            double factor;
            output_a = output_a * (1.0 - skin_weight) +
                       (double)source[1] * skin_weight;
            output_b = output_b * (1.0 - skin_weight) +
                       (double)source[2] * skin_weight;
            output_chroma = fmax(sqrt(output_a * output_a + output_b * output_b), 1e-6);
            cap_scale = fmin(1.0, (double)cap / output_chroma);
            knee = nf_smoothstep_f64(
                0.82, 1.0, output_chroma / fmax((double)cap, 1e-6));
            factor = (1.0 - knee) + knee * cap_scale;
            output[1] = (float)(output_a * factor);
            output[2] = (float)(output_b * factor);
        } else {
            float output_chroma = fmaxf(
                sqrtf(output[1] * output[1] + output[2] * output[2]), 1e-6F);
            float cap_scale = fminf(1.0F, cap / output_chroma);
            float knee = nf_smoothstep_f32(
                0.82F, 1.0F, output_chroma / fmaxf(cap, 1e-6F));
            float factor = (1.0F - knee) + knee * cap_scale;
            output[1] *= factor;
            output[2] *= factor;
        }
        if (!isfinite(output[0]) || !isfinite(output[1]) || !isfinite(output[2])) {
            return NF_SAFE_LAB_V3_NONFINITE_OUTPUT;
        }
    }
    return NF_SAFE_LAB_V3_OK;
}

#if defined(_WIN32)
static DWORD WINAPI nf_thread_entry_v3(LPVOID value) {
    nf_safe_lab_v3_context *context = (nf_safe_lab_v3_context *)value;
    context->status = nf_apply_range_v3(context);
    return 0;
}
#endif

int nf_safe_lab_pointwise_f32_v3(
    const float *source_lab,
    float *output_lab,
    ...
    if (source_lab == NULL || output_lab == NULL || source_mean == NULL ||
        ...
        skin_protect < 0.0 || max_chroma_gain <= 0.0F ||
        max_chroma_boost < 0.0F) {
        return NF_SAFE_LAB_V3_INVALID_ARGUMENT;
    }
    for (channel = 0; channel < 3; ++channel) {
    ...
    for (index = 0; index < pixel_count * 3U; ++index) {
        if (!isfinite(source_lab[index])) {
            return NF_SAFE_LAB_V3_NONFINITE_INPUT;
        }
    }
    ...
    serial.status = NF_SAFE_LAB_V3_OK;
#if defined(_WIN32)
    if (thread_count > 1 && pixel_count >= (uint64_t)thread_count) {
        HANDLE *handles = (HANDLE *)calloc(thread_count, sizeof(HANDLE));
        nf_safe_lab_v3_context *contexts =
            (nf_safe_lab_v3_context *)calloc(thread_count, sizeof(nf_safe_lab_v3_context));
        uint32_t created = 0;
        uint32_t thread;
        if (handles != NULL && contexts != NULL) {
            for (thread = 0; thread < thread_count; ++thread) {
                ...
                handles[thread] = CreateThread(
                    NULL, 0, nf_thread_entry_v3, &contexts[thread], 0, NULL);
            ...
            if (created == thread_count) {
                int status = NF_SAFE_LAB_V3_OK;
                for (thread = 0; thread < thread_count; ++thread) {
                    if (contexts[thread].status != NF_SAFE_LAB_V3_OK) {
...
#endif
    return nf_apply_range_v3(&serial);
}
